var OWM_FriendsConsole = function(params) {
    ...
    this.consoleAcceptRequest = function($node) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data) {
                    $row.remove();
                    MTM.trigger('mobile.console_item_removed', { section: "friend-requests" });
                    if (data.result == true && data.message != undefined) {
                        MTM.info(data.message);
                    }
                }
            }
        });
    ...
    this.consoleLoadMore = function($node) {
        ...
        MTM.loadComponent(
            "FRIENDS_MCMP_ConsoleItems", { limit: 3, exclude: exclude }, {
    ...
    MTM.bind("mobile.console_hide_friends_load_more", function() {
        self.hideLoadMoreButton();
    });

    MTM.bind("mobile.console_load_new_items", function(data) {
    ...
    MTM.bind("mobile.console_item_removed", function(data) {
    ...
    MTM.bind("mobile.hide_sidebar", function(data) {
        if (data.type == "right") {
            MTM.unbind("mobile.console_hide_friends_load_more");
            MTM.unbind("mobile.console_load_new_items");
            $("body")
