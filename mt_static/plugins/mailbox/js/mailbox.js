    ...
    initialize: function() {
        ...
        MT.bind('mailbox.application_started', function() {
            if (self.get('convId') == null) {
                MT.Mailbox.conversationsCollection.on('add', self.bindConversation, this);
            } else {
                var conversation = MT.Mailbox.conversationsCollection.findWhere({ opponentId: self.get('opponentId'), mode: 'chat' });
                if (conversation) {
...
OWMailbox.Application = function(params) {
    ...
    this.setData = function(data) {
        ...
        if (typeof data.userOnlineCount != 'undefined') {
            ...
            MT.trigger('mailbox.user_online_count_update', { userOnlineCount: data.userOnlineCount });
        }
        ...
        if (typeof data.markedUnreadConversationList != 'undefined') {
            ...
            MT.trigger('mailbox.console_update_counter', { unreadMessageList: [] });
            MT.MailboxConsole.updateCounter();
        }

        if (typeof data.messageList != 'undefined') {
            ///var tmpLastMessageTimestamp = MT.Mailbox.lastMessageTimestamp;
            $.each(data.messageList, function() {
                // if (this.timeStamp != self.lastMessageTimestamp){
                MT.trigger('mailbox.message', this);
                //   tmpLastMessageTimestamp = parseInt(this.timeStamp);
                //}
            });
            //MT.Mailbox.lastMessageTimestamp = tmpLastMessageTimestamp;
        }
        ...
        if (!self.appStarted) {
            MT.trigger('mailbox.ready');
        }

        MT.trigger('mailbox.after_ping');
        MT.MailboxConsole.updateCounter();
    }

    MT.getPing().addCommand('mailbox_ping', {
        params: {},
        before: function() {
...
OWMailbox.NewMessageForm.Controller = function(model) {
    ...
    this.closeNewMessageWindowWithConfirmation = function(activeChats) {
        if (this.isNewMessageWindowActive() && !activeChats) {
            ...
            var result = confirm(MT.getLanguageText('mailbox', 'close_new_message_windmt_confirmation'));
            if (result) {
    ...
    this.close = function() {
        MT.trigger('mailbox.close_new_message_form');
    };
    ...
    this.minimize = function(e) {
        ...
        if (self.newMessageWindowControl.hasClass('mt_active')) {
            MT.trigger('mailbox.minimize_new_message_form');
        } else {
            MT.trigger('mailbox.open_new_message_form');
        }
    };
    ...
    this.setUser = function(data) {
        ...
        $('.mt_chat_block', this.newMessageWindowControl).removeClass('mt_mailchat_select_user_wrap');
        $('.mt_chat_block', this.newMessageWindowControl).addClass('mt_mailchat_selected_user_wrap');
    };

    this.resetUser = function() {
        $('.mt_chat_block', this.newMessageWindowControl).addClass('mt_mailchat_select_user_wrap');
        $('.mt_chat_block', this.newMessageWindowControl).removeClass('mt_mailchat_selected_user_wrap');
    };

    this._prepareOpponent = function(data) {
        var opponent = MT.Mailbox.usersCollection.findWhere({ opponentId: data.opponentId });
    ...
    this.openForm = function(data) {
        self.newMessageWindowControl.addClass('mt_open');
        self.newMessageWindowControl.addClass('mt_active');
        self.newMessageWindowControl.removeClass('mt_chat_dialog_active');
        ...
        MT.trigger('mailbox.new_message_form_opened');
    };

    this.openFormMinimized = function(data) {
        self.newMessageWindowControl.addClass('mt_open');
        self.newMessageWindowControl.addClass('mt_chat_dialog_active');
    };
    ...
    this.closeForm = function() {
        ...
        self.newMessageWindowControl.removeClass('mt_open');
        self.newMessageWindowControl.removeClass('mt_active');
        ...
        MT.trigger('mailbox.new_message_form_closed');
        MT.removeScroll($('#userFieldAutocompleteControl'));
    };
    ...
    this.minimizeForm = function() {
        ...
        MT.trigger('mailbox.new_message_form_minimized');
    };
    ...
    MT.bind('mailbox.open_new_message_form', function(data) {
        self.openForm(data);
    });

    MT.bind('mailbox.open_new_message_form_minimized', function(data) {
        self.openFormMinimized(data);
    });

    MT.bind('mailbox.close_new_message_form', function() {
        self.closeForm();
    });

    MT.bind('mailbox.minimize_new_message_form', function() {
        self.minimizeForm();
    });
};
...
OWMailbox.NewMessageForm.restoreForm = function() {
    ...
    if (typeof newMessageFormOpened != 'undefined' && newMessageFormOpened != null) {
        if (newMessageFormOpened == "1") {
            ...
            MT.trigger('mailbox.open_new_message_form_minimized');
        }
    }
...
MT_MailboxConsole = function(itemKey, params) {
    var self = this;
    var listLoaded = false;

    self.model = MT.Console.getData(itemKey);
    var list = MT.Console.getItem(itemKey);
    var counter = new MT_DataModel();
    ...
    this.updateCounter = function(conversation) {
        ...
        var markedUnreadNotViewedConversations = MT.Mailbox.conversationsCollection.where({ conversationViewed: false });
        var markedUnreadConversations = MT.Mailbox.conversationsCollection.where({ conversationRead: 0 });

        //var all = markedUnreadConversations.length;
        //if (MT.Mailbox.markedUnreadConversationList.length > markedUnreadConversations.length){
        //    all = MT.Mailbox.markedUnreadConversationList.length;
        //}
        var all = MT.Mailbox.markedUnreadConversationList.length;
    ...
    MT.bind('mailbox.application_started', function() {
        MT.Mailbox.conversationsCollection.on('add', self.updateCounter, self);
        MT.Mailbox.conversationsCollection.on('change', self.updateCounter, self);
    ...
    this.sendMessageBtn.bind('click', function() {
        MT.trigger('mailbox.open_new_message_form');
        MT.Console.getItem('mailbox').hideContent();
    });

}

MT.MailboxConsole = null;
...
var MailboxUserField = function(id, name) {
    ...
    formElement.setValue = function(value) {
        ...
        if (value == '') {
            ...
            if (opponentId > 0) {
                user = MT.Mailbox.usersCollection.findWhere({ opponentId: opponentId });
            } else {
                user = value;
            }
            ...
            if (user.hasOwnProperty('opponentId')) {
                ...
                MT.Mailbox.newMessageFormController.setUser(user);
                ...
                if (user) {
                    ...
                    MT.Mailbox.newMessageFormController.setUser(user.attributes);
    ...
    formElement.resetValue = function() {
        ...
        MT.Mailbox.newMessageFormController.resetUser();
    ...
    this.removeItem = function(opponentId) {
        ...
        //        MT.updateScroll(self.autocompleteControl);
    };
    ...
    this.updateList = function(name) {
        ...
        //        MT.removeScroll(self.autocompleteControl);
        ...
        if (name == '') {
            ...
            _.each(MT.Mailbox.usersCollection.models, function(user) {
            ...
            if (!self.syncing) {
                ...
                $.getJSON(OWMailbox.userSearchResponderUrl, { term: name, idList: {}, context: 'user' }, function(data) {
                    ...
                    if (size == 0) {
                        ...
                        $('#userFieldUserListItem-notfound').hide();
                        //                        if (size > 8){
                        //                            if (self.autocompleteControl.hasClass('mt_scrollable')){
                        //                                MT.updateScroll($('#userFieldAutocompleteControl'));
                        //                            }
                        //                            else{
                        //                                MT.addScroll($('#userFieldAutocompleteControl'));
                        //                            }
                        //                        }
                    }
    ...
    //    $('.mt_mailchat_autocomplete_inner').on('scroll', function(){
    //
    //        if ( $('.mt_mailchat_autocomplete_inner').scrollTop() + $('#userFieldUserList').position().top == 0 )
    //        {
...
//    $.getScript('/mt_static/plugins/base/js/attachments.js');
//}
...
$(function() {
        ...
        dialogAutosize: function(options, action) {
            ...
            this.adjust = function() {
                ...
                for (var i = 1; i <= linesLength; i++) {
                    ...
                    if (i == 2) {
                        offset = offset + 12;
                        $('.mt_chat_message', options.control).removeClass('scroll');
                    } else {
                        if (i >= 3 && i <= 6) {
                            offset = offset + 17;
                            $('.mt_chat_message', options.control).removeClass('scroll');
                            ...
                            if (i > 6) {
                                $('.mt_chat_message', options.control).addClass('scroll');
                                offset = 80;
                                break;
                            }
                        }
    ...
    MT.bind('mailbox.ready', function() {
        MT.trigger('mailbox.application_started');
        MT.Mailbox.appStarted = true;
    });

    MT.bind('base.online_nmt_click',
        function(userId) {

            if (parseInt(userId) != OWMailbox.userDetails.userId) {
                $('#mt_chat_nmt_' + userId).addClass('mt_hidden');
                $('#mt_preloader_content_' + userId).removeClass('mt_hidden');
                    ...
                    if (typeof data != 'undefined') {
                        if (typeof data['warning'] != 'undefined' && data['warning']) {
                            ...
                            if (data['use_chat'] && data['use_chat'] == 'promoted') {
                                MT.Mailbox.contactManagerView.showPromotion();
                            } else {
                                MT.Mailbox.usersCollection.add(data);
                                MT.trigger('mailbox.open_dialog', { convId: data['convId'], opponentId: data['opponentId'], mode: 'chat' });
                            }
                        }
                    }
                }, 'json').complete(function() {

                    $('#mt_chat_nmt_' + userId).removeClass('mt_hidden');
                    $('#mt_preloader_content_' + userId).addClass('mt_hidden');
                });
            }
        });

    //    MT.bind('base.sign_out_click',
    //        function(userId){
    //        });
