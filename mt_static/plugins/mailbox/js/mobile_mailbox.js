...
MAILBOX_Message = Backbone.Model.extend({
    ...
    readMessage: function(actionParams, callback) {
        ...
        $.ajax({
            'type': 'POST',
            'url': MTM.Mailbox.get('authorizationResponderUrl'),
            'data': {
                'actionParams': actionParams
            },
            'success': function(data) {
                if (typeof data.error != 'undefined') {
                    MTM.error(data.error);
                } else {
                    if (typeof data.authorizationActionText != 'undefined') {
                        MTM.info(data.authorizationActionText);
                    }
        ...
        if (!this.model.get('isSystem')) {
            ...
            if (attachments.length != 0) {
                ...
                if (attachments[i]['type'] == 'image') {
                    this.$el.addClass('mt_dialog_picture_item');
                    ...
                    $('.owm_chat_bubble', this.$el).html('<a href="' + attachments[i]['downloadUrl'] + '" target="_blank">' + MTM.Mailbox.formatAttachmentFileName(attachments[i]['fileName']) + '</a>');
                }
            } else {
...
MAILBOX_MailMessageView = Backbone.View.extend({
    ...
    initialize: function() {
        if (this.model.get('isAuthor')) {
            this.model.set('profileUrl', MTM.Mailbox.get('user').profileUrl);
            this.model.set('avatarUrl', MTM.Mailbox.get('user').avatarUrl);
            this.model.set('displayName', MTM.Mailbox.get('user').displayName);
        } else {
            var user = MTM.Mailbox.userListCollection.findWhere({ opponentId: this.model.get('senderId') });
        ...
        if (!this.model.get('isSystem')) {
            ...
            if (attachments.length != 0) {
                ...
                for (var i = 0; i < attachments.length; i++) {
                    ...
                    $('a', attachment).prepend(MTM.Mailbox.formatAttachmentFileName(attachments[i]['fileName']));
                    $('a', attachment).attr('href', attachments[i]['downloadUrl']);
                    $('.owm_mail_attach_size', attachment).html(MTM.Mailbox.formatAttachmentFileSize(attachments[i]['fileSize']));
...
MAILBOX_ConversationItem = Backbone.Model.extend({
    ...
    initialize: function() {
        ...
        var conversationUnread = MTM.Mailbox.unreadMessageList.findWhere({ convId: this.get('conversationId') });
...
MAILBOX_Conversations = Backbone.Model.extend({
    ...
    initialize: function() {
        MTM.mailboxSidebarMenu.itemList.on('change:selected', this.changeActive, this);
        this.itemList = new MAILBOX_ConversationItemList();
        this.loadList();

        var selectedItem = MTM.mailboxSidebarMenu.itemList.findWhere({ selected: true });
    ...
    loadList: function() {
        ...
        if (n > MTM.Mailbox.convList.length) {
            n = MTM.Mailbox.convList.length;
        }

        for (var i = this.loadedConvCount; i < n; i++) {
            if (typeof MTM.Mailbox.convList[i] != 'undefined') {
                this.itemList.add(MTM.Mailbox.convList[i]);
            }
        }
        this.loadedConvCount = i;

        if (this.loadedConvCount == MTM.Mailbox.convList.length) {
...
MAILBOX_Users = Backbone.Model.extend({
    ...
    loadList: function() {
        ...
        if (n > MTM.Mailbox.userList.length) {
            n = MTM.Mailbox.userList.length;
        }

        for (var i = this.loadedUserCount; i < n; i++) {
            if (typeof MTM.Mailbox.userList[i] != 'undefined') {
                this.itemList.add(MTM.Mailbox.userList[i]);
            }
        }
        this.loadedUserCount = i;

        if (this.loadedUserCount == MTM.Mailbox.userList.length) {
            this.set('loadMore', false);
        } else {
            this.set('loadMore', true);
        }
    ...
    initialize: function() {
        ...
        this.updateList = function(name) {
            ...
            if (name == '') {
                ...
                var expr = new RegExp(MTM.escapeRegExp(name), 'i');
                ...
                for (var i = 0; i < MTM.Mailbox.userList.length; i++) {

                    var displayName = MTM.Mailbox.userList[i].displayName;
                    if (expr.test(displayName)) {
                        var list = self.items.where({ opponentId: MTM.Mailbox.userList[i].opponentId });
                        if (list.length == 0) {
                            self.addItem(MTM.Mailbox.userList[i]);
                        }
                    }
                }
...
MAILBOX_NewMessageForm = Backbone.Model.extend({
    ...
    initialize: function() {
        this.uid = MTM.Mailbox.uniqueId('mailbox_conversation_' + this.get('conversationId') + '_' + this.get('opponentId') + '_');
    },
    ...
    sendMessage: function(text) {
        ...
        ajaxData['actionData'] = {
            'uniqueId': MTM.Mailbox.uniqueId('postMessage'),
            'name': 'postMessage',
            'data': data
        };
        ...
        ajaxData['actionCallbacks'] = {
            success: function(data) {
                ...
                if (data.message) {
                    ...
                    that.uid = MTM.Mailbox.uniqueId('mailbox_conversation_' + that.get('conversationId') + '_' + that.get('opponentId') + '_');
                }
...
MAILBOX_NewMessageFormView = Backbone.View.extend({
    initialize: function() {
        ...
        var $textA = $(formElement.input),
            $submitCont = $('#newMessageSubmitForm');
...
MAILBOX_ComposeMessageFormView = Backbone.View.extend({
    ...
    initialize: function(params) {
        ...
        MTM.bind("mobile.before_shmt_sidebar", function(data) {
            data.openDefaultTab = false;
        });
        MTM.bind("mobile.shmt_sidebar", function(data) {
            if (data.type == "right") {
                MTM.trigger('mobile.open_sidebar_tab', { key: 'convers' });
            }
        });
    },

    backBtnHandler: function() {
        MTM.bind('mailbox.right_sidebar_loaded', function(data) {
            MTM.Mailbox.openUsers();
        });
        MTM.trigger('mobile.open_sidebar_tab', { key: 'convers' });
    }
});
...
MAILBOX_Conversation = Backbone.Model.extend({
    ...
    initialize: function() {
        ...
        MTM.Mailbox.openedConversationId = this.get('conversationId');
        MTM.Mailbox.openedOpponentId = this.get('opponentId');
    },
    ...
    afterAttachment: function(data) {
        ...
        if (owForms["newMailMessageForm"]) {
            ...
            owForms["newMailMessageForm"].elements.uid.setValue(MTM.Mailbox.uniqueId("mailbox_conversation_" + data.message.convId + "_" + data.message.recipientId + "_"));
    ...
    loadHistory: function(callback) {
        ...
        MTM.Mailbox.sendInProcess = true;
        $.ajax({
            url: MTM.Mailbox.get('getHistoryResponderUrl'),
            type: 'POST',
            data: {
                convId: this.get('conversationId'),
            ...
            complete: function() {
                ...
                MTM.Mailbox.sendInProcess = false;
                callback.apply();
            },
            dataType: 'json'
        });
...
MAILBOX_MailConversationView = MAILBOX_ConversationView.extend({
    ...
    backBtnHandler: function() {
        ...
        MTM.bind('mobile.console_page_loaded', function(data) {
            if (data.key == 'convers') {
                MTM.Mailbox.openConversations();
            }
        });
...
MAILBOX_Mobile = Backbone.Model.extend({
    ...
    initialize: function(params) {
        ...
        $.ajax({
            ...
            'success': function(data) {
                ...
                readyStatus++;
                MT.trigger('mailbox.ready', readyStatus);
            }
        });
        ...
        self.sendData = function(ajaxData) {
            ...
            $.ajax({
                url: MTM.Mailbox.get('pingResponderUrl'),
                type: 'POST',
            ...
            if (typeof data.convList != 'undefined') {
                ...
                if (self.convList > data.convList) {
                    ...
                    $(data.convList).each(function() {
                        ...
                        if (!exists) {
                            MT.trigger('mailbox.new_conversation_created', conv);
                        }
                    });
                }
            ...
            if (typeof data.messageList != 'undefined') {
                ...
                $.each(data.messageList, function() {
                    if (this.timeStamp != self.lastMessageTimestamp) {
                        if (typeof MTM.mailboxConversations != "undefined") {
                            MTM.mailboxConversations.itemList.findWhere({ conversationId: this.convId }).set('lastMessageTimestamp', this.timeStamp);
                        }
        ...
        MT.bind('mailbox.ready', function(readyStatus) {
            if (readyStatus == 2) {

                MT.bind('mailbox.message', function(message) {

                    if (typeof MTM.conversation != 'undefined' && MTM.conversation.get('conversationId') == message.convId) {
                        MTM.conversation.messageList.add(message);
                        self.readMessageList.add(message);
                        self.lastMessageTimestamp = message.timeStamp;
                    } else {
                        if (!message.isAuthor) {
                            self.unreadMessageList.add(message);
                            MT.trigger('mailbox.new_message_notification', { message: message, unreadMessageList: self.unreadMessageList });
                        }
                    }
                });
                ...
                this.updateCounter = function(data) {
                    ...
                    MTM.trigger('mobile.console_shmt_counter', { counter: counter, tab: false, options: { tab: 'convers' } });
                ...
                MT.bind('mailbox.new_message_notification', this.updateCounter);

                MT.bind('mailbox.message_was_read', this.updateCounter);
    ...
    openConversations: function() {
        if (MTM.mailboxSidebarMenu) {
            MTM.mailboxSidebarMenu.itemList.findWhere({ mode: 'conversations' }).set({ selected: true });
        }
    },

    openUsers: function() {
        if (MTM.mailboxSidebarMenu) {
            MTM.mailboxSidebarMenu.itemList.findWhere({ mode: 'userlist' }).set({ selected: true });
        }
    },
