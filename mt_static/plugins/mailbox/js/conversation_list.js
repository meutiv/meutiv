MAILBOX_ConversationItemView = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        this.$el.bind('click', function(e) {
            if ($(e.target).is('input.mt_mailbox_conv_option')) {
                //e.preventDefault();
                return;
            }

            if (!MT.Mailbox.conversationController.someConversationLoading) {
                MT.trigger('mailbox.conversation_item_selected', {
        ...
        this.model.on('change:conversationRead', function() {
            if (this.model.get('conversationRead')) {
                this.$el.removeClass('mt_mailbox_convers_info_new');
                this.$el.find('.mt_mailbox_convers_count_new').fadeOut(300);
            } else {
                this.$el.addClass('mt_mailbox_convers_info_new');
                this.$el.find('.mt_mailbox_convers_count_new').fadeIn(300);
            }
        }, this);

        this.model.on('change:isSelected', function() {
            if (this.model.get('isSelected')) {
                this.$el.addClass('mt_mailbox_convers_info_selected');
            } else {
                this.$el.removeClass('mt_mailbox_convers_info_selected');
            }
        }, this);
        ...
        MT.bind('mailbox.conversation_item_selected', function(data) {
        ...
        MT.bind('mailbox.conversation_marked_unread', function(data) {
            if (data.convId == self.model.get('conversationId')) {
                self.model.set('conversationRead', 0);
            }
        });

        MT.bind('mailbox.conversation_marked_read', function(data) {
            if (data.convId == self.model.get('conversationId')) {
        ...
        MT.bind('mailbox.conversation_deleted', function(data) {
            if (data.convId == self.model.get('conversationId')) {
                self.$el.remove();
                MT.updateScroll($('#conversationItemListContainer'));
                var user = MT.Mailbox.usersCollection.findWhere({ convId: data.convId });
                if (user) {
                    user.set('convId', 0);
                }
            }
        ...
        MT.bind('mailbox.message', function(message) {
            ...
            MT.trigger('mailbox.set_no_conversation', { show: false });
        ...
        MT.bind('mailbox.conversation_data_received_for_' + self.model.get('opponentId'), function(data) {
            self.model.set('conversationId', data.conversationId);
            self.model.set('isSelected', true);
        });

        MT.bind('mailbox.message_was_authorized', function(message) {
            if (message.convId != self.model.get('conversationId')) {
                return;
            }
    ...
    render: function() {
        ...
        if (this.model.get('newMessageCount') > 0) {
            this.$el.find('.mt_mailbox_convers_count_new').html(MT.getLanguageText('mailbox', 'new_message_count', { count: this.model.get('newMessageCount') }));
        } else {
            this.$el.find('.mt_mailbox_convers_count_new').html('');
        }
    ...
    changePreviewText: function() {
        if (this.model.get('mode') == 'mail') {
            $('#conversationItemPreviewText', this.$el).html(MT.getLanguageText('mailbox', this.model.get('mode') + '_subject_prefix') + this.model.get('subject'));
        }
        ...
        MT.trigger('mailbox.render_conversation_item', this);
    }
});
...
MAILBOX_ConversationListModel = Backbone.Model.extend({
    ...
    loadList: function(numberToLoad) {
        ...
        if (n > MT.Mailbox.conversationsCollection.length) {
            n = MT.Mailbox.conversationsCollection.length;
        }

        for (var i = this.get('loadedConvCount'); i < n; i++) {
            var conversation = MT.Mailbox.conversationsCollection.models[i];
    ...
    loadMore: function(options) {
        ...
        if (options.searching) {
            ...
            ajaxData['actionData'] = {
                ...
                'data': {
                    'searching': 1,
                    'from': $('.mt_mailbox_convers_info', $('#conversationItemListSub')).length,
                ...
                success: function(data) {
                    ...
                    if (typeof data != 'undefined') {
                        for (var i = 0; i < data.length; i++) {
                            ...
                            data[i]['show'] = true;
                            //                            MT.Mailbox.conversationsCollection.add(data[i]);
            ...
            ajaxData['actionData'] = {
                ...
                'name': 'loadMoreConversations',
                'data': {
                    'from': MT.Mailbox.conversationsCollection.length
                }
            };
            ajaxData['actionCallbacks'] = {
                success: function(data) {
...
MAILBOX_ConversationListView = Backbone.View.extend({
    initialize: function() {
        ...
        MT.Mailbox.conversationsCollection.on('add', this.createItem, this);
        MT.Mailbox.conversationsCollection.on('change', this.onListChange, this);
        MT.Mailbox.conversationsCollection.on('remove', this.onListChange, this);
        ...
        MT.addScroll(this.conversationItemListWrapper);

        $(document).ready(function() {
            self.searchFormElement = new SearchField("conversation_search", "conversation_search", MT.getLanguageText('mailbox', 'label_invitation_conversation_search'));
        ...
        this.conversationItemListWrapper.bind('jsp-scroll-y', function(event, scrollPositionY, isAtTop, isAtBottom) {
            //            if (self.model.get('loadedConvCount') < MT.Mailbox.conversationsCollection.length && !self.listLoadInProgress && isAtBottom){
            if (self.searching && !self.listLoadInProgress && isAtBottom) {
                self.listLoadInProgress = true;
                self.model.loadMore(self);
            } else {
                if (MT.Mailbox.conversationsCollection.length < MT.Mailbox.conversationsCount && !self.listLoadInProgress && isAtBottom) {
                    self.listLoadInProgress = true;
                    self.model.loadMore(self);
                }
            }
        ...
        $('#closeBulkOptionsBtn').click(function(e) {
            self.preloaderControl.removeClass('mt_mailbox_bulk_options');
            return false;
        });

        $('#openBulkOptionsBtn').click(function() {
            self.preloaderControl.addClass('mt_mailbox_bulk_options');
        });

        $('#mailboxConvOptionSelectAll').click(function() {
            if ($(this).prop('checked')) {

                $('.mt_mailbox_conv_option:visible').prop('checked', true);
            } else {
                $('.mt_mailbox_conv_option').prop('checked', false);

            }
        });

        $('#mailboxConvOpenActions').click(function() {
            $('mailboxConvOpenActionsContainer').toggleClass('mt_hidden');
        });

        $('#mailboxConvActionMarkUnread').click(function() {

            var list = $('.mt_mailbox_conv_option:checked');
            var convIdList = [];
        ...
        MT.bind('mailbox.menu_mode_changed', function(data) {
        ...
        MT.bind('mailbox.conversation_deleted', function(data) {
            ...
            var deletedItem = MT.Mailbox.conversationsCollection.findWhere({ conversationId: data.convId });

            if (MT.Mailbox.conversationsCollection.length > 1) {
                var nextItem = MT.Mailbox.conversationsCollection.at(MT.Mailbox.conversationsCollection.indexOf(deletedItem) + 1);
                if (!nextItem) {
                    nextItem = MT.Mailbox.conversationsCollection.at(MT.Mailbox.conversationsCollection.indexOf(deletedItem) - 1);
                }

                MT.trigger('mailbox.conversation_item_selected', { convId: nextItem.get('conversationId'), opponentId: nextItem.get('opponentId') });
                self.model.loadList(1);
            }

            //            MT.Mailbox.conversationsCollection.remove({conversationId: data.convId});
            MT.Mailbox.conversationsCollection.remove(deletedItem);

            if (MT.Mailbox.conversationsCollection.length == 0) {
                self.showNoConversation();
            }
        });

        MT.bind('mailbox.set_no_conversation', function(data) {
            if (data.show) {
                self.showNoConversation();
            } else {
        ...
        MT.bind('mailbox.application_started', function() {
            ...
            if (MT.Mailbox.conversationsCollection.length > 0) {
                ...
                if (self.model.get('pageConvId') != null) {
                    if (!MT.Mailbox.conversationsCollection.findWhere({ conversationId: self.model.get('pageConvId') })) {
                        self.model.loadMore(self);
                    }
                }
            } else {
    ...
    loadMoreToConversationId: function(conversationId) {
        if (!MT.Mailbox.conversationsCollection.findWhere({ conversationId: this.model.get('pageConvId') })) {
                ...
                if (typeof data != 'undefined') {
                    if (actionName == 'markUnread') {
                        _.each(convIdList, function(id) {
                            MT.trigger('mailbox.conversation_marked_unread', { convId: id });
                        });
                    }

                    if (actionName == 'markRead') {
                        _.each(convIdList, function(id) {
                            MT.trigger('mailbox.conversation_marked_read', { convId: id });
                        });
                    }

                    if (actionName == 'delete') {
                        _.each(convIdList, function(id) {
                            MT.trigger('mailbox.conversation_deleted', { convId: parseInt(id) });
                        });
                    }
    ...
    hideNoConversation: function() {
        ...
        this.messagesPageControl.removeClass('mt_mailbox_table_empty');
    },
    ...
    showNoConversation: function() {
        ...
        if ($('#conversationItem-noitems', this.conversationItemListContainer).length == 0) {
            ...
            this.messagesPageControl.addClass('mt_mailbox_table_empty');
            $('#conversationContainer').removeClass('mt_mailbox_right_loading');
        }
    },
    ...
    createItem: function(conversation) {
        ...
        var itemIndex;
        itemIndex = MT.Mailbox.conversationsCollection.indexOf(conversation);
        ...
        if (this.model.get('activeConvId') == conversation.get('conversationId')) {
            MT.trigger('mailbox.conversation_item_selected', { convId: conversation.get('conversationId'), opponentId: conversation.get('opponentId') });
        } else {
        ...
        MT.trigger('mailbox.render_conversation_item', conversationItemView);
    },

    showPreloader: function() {
        this.preloaderControl.addClass('mt_mailbox_left_loading');
    },

    hidePreloader: function() {
        this.preloaderControl.removeClass('mt_mailbox_left_loading');
        MT.updateScroll(this.conversationItemListWrapper);
    },
    ...
    updateList: function(name) {
        ...
        if (name == '') {
            ...
            self.model.loadList(MT.Mailbox.conversationsCollection.length);

            for (var i = 0; i < MT.Mailbox.conversationsCollection.length; i++) {
                var conv = MT.Mailbox.conversationsCollection.models[i];
            ...
            //            for (var i=0; i<MT.Mailbox.conversationsCollection.length; i++){
            //                var item = MT.Mailbox.conversationsCollection.models[i];
            ...
            //                }
            //            }
            $('.mt_btn_close_search').removeClass('mt_preloader');
        } else {
            ...
            if (!self.syncing) {
                $('.mt_btn_close_search').addClass('mt_preloader');
                self.syncing = true;
                        ...
                        if ($('#conversation_search').val() != data.kw) {
                            ...
                            _.each(data.list, function(conversation) {
                                ...
                                if ($('#conversationItem' + conversation.data.conversationId).length == 0) {
                                    ...
                                    self.createItem(conv);
                                    MT.Mailbox.conversationsCollection.add(conversation.data);
                                    conv.show();
                                }

                            });

                            MT.updateScroll(self.conversationItemListWrapper);
                            self.syncing = false;
                            $('.mt_btn_close_search').removeClass('mt_preloader');
                        }
                    });
...
MAILBOX_ConversationView = function() {
    ...
    $(document).click(function(e) {
        ...
        if (!isTextarea) {
            ...
            if (self.textareaControl && self.textareaControl.val() != '') {
                ...
                $('.mt_mailbox_log').removeClass('textarea_active');
            ...
            MT.updateScroll(self.messageListControl);
        }
    });
    ...
    this.setConversationId = function(params) {
        ...
        this.conversation = MT.Mailbox.conversationsCollection.findWhere({ conversationId: params.convId });
            ...
            success: function(data) {
                ...
                if (typeof data != 'undefined') {
                    ...
                    if (data.mode == 'chat') {
                        ...
                        $('#' + newUid + ' .mt_file_attachment_preview').html('');
                    ...
                    MT.trigger('mailbox.conversation_marked_read', { convId: self.model.convId });
                }
                ...
                success: function(data) {
                    if (typeof data != 'undefined') {
                        if (data.log.length > 0) {
                            ...
                            MT.trigger('mailbox.history_loaded');
    ...
    this.settingsBtn.bind('click', function() {
        if (self.settingsBlock.hasClass('mt_hidden')) {
    ...
    this.deleteBtn.bind('click', function() {
        ...
        if (confirm(MT.getLanguageText('mailbox', 'confirm_conversation_delete'))) {
            $.ajax({
                ...
                success: function(data) {
                    if (data.result == true) {
                        MT.info(data.notice);
                        self.reset();
                        MT.trigger('mailbox.conversation_deleted', { convId: self.model.convId, opponentId: self.model.opponentId });
        ...
        ajaxData['actionCallbacks'] = {
            success: function(data) {
                if (data.error != undefined) {
                    ...
                    MT.info(data.notice);
                    MT.trigger('mailbox.conversation_marked_unread', { convId: self.model.convId });
                }
            },
            complete: function() {}
        }
    ...
    this.switchToChatBtn.bind('click', function() {
        MT.trigger('mailbox.open_dialog', { convId: self.model.convId, opponentId: self.model.opponentId, mode: 'chat' });
    });
    ...
    this.sendMessageBtn.bind('click', function() {
        ...
        if (!$.trim(checkText).length) {
            MT.error(MT.getLanguageText('mailbox', 'chat_message_empty'));
            return;
        }
    ...
    this.model.modeSetSubject.addObserver(function() {
        if (self.model.mode == 'chat') {
            self.subjectBloсkControl.addClass('mt_hidden');
            self.capBlockControl.addClass('mt_mailbox_cap_chat');
            self.control.addClass('mt_mailbox_right_chat');
            self.conversationChatFormBlock.removeClass('mt_hidden');
            self.messageFormBlock.addClass('mt_hidden');
            ...
            if (!self.hasLinkObserver) {
                OWLinkObserver.observeInput('dialogTextarea', function(link) {
                    ...
                    this.onResult = function(r) {
                        ...
                        MT.trigger('mailbox.conversation_embed_link_request_result', r);
                    }
                });
            }
        }
        ...
        if (self.model.mode == 'mail') {
            self.subjectBloсkControl.removeClass('mt_hidden');
            self.capBlockControl.removeClass('mt_mailbox_cap_chat');
            self.control.removeClass('mt_mailbox_right_chat');
            self.conversationChatFormBlock.addClass('mt_hidden');
            self.messageFormBlock.removeClass('mt_hidden');
            self.textareaControl = $('#conversationTextarea', self.control);

            $('.mt_mailbox_form').click(function() {
                $('#conversationMessageFormBlock').removeClass('continue');
                $('#conversationMessageFormBlock').addClass('active');
                $('.mt_mailbox_log').addClass('textarea_active');
    ...
    this.model.profileUrlSetSubject.addObserver(function() {
        ...
        if (self.model.convId) {
            var conversation = MT.Mailbox.conversationsCollection.findWhere({ conversationId: self.model.convId });
            if (conversation) {
    ...
    MT.bind('mailbox.conversation_item_list_loaded', function(data) {
...
MAILBOX_ConversationView.prototype = {
    ...
    construct: function() {
        ...
        MT.addScroll(this.messageListControl, { contentWidth: '0px' });
    ...
    reset: function() {
        ...
        this.messageFormBlock.addClass('mt_hidden');
    ...
    hidePreloader: function() {
        ...
        this.preloaderControl.removeClass('mt_mailbox_right_loading');
        this.scrollDialog();
    },

    hideSettingsBlock: function() {
        this.settingsBtn.removeClass('active');
        this.settingsBlock.addClass('mt_hidden');
    },
    ...
    scrollDialog: function(scrollToBottom) {
        ...
        this.historyLoadAllowed = false;
        MT.updateScroll(this.messageListControl);

        var jsp = this.messageListControl.data('jsp');
        if (typeof jsp != 'undefined' && jsp != null) {
            lastMessage = this.messageListControl.find('.mt_mailbox_log_message').last();
    ...
    sendMessage: function(text, timeStamp) {
        ...
        if (!self.embedLinkDetected) {
            ...
            MT.trigger('mailbox.send_message', { 'sentFrom': 'conversation', 'opponentId': self.model.opponentId, 'convId': self.model.convId, 'tmpMessage': tmpMessage });
            ...
            if (self.embedLinkResult) {
                ...
                MT.bind('mailbox.conversation_embed_link_request_result', function(r) {
                    var data = {
                        'convId': self.model.convId,
                        'text': text,
                        'uid': self.uid,
        ...
        ajaxData['actionCallbacks'] = {
            ...
            'success': function(data) {
                ...
                if (typeof data.error == 'undefined' || data.error == null) {
                    ...
                    self.updateMessage(data.message);
                    MT.Mailbox.lastMessageTimestamp = parseInt(data.message.timeStamp);
                    MT.trigger('mailbox.update_message', { 'sentFrom': 'conversation', 'opponentId': self.model.opponentId, 'convId': self.model.convId, 'message': data.message });
    ...
    showSendMessageFailed: function(messageId) {
        ...
        if (self.model.mode == 'mail') {
            ...
            $('#messageItem' + messageId, self.control).prepend('<span class="mt_errormessage_not mt_red mt_small">' + MT.getLanguageText('mailbox', 'send_message_failed') + '</span>');
        }
    ...
    showPreloader: function() {
        this.preloaderControl.addClass('mt_mailbox_right_loading');
    },

    showSettingsBlock: function() {
        this.settingsBtn.addClass('active');
        this.settingsBlock.removeClass('mt_hidden');
    },
    ...
    updateChatMessage: function(message) {
        ...
        MT.trigger('mailbox.message_was_authorized', message);
    },
    ...
    updateMailMessage: function(message) {
        ...
        if (message.attachments.length != 0) {
            ...
            for (var i in message.attachments) {
                ...
                if (parseInt(i) % 2) {
                    attachment.addClass('mt_file_attachment_block2');
                } else {
                    attachment.addClass('mt_file_attachment_block1');
                }
    ...
    writeMailMessage: function(message, css_class) {
        ...
        MT.trigger('mailbox.after_write_mail_message', message);
    },
    ...
    writeHistoryChatMessage: function(message) {
        ...
        if (groupContainer.length == 0) {
            ...
            $('.mt_mailbox_date_cap', groupContainer).after(messageContainer);
            if (message.timeLabel != this.model.firstMessageTimeLabel) {
                ...
                $('.mt_mailbox_date_cap', groupContainer).after(timeBlock);
            }
