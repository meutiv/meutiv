...
MAILBOX_ContactView = Backbone.View.extend({
    ...
    template: function(data) {
        var itemControl = $('#mt_chat_list_proto ul li').clone();
    ...
    initialize: function() {
        ...
        this.itemLink = $('a.mt_chat_item', this.$el);
        this.unreadMessagesCountWrapper = $('.mt_count_wrap', this.$el);
        this.unreadMessagesCountControl = $('.mt_count', this.$el);
        ...
        this.model.on('change:status', function() {
            ...
            this.statusControl.addClass('mt_chat_status');
        ...
        this.model.on('change:unreadMessagesCount', function() {
            ...
            if (this.model.get('unreadMessagesCount') > 0) {
                ...
                this.itemLink.addClass('mt_active');
            } else {
                this.unreadMessagesCountWrapper.hide();
                this.statusControl.show();
                this.itemLink.removeClass('mt_active');
            }
        }, this);
        ...
        this.$el.click(function() {
            if (typeof MT.Mailbox.newMessageFormController != "undefined") {
                if (MT.Mailbox.newMessageFormController.closeNewMessageWindowWithConfirmation($('.mailboxDialogBlock.mt_open').length)) {
                    MT.trigger('mailbox.open_dialog', { convId: self.model.get('convId'), opponentId: self.model.get('opponentId'), mode: 'chat', isSelected: true, isActive: true });
                }
            } else {
                MT.trigger('mailbox.open_dialog', { convId: self.model.get('convId'), opponentId: self.model.get('opponentId'), mode: 'chat', isSelected: true, isActive: true });
            }
        ...
        MT.bind('mailbox.presence', function(presence) {
        ...
        MT.bind('mailbox.dialog_opened', function(data) {
            if (data.convId != self.model.get('convId')) {
                return;
            }

            //            self.model.set('unreadMessagesCount', 0);
            self.updateUnreadMessagesCount({ message: { convId: data.convId }, unreadMessageList: MT.Mailbox.contactManager.unreadMessageList });
        });

        MT.bind('mailbox.dialog_selected', function(data) {
            if (data.convId != self.model.get('convId')) {
                return;
            }
        ...
        MT.bind('mailbox.message_was_read', this.updateUnreadMessagesCount);

        MT.bind('mailbox.data_received_for_' + self.model.get('opponentId'), function(data) {
    ...
    hide: function() {
        this.$el.addClass('mt_hidden');
    },

    show: function() {
        this.$el.removeClass('mt_hidden');
    }
});
...
MAILBOX_ContactsCollection = Backbone.Collection.extend({
    ...
    initialize: function() {
        MT.Mailbox.usersCollection.on('add', this.addUser, this);
        MT.Mailbox.usersCollection.on('change', this.changeUser, this);
    },
...
MAILBOX_ContactManager = Backbone.Model.extend({
    ...
    addViewedConversation: function(conversationId) {
        ...
        var conversation = MT.Mailbox.conversationsCollection.findWhere({ conversationId: conversationId });
        if (conversation) {
...
MAILBOX_ContactManagerView = Backbone.View.extend({
    initialize: function() {
        ...
        MT.bind('mailbox.draggable.drag', function() {
            self.fitWindow();
        });

        MT.bind('mailbox.message', function(message) {

            var messagesOpened = false;
            if (typeof MT.Mailbox.conversationController != 'undefined') {
                messagesOpened = true;
            }

            var conversationOpened = false;
            if (messagesOpened && MT.Mailbox.conversationController.model.convId == message.convId) {
            ...
            if (OWMailbox.chatModeEnabled && OWMailbox.useChat == 'available' && message.mode == 'chat') {
                ...
                if (message.senderId == OWMailbox.userDetails.userId) {
                    ...
                    MT.Mailbox.lastMessageTimestamp = message.timeStamp;
                } else {
                    ...
                    if (dialog) {
                        ...
                        if (!dialog.model.isSelected && !conversationOpened) {
                            if (message.mode == 'chat' && self.newMessageTimeout === 0) {
                                var new_message_label = MT.getLanguageText('mailbox', 'new_message');
                            ...
                            MT.trigger('mailbox.new_message_notification', { message: message, unreadMessageList: self.model.unreadMessageList });
                        }
                    }
                }
            } else {
        ...
        MT.bind('mailbox.mark_message_read', function(data) {

            MT.trigger('mailbox.clear_new_message_notification', { convId: data.message.convId });

            if (!data.message.readMessageAuthorized) {
                MT.trigger('mailbox.clear_new_message_blinking', { message: data.message, unreadMessageList: self.model.unreadMessageList });
                return;
            }
            ...
            for (var i = 0; i < MT.Mailbox.markedUnreadConversationList.length; i++) {
                var convId = MT.Mailbox.markedUnreadConversationList;
                if (data.message.convId == convId) {
                    MT.Mailbox.markedUnreadConversationList.splice(i, 1);
                }
            }

            MT.trigger('mailbox.message_was_read', { message: data.message, unreadMessageList: self.model.unreadMessageList });
        ...
        if (OWMailbox.chatModeEnabled && OWMailbox.useChat == 'available') {
            ...
            this.model.on('change:chatSelectorUnreadMessages', function() {
                ...
                if (self.model.get('chatSelectorUnreadMessages') > 0 && !self.chatSelectorButton.hasClass('mt_active')) {
            ...
            this.model.on('change:chatBlockActive', function() {
                ...
                MT.updateScroll(this.contactListWrapper);
            ...
            MT.bind('mailbox.application_started', function() {
                self.model.loadList();
                MT.updateScroll(self.contactListWrapper);
                ...
                $.each(openedDialogs, function(convId, presence) {
                    ...
                    if (dialog == null) {
                        ...
                        MT.trigger('mailbox.open_dialog', { convId: convId, opponentId: presence.opponentId, mode: presence.mode, isActive: presence.isActive, isSelected: presence.isSelected });
                    }
                });

                self.fitWindow();
            });
            MT.bind('mailbox.clear_new_message_notification', function(data) {
            ...
            MT.bind('mailbox.dialog_moved_to_chat_selector', function(data) {
                self.updateChatSelector();
            });
            MT.bind('mailbox.dialog_removed_from_chat_selector', function(data) {
                self.updateChatSelector();
            });
            MT.bind('mailbox.open_dialog', function(data) {
                ...
                if (dialog.model.isOpened) {
                    if (dialog.model.isHidden) {
                        var dialogs = $('.mailboxDialogBlock.mt_open');
                        var firstDialogControl = dialogs.first();
                        var firstConvId = $(firstDialogControl).data('convId');
                        MT.trigger('mailbox.move_dialog_to_chat_selector', { convId: firstConvId });

                        MT.trigger('mailbox.remove_dialog_from_chat_selector', { convId: dialog.model.convId });
                    }
                }
            ...
            MT.bind('mailbox.close_dialog', function(data) {
            ...
            MT.bind('mailbox.conversation_deleted', function(data) {
                MT.trigger('mailbox.close_dialog', data);
            });
            MT.bind('mailbox.dialog_tab_shown', function(params) {
            ...
            MT.bind('mailbox.dialog_hidden', function(params) {

            });
            MT.bind('mailbox.dialog_selected', function(params) {
            ...
            MT.bind('mailbox.dialog_closed', function(params) {
                self.removeDialog(params.convId);
                self.fitWindow();
            });
            MT.bind('mailbox.new_message_form_opened', function(data) {
                self.fitWindow();
            });
            MT.bind('mailbox.new_message_form_closed', function(data) {
                self.fitWindow();
            });
            MT.bind('mailbox.new_message_form_minimized', function(data) {
                self.fitWindow();
            });
            ...
            $(document).click(function(e) {
                ...
                if (isTarget && !isContent) {
                    if (self.chatSelectorList.hasClass('mt_hidden')) {
                        if (countOfDialogsInChatSelector > 0) {
                            self.chatSelectorButton.addClass('mt_active');
                            self.chatSelectorList.removeClass('mt_hidden');

                            //Hide if there are no hidden unread dialogs
                            self.chatSelectorTotalUnreadMessagesCountWrapper.hide();
                        }
                    } else {
                        self.chatSelectorButton.removeClass('mt_active');
                        self.chatSelectorList.addClass('mt_hidden');
                ...
                var isTarget = $('.mt_chat_in_dialog_wrap').is(e.target) || $('.mt_chat_in_dialog_wrap').find(e.target).length || $('.mt_chat_message_block').is(e.target) || $('.mt_chat_message_block').find(e.target).length;
                var isSwitchToChatBtn = $('#conversationSwitchToChatBtn').is(e.target);
                var isContactArea = $('.mt_chat_list').find(e.target).length;
                var isChatSelector = $('.mt_chat_selector').find(e.target).length;
            ...
            $(document).ready(function() {
                self.searchFormElement = new SearchField("im_find_contact", "im_find_contact", MT.getLanguageText('mailbox', 'find_contact'));
            ...
            $('.mt_chat_list .mt_chat_preloader').remove();
    ...
    addContact: function(user) {
        ...
        if (jsp) {
            ...
            MT.addScroll(this.contactListWrapper);
        }
    ...
    construct: function() {
        ...
        this.contactListContainer = $('.mt_chat_list ul');
        this.chatSelector = $('.mt_chat_selector');
        this.chatSelectorButton = $('.mt_btn_dialogs');
        this.chatSelectorList = $('.mt_chat_selector_list');

        this.chatSelectorTotalUnreadMessagesCountWrapper = $('.mt_chat_selector .mt_chat_block .mt_selector_panel .mt_count_wrap');
        this.chatSelectorUnreadMessagesCounter = $('.mt_chat_selector .mt_chat_block .mt_selector_panel .mt_count_wrap .mt_count_bg .mt_count');
        this.hiddenContactsCountWrapper = $('.mt_chat_selector .mt_chat_block .mt_selector_panel .mt_dialog_count');

        if (OWMailbox.chatModeEnabled && OWMailbox.useChat == 'available') {
            this.chatSettingsActive = false;
            this.fitWindowNumber = 0;

            this.contactListWrapper = $('.mt_chat_list');

            this.mainWindow = $('.mt_chat .mt_chat_block_wrap .mt_chat_block');

            this.minimizeButton = $('.btn2_panel');

            this.puller = $('.mt_puller', $('.mt_chat'));
            ...
            this.puller.draggable({
                ...
                stop: function(event, ui) {
                    ...
                    if ($('.mt_chat_list ul').height() > self.contactListWrapper.height()) {
                        MT.addScroll(self.contactListWrapper);
                    } else {
                ...
                start: function(event, ui) {
                    ...
                    MT.removeScroll(self.contactListWrapper);
            ...
            this.notificationWrapper = $('.mt_chat');
            this.settingsWrapper = $('.mt_chat_settings');
    ...
    fitWindow: function() {
        ...
        var allChatsWidth = $('.mt_chat').outerWidth(true);
        ...
        var minOpenedChats = typeof MT.Mailbox.newMessageFormController != "undefined" &&
            MT.Mailbox.newMessageFormController.isNewMessageWindowActive() ? 0 : 1;

        if (winWidth < allChatsWidth) {
            // folding
            if ($('.mailboxDialogBlock.mt_open').length > minOpenedChats) {
                var dialogs = $('.mailboxDialogBlock.mt_open');
                ...
                MT.trigger('mailbox.move_dialog_to_chat_selector', { convId: convId });

                self.fitWindow();
            }
        } else {
            // get last hidden chat box width
            if ($('.mt_chat_selector_items li.mt_dialog_in_selector').length > 0) {
                var dialogs = $('div.mailboxDialogBlock.mt_hidden');
                var box = dialogs.last();

                if (winWidth > (allChatsWidth + $(box).outerWidth(true))) {
                    // unfolding
                    var convId = $(box).data('convId');
                    MT.trigger('mailbox.remove_dialog_from_chat_selector', { convId: convId });
                    self.fitWindow();
                }
            }
        }
    ...
    getContactFromUserList: function(userId) {
        return MT.Mailbox.usersCollection.findWhere({ opponentId: userId });
    },
                ...
                $.post(OWMailbox.openDialogResponderUrl, {
                    ...
                    if (typeof data != 'undefined') {
                        if (typeof data['warning'] != 'undefined' && data['warning']) {
                            MT.message(data['message'], data['type']);
                        } else {
                            if (data['use_chat'] && data['use_chat'] == 'promoted') {
                                self.showPromotion();
                            } else {
                                MT.Mailbox.usersCollection.add(data);
    ...
    hideChatSelector: function() {
        this.chatSelector.addClass('mt_hidden');
    },

    hidePromotion: function() {
        $('#chatPromotionBlock').addClass('mt_hidden');
    },

    isActiveMode: function() {
        return this.mainWindow.hasClass('mt_active');
    },

    isCompactMode: function() {
        return this.mainWindow.hasClass('mt_compact');
    },

    maximize: function() {
        this.mainWindow.addClass('mt_active');
        if (this.mainWindow.hasClass('mt_compact')) {
            this.mainWindow.removeClass('mt_compact');
        }
        this.puller.draggable("enable");
        //this.puller.addClass('mt_im_draggable');
    },

    minimize: function() {
        this.mainWindow.removeClass('mt_active');
        this.puller.draggable("disable");
        //this.puller.removeClass('mt_im_draggable');
    },

    moveToActiveMode: function() {
        var self = this;

        if (self.mainWindow.hasClass('mt_compact') && self.mainWindow.hasClass('mt_active')) {
            var speed = 'normal';

            self.mainWindow.removeClass('mt_compact');
            $('.mt_chat_cont').animate({
                right: 0
            }, speed);
    ...
    showChatSelector: function() {
        this.chatSelector.removeClass('mt_hidden');
    },

    showPromotion: function() {
        $('#chatPromotionBlock').removeClass('mt_hidden');
    },
    ...
    updateList: function(name) {
        ...
        if (name == '') {
            ...
            var expr = new RegExp(MT.escapeRegExp(name), 'i');
    ...
    addUnreadMessage: function(message) {
        ...
        var conversation = MT.Mailbox.conversationsCollection.findWhere({ conversationId: message.get('convId') });
        if (conversation) {
    ...
    setShowOnlineOnly: function() {
        ...
        if (self.model.get('showOnlineOnly')) {
            $('#mailboxSortUsersPreference span').addClass('mt_btn_sort_online');
            $('#mailboxSortUsersPreference').attr('title', MT.getLanguageText('mailbox', 'shmt_all_users'));
            ...
            $('#mailboxSortUsersPreference span').removeClass('mt_btn_sort_online');
            $('#mailboxSortUsersPreference').attr('title', MT.getLanguageText('mailbox', 'shmt_online_only'));
            self.contactListWrapper.addClass('showAllUsers');
            self.contactListWrapper.removeClass('showOnlineOnly');
            this.onMainCounterUpdate();
        }

        MT.hideTip($('#mailboxSortUsersPreference'));
        $('#mailboxSortUsersPreference').removeData('owTip');

        MT.bindTips($('#mailboxSortUsersPreference').parent());
    ...
    setSoundEnabled: function() {
        ...
        if (self.model.get('soundEnabled')) {
            $('#mailboxSoundPreference span').removeClass('mt_btn_sound_off');
            $('#mailboxSoundPreference').attr('title', MT.getLanguageText('mailbox', 'silent_mode_on'));
        } else {
            $('#mailboxSoundPreference span').addClass('mt_btn_sound_off');
            $('#mailboxSoundPreference').attr('title', MT.getLanguageText('mailbox', 'silent_mode_off'));
        }

        MT.hideTip($('#mailboxSoundPreference'));
        $('#mailboxSoundPreference').removeData('owTip');

        MT.bindTips($('#mailboxSoundPreference').parent());
    }
...
OWMailbox.Dialog.Controller = function(model) {
    ...
    MT.bind('base.add_attachment_to_queue', function(data) {
    ...
    MT.bind('base.update_attachment', function(data) {
        ...
        $.each(data.items, function() {
            if (!this.result) {
                MT.error(this.message);
            }
        })

        var newUid = OWMailbox.uniqueId('mailbox_dialog_' + self.model.convId + '_' + self.model.opponentId + '_');
        MT.trigger('base.file_attachment', { 'uid': self.uid, 'newUid': newUid });
        self.uid = newUid;

        MT.getPing().getCommand('mailbox_ping').start();
    });

    $('.mt_chat_in_dialog_wrap, .mt_chat_message_block', this.control).click(function(e) {
    ...
    this.minimizeMaximizeBtn.click(function() {
        ...
        if (!self.model.isSelected) {
            MT.trigger('mailbox.clear_new_message_notification', { convId: self.model.convId });
        }
    ...
    this.smallItemControl.click(function() {
        if (typeof MT.Mailbox.newMessageFormController != "undefined") {
            if (MT.Mailbox.newMessageFormController.closeNewMessageWindowWithConfirmation($('.mailboxDialogBlock.mt_open').length)) {
                MT.trigger('mailbox.open_dialog', { convId: self.model.convId, opponentId: self.model.convId, mode: self.model.mode });
            }
        } else {
            MT.trigger('mailbox.open_dialog', { convId: self.model.convId, opponentId: self.model.convId, mode: self.model.mode });
        }

        $('.mt_btn_dialogs').click();
    });
        ...
        if (isAtTop && !self.historyLoadInProgress && self.model.firstMessageId != null && self.historyLoadAllowed) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (typeof data != 'undefined') {
                        if (data.log.length > 0) {
                            ...
                            MT.updateScroll(self.messageListWrapper);
                ...
                complete: function() {
                    ...
                    MT.Mailbox.sendInProcess = false;
                },
                dataType: 'json'
            });
            ...
            'success': function(data) {
                if (typeof data.error != 'undefined') {
                    ...
                    if (typeof data.authorizationActionText != 'undefined') {
                        MT.info(data.authorizationActionText);
                    }
        ...
        if (!self.hasLinkObserver) {
            OWLinkObserver.observeInput('main_tab_contact_' + self.model.opponentId + ' #dialogTextarea', function(link) {
                ...
                this.onResult = function(r) {
                    ...
                    MT.trigger('mailbox.embed_link_request_result_' + self.model.convId, r);
                }
            });
    ...
    this.model.statusUpdateSubject.addObserver(function() {
        ...
        self.statusControl.addClass('mt_chat_status');
    ...
    this.model.isActiveSetSubject.addObserver(function() {
        if (!self.model.isActive) {
            $('#main_tab_contact_' + self.model.opponentId).removeClass('mt_chat_dialog_active');
            $('#main_tab_contact_' + self.model.opponentId).removeClass('mt_active');
            if (self.model.unreadMessagesCount > 0) {
                self.unreadMessageCountBlock.addClass('mt_count_active');
            }
        } else {
            $('#main_tab_contact_' + self.model.opponentId).addClass('mt_chat_dialog_active');
            $('#main_tab_contact_' + self.model.opponentId).addClass('mt_active');
            if (self.model.unreadMessagesCount > 0) {
                self.unreadMessageCountBlock.removeClass('mt_count_active');
            }
        }
        ...
        if (self.model.isSelected) {
            if (self.model.unreadMessagesCount > 0) {
                $.each(self.model.unreadMessageList, function(id, message) {
                    MT.trigger('mailbox.mark_message_read', { message: message });
                    self.model.removeUnreadMessageFromList(id);
                });
            }
            self.chatBlock.addClass('mt_chat_block_active');
            //self.textareaControl.focus();
            MT.trigger('mailbox.dialog_selected', { convId: self.model.convId });
        } else {
            self.chatBlock.removeClass('mt_chat_block_active');
        }
        ...
        if (self.model.unreadMessagesCount == 0) {
            ...
            if (self.model.isOpened && self.newMessageTimeout === 0) {
                self.newMessageTimeout = setInterval(function() {
                    ...
                    if (self.control.hasClass('mt_chat_new_message')) {
                        self.control.removeClass('mt_chat_new_message');
                    } else {
                        self.control.addClass('mt_chat_new_message');
                    }
    ...
    this.model.isOpenedSetSubject.addObserver(function() {
        if (!self.control.hasClass('mt_hidden')) {
            self.control.addClass('mt_open');
        }
        ...
        if (typeof message != 'undefined' && message != null && message != '') {
            ...
            for (var i = 1; i <= lines.length; i++) {
                if (i == 2) {
                    offset = offset + 12;
                    $('.mt_chat_message', self.control).removeClass('scroll');
                } else {
                    if (i >= 3 && i <= 6) {
                        offset = offset + 17;
                        $('.mt_chat_message', self.control).removeClass('scroll');
                        ...
                        if (i > 6) {
                            offset = 80;
                            $('.mt_chat_message', self.control).addClass('scroll');
                            break;
                        }
                    }
                }
    ...
    MT.bind('mailbox.move_dialog_to_chat_selector', function(data) {
        ...
        self.control.addClass('mt_hidden');
        self.control.removeClass('mt_open');

        self.smallItemControl.show();
        self.smallItemControl.addClass('mt_dialog_in_selector');

        MT.trigger('mailbox.dialog_moved_to_chat_selector', { convId: self.model.convId });
    });

    MT.bind('mailbox.remove_dialog_from_chat_selector', function(data) {
        if (data.convId != self.model.convId) {
            return;
        }
        ...
        self.control.removeClass('mt_hidden');
        self.control.addClass('mt_open');

        self.scrollDialog();

        self.smallItemControl.hide();
        self.smallItemControl.removeClass('mt_dialog_in_selector');

        MT.trigger('mailbox.dialog_removed_from_chat_selector', { convId: self.model.convId });
    });

    MT.bind('mailbox.new_message_notification', function(data) {
    ...
    MT.bind('mailbox.data_received_for_' + self.model.opponentId, function(data) {

        if (self.model.convId == 0) {
            delete MT.Mailbox.contactManagerView.dialogs[self.model.convId];

            self.model.setConversationId(data.conversationId);
            MT.Mailbox.contactManagerView.dialogs[data.conversationId] = self;
        }
    ...
    MT.bind('mailbox.message_was_read', function(data) {
        if (data.message.convId != self.model.convId) {
            return;
        }

        MT.trigger('mailbox.clear_new_message_blinking', data);
    });

    MT.bind('mailbox.clear_new_message_blinking', function(data) {
        if (data.message.convId != self.model.convId) {
            return;
        }
...
OWMailbox.Dialog.Controller.prototype = {
    ...
    construct: function() {
        ...
        this.chatBlock = $('.mt_chat_block', this.control);

        this.messageListWrapper = $('.mt_chat_in_dialog', this.control);
        ...
        this.puller = $('.mt_vertical_puller', this.control);
        ...
        this.diagPuller = $('.mt_diagonal_puller', this.control);
        ...
        this.diagPuller.draggable({
            ...
            drag: function(event, ui) {
                ...
                self.messageListWrapper.height(self.dialogWindowHeight - ui.position.top);
                MT.trigger('mailbox.draggable.drag', { "puller": self.puller });
            },
            stop: function(event, ui) {
                ...
                MT.updateScroll(self.messageListWrapper);
                MT.trigger('mailbox.draggable.stop', { "puller": self.puller });
            },
            start: function(event, ui) {
                self.dialogWindowHeight = self.messageListWrapper.height();
        ...
        this.chatSelectorContactListContainer = $('.mt_chat_selector_items');

        this.smallItemControl = $('#mt_chat_selector_items_proto li').clone();
        ...
        MT.trigger('mailbox.after_dialog_render', [{ 'control': this.control, 'opponentId': this.model.opponentId }]);
    ...
    hide: function() {
        ...
        this.control.removeClass('mt_active');
        this.disablePuller();

        MT.updateScroll(this.messageListWrapper);
        ...
        MT.trigger('mailbox.dialog_hidden', { convId: this.model.convId });
    ...
    hideTab: function() {
        ...
        MT.trigger('mailbox.dialog_closed', { convId: this.model.convId });
    },
    ...
    loadHistory: function() {
        ...
        ajaxData['actionCallbacks'] = {
            success: function(data) {
                ...
                if (typeof data != 'undefined') {
                    MT.trigger('mailbox.data_received_for_' + data.opponentId, data);

                    if (data.close_dialog) {
                        MT.trigger('mailbox.close_dialog', { convId: self.model.convId, opponentId: self.model.opponentId });
                        return;
                    }
                ...
                MT.trigger('mailbox.dialogLogLoaded', { opponentId: data.opponentId });
        ...
        MT.Mailbox.addAjaxData(ajaxData);
        ...
        if (self.model.isSelected) {
            ...
            MT.Mailbox.addAjaxData(ajaxData2);
        }
    ...
    open: function() {
        ...
        //        else
        //        {
        //            MT.bind(self.model.convId+'_tabOpened', function(){
        //                self.onOpen();
        //                MT.unbind(self.model.convId+'_tabOpened');
        //            });
        //        }
    ...
    onOpen: function() {
        ...
        MT.trigger('mailbox.dialog_opened', { convId: this.model.convId });
    },

    removePreloader: function() {
        this.preloaderControl.remove();
        MT.addScroll(this.messageListWrapper, { contentWidth: '0px' });
    },
    ...
    sendMessage: function(text) {
        ...
        if (!self.embedLinkDetected) {
            ...
            MT.trigger('mailbox.send_message', { 'sentFrom': 'dialog', 'opponentId': self.model.opponentId, 'convId': self.model.convId, 'tmpMessage': tmpMessage });
            ...
            if (self.embedLinkResult) {
                ...
                MT.bind('mailbox.embed_link_request_result_' + self.model.convId, function(r) {
                    var data = {
                        'convId': self.model.convId,
                        'text': text,
                        'uid': self.uid,
                    ...
                    self.postMessage(tmpMessageUid, data);
                    MT.unbind('mailbox.embed_link_request_result_' + self.model.convId);
                });
            }
        ...
        ajaxData['actionCallbacks'] = {
            ...
            'success': function(data) {
                ...
                if (typeof data.error == 'undefined' || data.error == null) {
                    ...
                    self.updateMessage(data.message);
                    MT.Mailbox.lastMessageTimestamp = data.message.timeStamp;
                    MT.trigger('mailbox.update_message', { 'sentFrom': 'dialog', 'opponentId': self.model.opponentId, 'convId': self.model.convId, 'message': data.message });
                } else {
                    MT.error(data.error);
    ...
    showSendMessageFailed: function(messageId) {
        ...
        $('#messageItem' + messageId + ' .mt_dialog_in_item', self.control).addClass('errormessage');
        $('#messageItem' + messageId + ' .mt_dialog_in_item', self.control).prepend('<span class="mt_errormessage_not mt_red mt_small">' + MT.getLanguageText('mailbox', 'send_message_failed') + '</span>');
    ...
    showComposing: function() {
        ...
        $('.mt_dialog_item', message_composing_container).addClass('odd');
    ...
    showTab: function() {
        ...
        this.control.addClass('mt_open');

        MT.trigger('mailbox.dialog_tab_shown', { convId: this.model.convId });
        MT.trigger(this.model.convId + '_tabOpened');
    ...
    showTimeBlock: function(timeLabel) {
        ...
        $('.mt_time_text', timeBlock).html(timeLabel);
    ...
    updateChatMessage: function(message) {
        ...
        if (message.isSystem) {
            ...
            if (message.attachments.length != 0) {
                ...
                if (message.attachments[i]['type'] == 'image') {
                    messageContainer.addClass('mt_dialog_picture_item');
                    ...
                    $('.mt_dialog_in_item', messageContainer).addClass('fileattach');
                    ...
                    $('.mt_dialog_in_item', messageContainer).html(attachment.html());
        ...
        if (message.senderId != this.model.opponentId) {
            $('div.mt_dialog_item', messageContainer).addClass('even');
        } else {
            $('div.mt_dialog_item', messageContainer).addClass('odd');
        }

        this.scrollDialog();

        MT.trigger('mailbox.update_chat_message', message);
    },
    ...
    writeChatMessage: function(message, css_class) {
        ...
        if (css_class != null) {
            $('div.mt_dialog_item', messageContainer).addClass(css_class);
        }
    ...
    writeHistoryChatMessage: function(message) {
        ...
        if (groupContainer.length == 0) {
            ...
            $('.mt_time_text', timeBlock).html(message.timeLabel);
