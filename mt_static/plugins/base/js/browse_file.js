        ...
        _methods = {
            ...
            removePhotoItems: function(idList) {
                ...
                if (result) {
                    ...
                    MT.trigger('photo.onRemovePhotoItems', iDs);
                }
            ...
            buildPhotoList: function(data) {
                if (data.photoList.length === 0) {
                    ...
                    if (_elements.content[0].childNodes.length === 0) {
                        _elements.content.append($('<div>', { style: 'text-align:center; padding-top: 24px;' }).html(MT.getLanguageText('admin', 'no_items')));
                    }
            ...
            buildPhotoItem: function(photo) {
                ...
                MT.trigger('photo.onRenderPhotoItem', [$.extend({}, photo)], photoItem);

                _methods.buildPhotoItem.buildByMode(photoItem, photo);
                MT.trigger('photo.photoItemRendered', photoItem);
            },
            ...
            reorder: function() {
                ...
                $('.mt_photo_item_wrap', _elements.content).each(function() {
                    var self = $(this),
                        top, left;
            ...
            setDescription: function(entity) {
                this.find('.mt_photo_item_info_title span').html(entity.title);
                this.find('.mt_photo_item_info_dimensions span').html(entity.dimensions);
                this.find('.mt_photo_item_info_fileszie span').html(entity.filesize);
                this.find('.mt_photo_item_info_uploaddate span').html(entity.addDatetime);
                var link = $('<a></a>', {
                    href: "javascript://",
                    class: 'clipboard-button',
                    'data-clipboard-text': entity.url,
                    'text': MT.getLanguageText('admin', 'copy_url')

                });
                this.find('.mt_photo_item_info_url').append(link);
            },
    ...
    _methods.buildPhotoItem.buildByMode = (function() {
        if (_vars.classicMode) {
            return function(photoItem, photo) {
                photoItem.find('.mt_photo_item')[0].style.backgroundImage = 'url(' + photo.url + ')';
                photoItem.find('img.mt_hidden').attr('src', photo.url);
    ...
    window.browsePhoto = Object.defineProperties({}, {
        init: {
            value: function() {
                ...
                MT.bind('photo.onSetRate', _methods.updateRate);
    ...
    var _methods = {
        ...
        deleteImages: function(imagesIds) {
            if (imagesIds.length == 0) {
                MT.error(MT.getLanguageText('admin', 'no_photo_selected'));
                return false;
            }
            if (!confirm(MT.getLanguageText('admin', 'confirm_delete_images'))) {
                return false;
            }
        ...
        call: function(event) {
            var closest = $(this).closest('.mt_photo_item_wrap');
        ...
        init: function() {
            ...
            var event = { buttons: [] };
            MT.trigger('photo.collectMenuItems', [event]);

            if (_contextList.length === 0 && event.buttons.length === 0) {
                return;
            }

            var list = $('<ul>', { class: 'mt_context_action_list' });

            _contextList.concat(event.buttons).forEach(function(item) {
                item.appendTo(list);
            });

            _params.contextAction = $('<div>', { class: 'mt_photo_context_action' }).on('click', function(event) {
            ...
            MT.bind('photo.onRenderPhotoItem', function(album) {
                var self = $(this);
                ...
                if (_params.listType == 'albums' && album.name.trim() == MT.getLanguageText('photo', 'newsfeed_album').trim()) {
