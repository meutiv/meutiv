...
componentDragAndDrop.prototype = {
    ...
    prepare: function() {
        ...
        this.defaultSettings = {
            ...
            beforeStop: function(e, ui) {
                if (ui.item.parent().hasClass('mt_dnd_locked_section')) {
    ...
    initializePanel: function() {
        ...
        var panelSettings = this.extendSettings({
            ...
            over: function(e, ui) {
                ui.placeholder.removeClass('mt_dnd_placeholder');
    ...
    initializeSections: function() {
        ...
        var sectionSettings = this.extendSettings({
            cancel: '.mt_dnd_freezed',
            items: '.component:not(.mt_dnd_freezed)',
            receive: function(e, ui) {
                if (ui.placeholder.hasClass('mt_dnd_placeholder')) {
            ...
            over: function(e, ui) {
                ...
                ui.placeholder.addClass('mt_dnd_placeholder');
            },
    ...
    initializeActions: function() {
        ...
        this.$panel.find('.dd_delete').unbind().click(function() {
            if (!confirm(MT.getLanguageText('base', 'widgets_delete_component_confirm'))) {
                return false;
            }
    ...
    initializeScheme: function() {
        ...
        $sliderNode.schemeSwitcher({
            ...
            update: function(event) {
                var schemeId = $(event.marker).attr('mt_scheme');
    ...
    getHelper: function(sortable, e, ui) {
        ...
        return $('<div class="mt_dnd_helper" style="width: 160px; height: 30px"></div>');
    },
    ...
    handlStart: function(sortable, e, ui) {
        ...
        var within = $(ui.item).attr('mt_avaliable_sections');
    ...
    handleStop: function(sortable, e, ui) {
        ...
        event.to = {
            section: toSection,
            sectionName: toSection.attr('mt_place_section')
        };
        event.from = {
            section: fromSection,
            sectionName: fromSection.attr('mt_place_section')
        };
        event.ui = ui;
    ...
    handleSectionChange: function(sortable, e, ui) {
        ...
        if ($ph.next().is(cmpNode) || $ph.prev().is(cmpNode)) {
            $ph.removeClass('mt_dnd_placeholder');
            $ph.addClass('hidden-placeholder');
        } else {
            $ph.removeClass('hidden-placeholder');
            $ph.addClass('mt_dnd_placeholder');
        }
    },
    ...
    handleRemoveFromSection: function($item, completeFnc) {
        ...
        $item.removeClass('mt_dnd_freezed');
    ...
    disableSections: function($sections) {
        $.each($sections, function(i, item) {
            $(item).addClass('mt_dnd_locked_section');
        });
    },

    enableSections: function($sections) {
        $.each($sections, function(i, item) {
            $(item).removeClass('mt_dnd_locked_section');
        });
    },
    ...
    clone: function(sortable, $cloningItem) {
        ...
        var section = $(sortable).attr('mt_place_section');
    ...
    update: function(sortable) {
        ...
        var section = $selfNode.attr('mt_place_section');
        var stack = [];
    ...
    showSettings: function($component) {
        ...
        var $content = $('.settings_content', '#fb_settings').addClass('mt_preloader_content');

        var $controls = $('.settings_controls', '#fb_settings');
        $controls.find('input.dd_save').unbind();

        this.settingBox = new MT_FloatBox({
            $title: $title,
            $contents: $content,
            $controls: $controls,
            width: 500
        ...
        this.trigger('loadSettings', [cmpId, function(settingMarkup) {
            $content.empty();
            $content.removeClass('mt_preloader_content');
            ...
            if (settingMarkup.styleDeclarations) {
                MT.addCss(settingMarkup.styleDeclarations)
            }

            if (settingMarkup.scriptFiles) {
                MT.addScriptFiles(settingMarkup.scriptFiles, function() {
                    if (settingMarkup.onloadScript) {
                        MT.addScript(settingMarkup.onloadScript);
                    }
                });
            } else {
                if (settingMarkup.onloadScript) {
    ...
    saveSettings: function(cmpId, form, submitControl) {
        ...
        MT.inProgressNode(submitControl);
    ...
    applyComponentSettings: function(cmpId, settings) {
        ...
        if (settings.freeze > 0) {
            $component.addClass('mt_dnd_freezed');
    ...
    arrangeFreezed: function(section) {
        ...
        /*var components = $section.find('.component').sort(function(a, b){
        	var x = $(a).is('.mt_dnd_freezed') ? 0 : 1;
        	var y = $(b).is('.mt_dnd_freezed') ? 0 : 1;
        ...
        var components = [];
        $section.find('.mt_dnd_freezed').each(iteration);
        $section.find('.component:not(.mt_dnd_freezed)').each(iteration);
