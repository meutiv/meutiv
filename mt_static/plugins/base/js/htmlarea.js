    ...
    jHtmlArea.fn = jHtmlArea.prototype = {
        ...
        init: function(elem, options) {
            ...
            if (elem.nodeName.toLowerCase() === "textarea") {
                ...
                var container = this.container = $("<div/>").addClass("jhtmlarea").width('95%').addClass('mt_smallmargin').insertAfter(textarea);
                ...
                MT.trigger('base.initjHtmlArea', [], this);
            }
        },
        dispose: function() {
        ...
        pasteHTML: function(html) {
            ...
            var window = this.iframe[0].contentWindow,
        ...
        insertVideo: function(params) {
            ...
            if (!params || !params.code) {
                MT.error(window.htmlAreaData.labels.messages.videoEmptyField);
                return;
            }
            $html = $('<div><span class="mt_ws_video"></span></div>');
        ...
        addHtml: function(params) {
            ...
            $html = $('<div><span class="mt_ws_html"></span></div>');
            $('span', $html).append(params.code);
            this.pasteHTML($html.html());
            this.tempFB.close();
        },
        ...
        insertLink: function(params) {
            ...
            if (!params || !params.url || !params.label) {
                MT.error(window.htmlAreaData.labels.messages.linkEmptyFields);
                return;
            }

            this.pasteHTML('<span class="mt_ws_link"><a rel="nofollow" href="' + params.url + '"' + (params.newWindow ? ' target="_blank"' : '') + '>' + params.label + '</a></span>');
            this.tempFB.close();
        },
            ...
            $.each(this.toolbarArray,
                function(key, $item) {
                    if ($item.toolbarHandler.call(self)) {
                        $item.addClass('mt_ws_active');
                    } else {
                        $item.removeClass('mt_ws_active');
                    }
                }
            );
        },
