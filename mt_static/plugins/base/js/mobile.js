var OWMobile = function() {
    ...
    var langs = {};
    var events = {};
    var $overlay = $('#owm_overlay'),
        leftSbClass = 'owm_sidebar_left_active',
        rightSbClass = 'owm_sidebar_right_active',
        $main = $('#main');
    var $leftHeaderBtn = $('#owm_header_left_btn'),
        $rightHeaderBtn = $('#owm_header_right_btn'),
        $heading = $('#owm_heading'),
        $title = $('title');
    var state = "content";
    ...
    this.showLeftSidebar = function() {
        ...
        state = 'left';
        self.trigger('mobile.shmt_sidebar', { type: state });
    };
    ...
    this.showRightSidebar = function(options) {
        ...
        if (!silent) {
            state = 'right';
            self.trigger('mobile.shmt_sidebar', { type: "right" });
        }
    };
    ...
    this.authorizationLimitedFloatbox = function(message) {
        OWM.ajaxFloatBox("BASE_MCMP_AuthorizationLimited", [message], { title: OWM.getLanguageText('base', 'authorization_limited_permissions') });
    };
...
var OWM_Console = function(params) {
    ...
    OWM.bind("mobile.shmt_sidebar", function(data) {

        var beforeShowSidebarData = { openDefaultTab: true };
        MT.trigger("mobile.before_shmt_sidebar", beforeShowSidebarData);
    ...
    OWM.bind('mobile.console_shmt_counter', function(data) {
    ...
    $("body")
        .on("click", "li.owm_sidebar_msg_disabled", function() {
            OWM.error(
                OWM.getLanguageText(
                    'base',
                    'mobile_disabled_item_message', { url: $(this).data("disabled-url") != "" ? decodeURIComponent($(this).data("disabled-url")) : params.desktopUrl })
            );
        });
...
var OWM_InvitationsConsole = function(params) {
    ...
    this.consoleLoadMore = function($node) {
        ...
        OWM.loadComponent(
            "BASE_MCMP_ConsoleInvitations", { limit: self.params.limit, exclude: exclude }, {
                onReady: function(html) {
...
OwForm.prototype = {
    ...
    submitForm: function() {
        ...
        var buttons = $('input[type=button], input[type=submit], button', '#' + this.id).addClass('mt_inprogress');
...
OwMobileComments.prototype = {
    ...
    initForm: function(textareaId, submitId) {
        ...
        var $textA = $('#' + textareaId, this.$cmpContext),
            $submitCont = $('#' + submitId, this.$cmpContext).closest('.comment_submit');
        if (!this.taMessage) {
        ...
        owForms[this.formName].bind('success',
            function(data) {
                ...
                MT.trigger('base.comment_add', { entityType: data.entityType, entityId: data.entityId }, this);
                $textA.focus();
            }
        );
        ...
        owForms[this.formName].bind('submit',
            function(data) {
                var cmpObj = owCommentCmps[self.genId],
                    count = cmpObj.getCommentsCount() + 1;
...
OwMobileCommentsList.prototype = {
    ...
    updateMarkup: function(data) {
        ...
        MT.trigger('base.comments_list_update', { entityType: data.entityType, entityId: data.entityId, id: this.genId });
    },
