var avatarChange = function(params) {
    ...
    this.initCrop = function(callback) {
        ...
        image.onload = function(e) {
            ...
            document.avatarFloatBox.$header.find(".floatbox_title span").html(MT.getLanguageText('base', 'avatar_crop'));
            var cropWidth = document.avatarFloatBox.$body.width() - self.params.minCropSize - 16;

            $(".mt_avatar_crop_area").css({ width: cropWidth });

            $(".mt_avatar_crop_result").css({ width: self.params.minCropSize });
    ...
    if (self.params.step == 2) {
        self.$backButton.closest(".mt_button").hide();

        // join form
        if (self.params.inputId) {

            self.$step2.hide();
            MT.inProgressNode(self.$preloader);
            self.$preloader.show();
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    image.onload = function(e) {
                        ...
                        var height = this.height;
                        if (width < self.params.minCropSize || height < self.params.minCropSize) {
                            MT.error(MT.getLanguageText('base', 'avatar_image_too_small', { width: self.params.minCropSize, height: self.params.minCropSize }));
    ...
    this.$cropButton.click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.result) {
                    if (!self.params.inputId) {
                        MT.info(MT.getLanguageText('base', 'avatar_changed'));
                    }

                    MT.trigger('base.avatar_cropped', data); // Listen this event to update interface
                    document.avatarFloatBox.close();
                } else {
                    if (data.error) {
                        MT.error(data.error);
                    } else {
                        MT.error(MT.getLanguageText('base', 'crop_avatar_failed'));
                    }
                }

                MT.activateNode(button);
            }
        });
    });
    ...
    this.$backButton.click(function() {
        self.$step2.hide();
        self.$step1.show();
        document.avatarFloatBox.$header.find(".floatbox_title span").html(MT.getLanguageText('base', 'avatar_change'));
    });
    ...
    self.$fileInput.change(function(e) {
        ...
        if (!files || !self.checkFileType(files[0].type)) {
            MT.error(MT.getLanguageText('base', 'not_valid_image'));
        ...
        reader.onload = function(e) {
            ...
            image.onload = function(e) {
                ...
                if (width < self.params.minCropSize || height < self.params.minCropSize) {
                    ...
                    self.$step1.show();
                    MT.activateNode(self.$preloader);
                    self.$preloader.hide();
                } else {
                    var fd = new FormData();
    ...
    this.$dropArea.on('dragenter', function(e) {
        e.stopPropagation();
        e.preventDefault();
        self.$dropAreaLabel.html(MT.getLanguageText('base', 'drop_image_here'));
    });
    this.$dropArea.on('dragleave', function(e) {
        self.$dropAreaLabel.html(MT.getLanguageText('base', 'drag_image_or_browse'));
    });
    this.$dropArea.on('drop', function(e) {
        ...
        if (files && files.length > 1) {
            MT.error(MT.getLanguageText('base', 'avatar_drop_single_image'));
        } else {
            if (!files || !self.checkFileType(files[0].type)) {
                MT.error(MT.getLanguageText('base', 'not_valid_image'));
            } else {
    ...
    $("body").on("click", ".mt_photo_avatar_hover", function() {
        var image = new Image();
        var $btn = $(this).find(".avatar_select");
        var node = $(this).parents(".mt_photo_item:eq(0)");

        $btn.hide();
        node.addClass("mt_avatar_preloader");
        var timeout;

        function hidePreolader() {
            node.removeClass("mt_avatar_preloader");
            $btn.show();
    ...
    $(".avatar_load_more").click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.result) {
                    var $item = $parent.closest(".mt_photo_item_wrap");

                    $item.prev().show();
                    $item.before(data.markup);
                    MT.updateScroll($('.mt_photo_library_wrap'));
                    $parent.removeClass("mt_preloader");
