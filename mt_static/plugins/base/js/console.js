MT_DataModel = function(data) {
    this.data = data || {};
    this.observers = [];
};

MT_DataModel.PROTO = function() {
...
MT_DataModel.prototype = new MT_DataModel.PROTO();


MT_Console = function(params, modelsData) {
    ...
    this.getData = function(key) {
        _models[key] = _models[key] || new MT_DataModel();
    ...
    var command = MT.getPing().addCommand('consoleUpdate', {
        before: function() {
            var c = this;
...
MT_ConsoleItem = {
    ...
    showContent: function() {
        ...
        MT.Console.hideOtherContents(this);
        ...
        this._$tooltip.css({ opacity: 0, top: this.$node.height() - 12 }).stop(true, true).animate({ top: this.$node.height(), opacity: 1 }, 'fast', function() {
            self.animate = false;
            MT.addScrolls(self.$contentNode);
...
MT_ConsoleDropdownHover = function(uniqId, contentUniqId) {
...
MT_ConsoleDropdownHover.prototype = MT_ConsoleItem;


MT_ConsoleDropdownClick = function(uniqId, contentUniqId) {
    this.init(uniqId, contentUniqId);

    this.initDropdown();
}
$.extend(MT_ConsoleDropdownClick.prototype, MT_ConsoleItem, {
    initDropdown: function() {
        var self = this;

        this.addObserver({
            beforeShow: function() {
                self.$node.addClass('mt_console_dropdown_pressed');
            },

            beforeHide: function() {
                self.$node.removeClass('mt_console_dropdown_pressed');
            }
        });
...
MT_ConsoleDropdownList = function(uniqId, contentUniqId) {
    this.init(uniqId, contentUniqId);
    this.initDropdown();

    var $counter = $('.MT_ConsoleItemCounter', this.$node),
        $number = $counter.find('.MT_ConsoleItemCounterNumber'),
        $place = $counter.find('.MT_ConsoleItemCounterPlace');
    ...
    var numberSetActive = function(active) {
        ...
        $place[active ? 'addClass' : 'removeClass']('mt_count_active');
    }
...
MT_ConsoleDropdownList.prototype = MT_ConsoleDropdownClick.prototype;
...
MT_ConsoleList = {
    construct: function(params) {
        ...
        this.rsp = params.rsp;
        this.key = params.key;
        this.data = MT.Console.getData(this.key);
        this.item = MT.Console.getItem(this.key);

        this.$container = function() {
            return $('.MT_ConsoleListContainer', self.$contentNode);
        };

        this.$list = function() {
            return $('.MT_ConsoleList', self.$container());
        };

        this.$preloader = function() {
            return $('.MT_ConsoleListPreloader', self.$container());
        };

        this.$noContent = function() {
            return $('.MT_ConsoleListNoContent', self.$container());
        };
        ...
        this.item._onHide = function() {
            MT.removeScroll(this.$container().get(0));
        };
    },
    ...
    _loadList: function() {
        ...
        var request = JSON.stringify({
            console: MT.Console.getData('console').get(),
            data: this.data.get(),
            target: target,
    ...
    addItems: function(items, updateScrooll) {
        ...
        $.each(items, function(i, item) {
            ...
            MT.trigger('base.onAddConsoleItem', [], $item);
        });
    ...
    updateScroll: function(toTop) {
        ...
        if (this.opened) {
            ...
            var jsp = MT.addScroll(this.$container().get(0));
    ...
    _ajaxSuccess: function(resp) {
        ...
        if (resp && resp.markup) {
            ...
            if (resp.markup.styleDeclarations) {
                MT.addCss(resp.markup.styleDeclarations);
            }

            if (resp.markup.beforeIncludes) {
                MT.addScript(resp.markup.beforeIncludes);
            }

            if (resp.markup.scriptFiles) {
                MT.addScriptFiles(resp.markup.scriptFiles, function() {
                    if (resp.markup.onloadScript) {
                        MT.addScript(resp.markup.onloadScript);
                    }
                });
            } else {
                if (resp.markup.onloadScript) {
                    MT.addScript(markup.onloadScript);
                }
            }
        }
...
MT_Invitation = function(itemKey, params) {
    ...
    this.removeItem = function(invitationKey) {
        ...
        if (item.hasClass('mt_console_new_message')) {
