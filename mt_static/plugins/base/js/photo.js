(function(window, $, params) {
    ...
    var _vars = $.extend({}, (params || {}), {
        ...
        _elements = {
            preloader: $('<div>', { class: 'mt_floatbox_preloader mt_photo_preload' })
        },
        _methods = {
            isLoading: function() {
            ...
            setContent: function(photoId) {
                ...
                if (cmp.contextAction.length) {
                    $(cmp.contextAction).addClass('mt_photo_context_action').insertAfter($('.mt_photoview_arrmt_right', content)).show();
                }

                $("#photo-delete").off().on('click', function(event) {
                    var photoId = $(this).attr("rel");

                    if (confirm(MT.getLanguageText('admin', 'confirm_delete'))) {
                        $.ajax({
                            url: _vars.ajaxResponder,
                            type: 'POST',
                ...
                $('.mt_photoview_title input', content).val(cmp.photo.title);
                $('.mt_photoview_url input', content).val(cmp.photo.url).attr('id', 'photo-target-' + photoId);
                $('.mt_photoview_url a', content).addClass('floatbox-clipboard-button');
                var randId = Math.random().toString(36).substring(7);
                $('.mt_photoview_url a', content).attr('id', randId);
                $('.mt_photoview_url a', content).unbind('click');
                document.getElementById(randId).setAttribute('data-clipboard-target', '#photo-target-' + photoId);
                $('.mt_photoview_date span', content).html(cmp.photo.addDatetime);
                $('.mt_photoview_size span', content).html(cmp.photo.dimensions);
                $('.mt_photoview_filesize span', content).html(cmp.photo.filesize);
                $('.mt_photoview_id', content).val(photoId);
            },
            ...
            init: function() {
                ...
                $('.mt_photoview_stage_wrap,img.mt_photo_img', content).on('click', function(event) {
