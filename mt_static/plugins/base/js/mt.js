var OwUtils = function() {
    ...
    var $messageCont = $('<div class="mt_message_cont"></div>');
    var events = {};
    ...
    this.message = function(message, type, paramTime) {
        var $messageNode = $('<div class="mt_message_node ' + type + '" style="display:none;"><div><div><a class="close_button" href="javascript://" onclick="$(this).closest(\'.mt_message_node\').slideUp(200, function(){$(this).remove();})"></a>' + message + '</div></div></div>').appendTo($messageCont);
    ...
    this.flagContent = function(entityType, entityId) {
        MT.registry['flag-panel-fb'] = MT.ajaxFloatBox("BASE_CMP_Flag", [entityType, entityId], {
            width: 315,
            title: MT.getLanguageText('base', 'flag_as')
        });
    };
    ...
    this.showUsers = function(userIds, title) {
            MT.Users.showUsers(userIds, title);
        },
         ...
         * Examples:
         * 1:
         * MT.loadComponent(cmpClass, '.place_to_node');
         *
         * 2:
         * MT.loadComponent(cmpClass, function(){
         *     //onReady event
         *     //Add the html somewhere
         * });
         *
         * 3:
         * MT.loadComponent(cmpClass, [ p1, p2, ... ], '.place_to_node');
         *
         * 4:
         * MT.loadComponent(cmpClass, [ p1, p2, ... ], function(html){
         *     //onReady event
         *     //Add the html somewhere
         * });
         *
         * 5:
         * MT.loadComponent(cmpClass, [ p1, p2, ... ],
         * {
         *     onLoad: function(){},
         *     onError: function(){},
         *     onComplete: function(){},
        ...
        this.loadComponent = function(cmpClass, p1, p2) {
            ...
            if (options.place) {
                $preloader = $('<div class="mt_ajaxloader_preloader mt_preloader_content ' + options.addClass + '"></div>');
            ...
            var ajaxOptions = {
                ...
                success: function(markup) {
                    ...
                    MT.bindAutoClicks($contentHtml);
                    MT.bindTips($contentHtml);
                    ...
                    if (markup.styleDeclarations) {
                        MT.addCss(markup.styleDeclarations);
                    }

                    if (markup.beforeIncludes) {
                        MT.addScript(markup.beforeIncludes, options.scope, options.context);
                    }

                    if (markup.scriptFiles) {

                        MT.addScriptFiles(markup.scriptFiles, function() {
                            if (markup.onloadScript) {
                                MT.addScript(markup.onloadScript, options.scope, options.context);
                                options.onLoad();
                            }
                        });
                    } else {
        ...
        this.ajaxFloatBox = function(cmpClass, params, options) {
            ...
            this.loadComponent(cmpClass, params, {
                ...
                onReady: function(r) {
                    ...
                    MT.trigger('base.ajax_floatbox_loaded', { cmpClass: cmpClass });
                }
            })
    ...
    this.editLanguageKey = function(prefix, key, success) {
        var fb = MT.ajaxFloatBox("BASE_CMP_LanguageValueEdit", [prefix, key, true], { width: 520, title: this.getLanguageText('admin', 'edit_language') });

        MT.bind("admin.language_key_edit_success", function(e) {
            fb.close();
            MT.unbind("admin.language_key_edit_success");
            success(e);
        });
    };
    ...
    this.authorizationLimitedFloatbox = function(message) {
        MT.ajaxFloatBox(
            "BASE_CMP_AuthorizationLimited", { message: message }, { width: 500, title: this.getLanguageText('base', 'authorization_limited_permissions') }
        );
    };
        ...
        $.each(autoClicks, function(i, o) {
            ...
            $('textarea.invitation, textarea[placeholder]', context)
                .bind('focus.auto_click', { context: context },
                    function(e) {
                        $('.mt_submit_auto_click', e.data.context).show();
                        ...
                        .bind('blur.auto_click', {context:context},
                            function(e){
                                if( $(this).hasClass('invitation') ){
                                    $('.mt_submit_auto_click', e.data.context).hide();
                                }
                            }
                        );*/
        });
    ...
    this.initWidgetMenu = function(items) {
        ...
        $.each(items, function(key, value) {
            if ($toolbarCont === null) {
                $contex = $('#' + value['contId']).closest('.mt_box, .mt_box_empty');
                $toolbarCont = $('.mt_box_toolbar_cont', $contex);
            }
                    ...
                    if (e.data.value.toolbarId != undefined) {
                        if (e.data.value.toolbarId) {
                            if ($toolbarCont.length === 0) {
                                $toolbarCont = $('<div class="mt_box_toolbar_cont"></div>');
    ...
    this.showTip = function($el, params) {
        ...
        var showTipN = function() {
            ...
            var tod = setTimeout(function() {
                $el.data('owTip').show(1,
                    function() {
                        if (params.hideEvent) {
                            $el.bind(params.hideEvent, function() { MT.hideTip($el) });
                        }
        ...
        if (params.show != null) {
            ...
            showContent = '<span class="mt_tip_title">' + $el.attr('title') + '</span>';
        }

        var $rootEl = $('<div class="mt_tip mt_tip_' + params.side + '"></div>').css({ display: 'none' }).append($('<div class="mt_tip_arrow"><span></span></div><div class="mt_tip_box">' + showContent + '</div>'));
    ...
    this.bindTips = function($context) {
        //$('*[title]', $context).each( function(i, o){$(o).hover(function(){MT.showTip($(this), {timeout:200})}, function(){MT.hideTip($(this))})});
        $('*[title]', $context).each(function(i, o) {
            $(o).on('mouseenter', function() { MT.showTip($(this), { timeout: 200 }); });
            $(o).on('mouseleave', function() { MT.hideTip($(this)); });
            $(o).on('mousewheel', function() { MT.hideTip($(this)); });
        });
    };
    ...
    this.showImageInFloatBox = function(src) {
        var floatBox = new MT_FloatBox({ layout: 'empty' });
...
jQuery.fn.extend({
    inprogress: function() {
        this.each(function() {
            var $this = jQuery(this).addClass('mt_inprogress');
            this.disabled = true;
    ...
    activate: function() {
        this.each(function() {
            var $this = jQuery(this).removeClass('mt_inprogress');
            this.disabled = false;
 ...
 */
function MT_FloatBox(options) {
    var fl_box = this;
    var fb_class;
    this.parentBox = MT.getActiveFloatBox();
    ...
    if (typeof document.body.style.maxHeight === 'undefined') { //if IE 6
        ...
        if (document.getElementById('floatbox_HideSelect') === null) { //iframe to hide select elements in ie6
            ...
            fb_class = MT_FloatBox.detectMacXFF() ? 'floatbox_overlayMacFFBGHack' : 'floatbox_overlayBG';
    ...
    MT.trigger('base.floatbox_loaded');
}

MT_FloatBox.version = 3;
MT_FloatBox.detectMacXFF = function() {
    var userAgent = navigator.userAgent.toLowerCase();
    return (userAgent.indexOf('mac') != -1 && userAgent.indexOf('firefox') != -1);
}

MT_FloatBox.prototype = {
    ...
    showPreloader: function(preloaderContent) {
        ...
        this.$canvas.addClass('mt_floatbox_loading');
    },

    hidePreloader: function() {
        this.$canvas.removeClass('mt_floatbox_loading');
    },
...
OwForm.prototype = {
    ...
    submitForm: function(target) {
        ...
        $(target).addClass('mt_inprogress');

        if (this.ajax) {
            MT.inProgressNode(buttons);
            var postString = '';
...
var OwAvatarField = function(id, name, params) {
    var formElement = new OwFormElement(id, name);
    var $preview = $(formElement.input).closest(".mt_avatar_field").find(".mt_avatar_field_preview");
    ...
    $(formElement.input).change(function(e) {
        document.avatarFloatBox = MT.ajaxFloatBox(
            "BASE_CMP_AvatarChange", { params: { step: 2, inputId: id, hideSteps: true, displayPreloader: true, changeUserAvatar: params.changeUserAvatar } }, { width: 749, title: MT.getLanguageText('base', 'avatar_change') }
        );
    });
...
var OwComments = function(params) {
    ...
    this.submitHandler = function() { MT.error('FORM IS NOT INITIALIZED!'); };
    this.attachmentInProgressHandler = function() { MT.error(params.labels.attachmentLoading); };
    ...
    this.$formWrapper = $('.mt_comments_form_wrap', this.$cmpContext);
    this.$formWrapperPre = $('.mt_comments_form_wrap_pre', this.$cmpContext);
    this.$hiddenBtnCont = $('.comments_hidden_btn', this.$cmpContext);
    this.$commentsInputCont = $('.mt_comments_item_info', this.$formWrapper);
    this.initTextarea();
    this.attachmentInfo = false;
    this.oembedInfo = false;

    MT.bind('base.comments_list_init',
        function(data) {
            if (sleeping) return;
            if (checkAddress(data)) {
    ...
    MT.bind('base.add_photo_attachment_submit',
        function(data) {
            if (sleeping) return;
            if (data.uid == self.attchUid) {
                ...
                MT.trigger('base.comment_button_show', self.eventParams);
            }
        }
    );

    MT.bind('base.attachment_added',
        function(data) {
            if (sleeping) return;
            if (data.uid == self.attchUid) {
                ...
                MT.trigger('base.comment_attachment_added', self.eventParams);
            }
        }
    );

    MT.bind('base.attachment_deleted',
        function(data) {
            if (sleeping) return;
            if (data.uid == self.attchUid) {
                self.attachmentInfo = false;
                self.$hiddenBtnCont.hide();
                MT.trigger('base.comment_attachment_deleted', self.eventParams);
            }
        }
    );

    MT.bind('base.move_comments_form',
        function(data) {
            if (sleeping) return;
...
OwComments.prototype = {
    repaintCommentsList: function(data) {
        ...
        //MT.trigger('base.comments_list_update', {entityType: data.entityType, entityId: data.entityId, id:this.uid});
        $('.comments_list_cont', this.$cmpContext).empty().append($(data.commentList));
        MT.addScript(data.onloadScript);
    },
    ...
    sendMessage: function(message) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                MT.trigger('base.photo_attachment_uid_update', { uid: self.attchUid, newUid: data.newAttachUid });
                self.eventParams.commentCount = data.commentCount;
                MT.trigger('base.comment_added', self.eventParams);
                self.attchUid = data.newAttachUid;

                self.$formWrapper.removeClass('mt_preloader');
    ...
    initTextarea: function() {
        ...
        this.realSubmitHandler = function() {
            ...
            self.$attchCont.empty();
            MT.trigger('base.photo_attachment_reset', { pluginKey: self.pluginKey, uid: self.attchUid });
            MT.trigger('base.comment_add', self.eventParams);

            self.$formWrapper.addClass('mt_preloader');
        ...
        this.$textarea
                ...
                function(e) {
                    if (e.which === 13 && !e.shiftKey) {
                        ...
                        if ($.trim(textBody) == '' && !self.attachmentInfo && !self.oembedInfo) {
                            MT.error(self.labels.emptyCommentMsg);
                            return false;
                        }
            ...
            .one('focus', function() { $(this).removeClass('invitation').val('').autosize({ callback: function(data) { MT.trigger('base.comment_textarea_resize', self.eventParams); } }); });
        ...
        if (this.mediaAllowed) {
            OWLinkObserver.observeInput(this.textAreaId, function(link) {
                if (!self.attachmentInfo) {
                    self.$attchCont.html('<div class="mt_preloader" style="height: 30px;"></div>');
                    this.requestResult(function(r) {
                        self.$attchCont.html(r);
                        self.$hiddenBtnCont.show();

                        MT.trigger('base.comment_attach_media', {})
                    });
...
var OwCommentsList = function(params) {
    ...
    this.$loader = $('.mt_comment_list_loader', this.$context);
}

OwCommentsList.prototype = {
    init: function() {
        var self = this;
        $('.mt_comments_item', this.$context).hover(function() {
            $('.cnx_action', this).show();
            $('.mt_comments_date_hover', this).hide();
        }, function() {
            $('.cnx_action', this).hide();
            $('.mt_comments_date_hover', this).show();
        });
        this.$loader.one('click',
            function() {
                self.$loader.addClass('mt_preloader');
                $('a', self.$loader).hide();
        ...
        //        MT.bind('base.comments_list_update',
        //            function(data){
        ...
        MT.trigger('base.comments_list_init', { entityType: this.entityType, entityId: this.entityId }, this);

        MT.bind('base.comment_add', function(data) { if (data.entityType == self.entityType && data.entityId == self.entityId) self.initialCount++ });
                ...
                $('#' + i).click(
                    function() {
                        if (confirm(self.delConfirmMsg)) {
                            $(this).closest('div.mt_comments_item').slideUp(300, function() { $(this).remove(); });
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    MT.trigger('base.comment_delete', eventParams, this);
                                }
                            });
                        }
                    }
    ...
    reload: function(page) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                self.$loader.removeClass('mt_preloader');
                $('a', self.$loader).hide();
                self.$context.replaceWith(data.commentList);
                MT.addScript(data.onloadScript);
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                MT.error('Ajax Error: ' + textStatus + '!');
                throw textStatus;
            }
        });
    }
    ...
    updateRate: function(rate) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                MT.trigger('base.rate_update', { entityType: self.entityType, entityId: self.entityId, rate: rate, ownerId: self.ownerId }, this);
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
...
OWLinkObserver.handler.prototype = {
    ...
    requestResult: function(callback, link) {
        ...
        $.ajax({
            type: 'POST',
            url: MT.ajaxAttachmentLinkRsp,
            data: { "url": link },
            dataType: 'json',
            success: function(r) {
                if (r.content) {
                    if (r.content.css) {
                        MT.addCss(r.content.css);
                    }
                    ...
                    if (r.content.js) {
                        MT.addScript(r.content.js);
                    }
                }

                if (r.attachment && MT_AttachmentItemColletction[r.attachment]) {
                    MT_AttachmentItemColletction[r.attachment].onChange = function(oembed) {
...
MT_AttachmentItemColletction = {};

MT_Attachment = function(uniqId, data) {
    var self = this;
    ...
    //MT.resizeImg(this.$('.EQ_AttachmentImageC'),{width:'150'});

    this.$('.MT_AttachmentSelectPicture').click(function() {
        self.showImageSelector();
    });

    this.$('.MT_AttachmentDelete').click(function() {
        $(self.node).remove();
...
MT_AttachmentProto = function() {
    ...
    this.showImageSelector = function() {
        ...
        $contents = this.$('.MT_AttachmentPicturesFbContent')

        fb = new MT_FloatBox({
            $title: this.$('.MT_AttachmentPicturesFbTitle'),
            $contents: $contents,
            width: 520
        });

        $contents.find('.MT_AttachmentPictureItem').unbind().click(function() {
            var img = $('img', this);
    ...
    this.changeImage = function(url) {
        ...
        original = this.$('.MT_AttachmentImage');
...
MT_Attachment.prototype = new MT_AttachmentProto();
...
MT_PingCommand = function(commandName, commandObject, stack) {
...
MT_PingCommand.prototype = new MT_PingCommand.PROTO();
...
MT_Ping = function() {
    ...
    this.addCommand = function(commandName, commandObject) {
        ...
        _commands[commandName] = new MT_PingCommand(commandName, commandObject, _stack);
...
MT_Ping.getInstance = function() {
    if (!MT_Ping.pingInstance) {
        MT_Ping.pingInstance = new MT_Ping();
    }

    return MT_Ping.pingInstance;
}



MT.getPing = function() {
    return MT_Ping.getInstance();
};

/* Add command example
MT.getPing().addCommand('ajaxim', {
    params: {
        p1: 1,
        p2: 3
    },
...
MT.addScroll = function(node, settings) {
    settings = settings || {};
    settings = $.extend({}, MT.Scroll.settings, settings);
    ...
    if (!$node.hasClass('mt_scrollable')) {
        $node.addClass('mt_scrollable');
    }
...
MT.removeScroll = function(node) {
    ...
    $node.removeClass('mt_scrollable');
};

MT.updateScroll = function(node, settings) {
    var jsp = $(node).data('jsp');
...
MT.addScrolls = function(context) {
    context = context || null;

    $('.mt_scrollable', context).each(function() {
        MT.addScroll(this);
    });
};

MT.removeScrolls = function(context) {
    context = context || null;

    $('.mt_scrollable', context).each(function() {
        MT.removeScroll(this);
    });
};

MT.updateScrolls = function(context) {
    context = context || null;

    $('.mt_scrollable', context).each(function() {
        MT.updateScroll(this);
    });
};
...
MT.Users = null;
MT_UsersApi = function(_settings) {
    var _usersApi = this;

    var _query = function(command, params, callback) {
        callback = callback || function(r) {
            if (!r) return;
            if (r.info) MT.info(r.info);
            if (r.error) MT.error(r.error);
        };
    ...
    this.deleteUser = function(userId, callBack, showMessage) {
            ...
            floatBox = MT.ajaxFloatBox('BASE_CMP_DeleteUser', [{ userId: userId, showMessage: showMessage && redirectUrl }], {
                title: MT.getLanguageText('base', 'delete_user_confirmation_label'),
                iconClass: 'mt_ic_add',
                scope: {
        ...
        this.showUsers = function(userIds, title) {
            title = title || MT.getLanguageText('base', 'ajax_floatbox_users_title');

            MT.ajaxFloatBox('BASE_CMP_FloatboxUserList', [userIds], { iconClass: "mt_ic_user", title: title, width: 470 });
        },
    ...
    this.blockUserWithConfirmation = function(userId, confirmationCallback, callback) {
        var floatBox = MT.ajaxFloatBox("BASE_CMP_BlockUser", [userId], {
            scope: {
...
MT.postRequest = function(url, params) {
    var $form = $("<form></form>")
        .attr({
            action: url,
            method: "post"
...
MT.ResponsiveMenu = (function() {
    ...
    // Private methods
    function changed() {
        this.node[this.moreItems.length ? "addClass" : "removeClass"]("mt_main_menu_more_active");
    }
