var MT_BaseFieldValidators = function($params, $emailPattern, $usernamePattern, userId) {
    var self = this;
    ...
    if (username.length == 0) {
        username = $("form[name='" + this.formName + "'] input.mt_username_validator");
    }

    if (email.length == 0) {
        email = $("form[name='" + this.formName + "'] input.mt_email_validator");
    }
        ...
        if (username.val().length > 0) {
            if (self.errors['username']['value'] !== username.val()) {
                ...
                if (!this.usernamePattern.test(username.val())) {
                    self.errors['username']['error'] = MT.getLanguageText('base', 'join_error_username_not_valid');
                    element.showError(MT.getLanguageText('base', 'join_error_username_not_valid'));
                    return false;
                } else {
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data.result == false) {
                                self.errors['username']['error'] = MT.getLanguageText('base', 'join_error_username_already_exist');
                                element.showError(MT.getLanguageText('base', 'join_error_username_already_exist'));
                            }
                        }
                    });
                }
        ...
        if (email.val().length > 0) {
            if (self.errors['email']['value'] !== email.val()) {
                ...
                if (!this.emailPattern.test(email.val())) {
                    self.errors['email']['error'] = MT.getLanguageText('base', 'join_error_email_not_valid');
                    element.showError(MT.getLanguageText('base', 'join_error_email_not_valid'));
                    return false;
                } else {
                    $.ajax({
                        url: self.responderUrl,
                        ...
                        success: function(data) {
                            if (data.result == false) {
                                self.errors['email']['error'] = MT.getLanguageText('base', 'join_error_email_already_exist');
                                element.showError(MT.getLanguageText('base', 'join_error_email_already_exist'));
                            }
                        }
                    });
    ...
    this.validatePassword = function() {
        ...
        if (password.val().length > 0 && password.val().length < this.passwordMinLength) {
            self.errors['password']['error'] = MT.getLanguageText('base', 'join_error_password_too_short');
            return false;
        } else if (password.val().length > this.passwordMaxLength) {
            self.errors['password']['error'] = MT.getLanguageText('base', 'join_error_password_too_long');
            return false;
        } else if (password.val() !== passwordRepeat.val() & passwordRepeat.val().length > 0) {
            self.errors['password']['error'] = MT.getLanguageText('base', 'join_error_password_not_valid');
            return false;
        }
