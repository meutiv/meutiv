(function($) {
    ...
    $.fn.schemeSwitcher = function(options) {
        var $node = $(this);
        var $handle = $node.find('.mt_dnd_slider_handle');
        var $markers = $node.find(".mt_dnd_slider_marker");
        var $activeMarker = $node.find('.mt_dnd_slider_item .current');
        ...
        $handle.draggable({
            ...
            start: function() {
                $(this).addClass('mt_dnd_slider_in_move');
            },
            stop: function() {
                $(this).removeClass('mt_dnd_slider_in_move');
                updateDelegate.apply($activeMarker.get(0));
            },
            helper: function() {
                return $(this).clone(false).addClass('mt_dnd_slider_helper');
            }
        });

        $markers.droppable({
            tolerance: 'touch',
            accept: '.mt_dnd_slider_handle',
            over: changeDelegate
        });
        ...
        for (var i = 0; i < $markers.length; i++) {
            ...
            var $pointer = $('<div class="mt_dnd_slider_marker_point"></div>');
            $node.append($pointer);
