var OWFileAttachment = function(params) {
    ...
    this.$previewCont = $('.mt_file_attachment_preview', this.$context);
    ...
    var refreshClasses = function() {
        ...
        $.each(items,
            function(index, data) {
                data['html'].removeClass('mt_file_attachment_block1').removeClass('mt_file_attachment_block2').addClass('mt_file_attachment_block' + itemIndex);
    ...
    this.addItem = function(data, loader, customDeleteUrl, customConfirmation) {
        data['html'] = $('<div><div class="mt_file_attachment_info">' +
            '<div class="mt_file_attachment_name">' + data.name + ' <span class="mt_file_attachment_size" style="display: inline-block;">(' + data.size + 'KB)</span></div>' +
            '<div class="mt_file_attachment_preload" style="display:' + (loader ? 'block' : 'none') + ';"></div>' +
            '<a href="javascript://" class="mt_file_attachment_close"></a>' +
            '</div></div>');
        ...
        MT.trigger('base.attachment_rendered', { 'data': data }, this);

        $('.mt_file_attachment_close', data['html']).bind('click', function() {
            var confirmed = true;
    ...
    this.initInput = function() {
        var $input = $('<input class="mlt_file_input" type="file"' + (this.multiple ? ' multiple=""' : '') + ' name="mt_file_attachment[]" />');
        ...
        $input.change(
            function(e) {
                ...
                MT.trigger('base.add_attachment_to_queue', { 'pluginKey': self.pluginKey, 'uid': self.uid, 'items': items });
        ...
        if (data.items) {
            $.each(data.items, function(index, item) {
                ...
                if (item.result) {
                    ...
                    if (self.showPreview) {
                        $('.mt_file_attachment_preload', items[index]['html']).hide();
                    }
                } else {
                    self.deleteItem(index);
                    if (self.showPreview) {
                        MT.error(item.message);
                    }
                }
            });

            MT.trigger('base.update_attachment', { 'pluginKey': self.pluginKey, 'uid': self.uid, 'items': data.items });
        }
    ...
    this.deleteItem = function(id, customDeleteUrl) {
        MT.trigger('base.attachment_deleted', { 'id': id }, this);
...
MT.bind('base.file_attachment', function(data) {
...
var OWPhotoAttachment = function(params) {
    $.extend(this, params);
    var self = this,
        $previewCont = $('#' + this.previewId),
        $buttonCont = $('#' + this.buttonId),
        $form,
        $iframe = null,
        $item = $('.mt_photo_attachment_pic', $previewCont),
        canceled = false;

    this.eventParams = { uid: self.uid, pluginKey: self.pluginKey };

    this.initInput = function() {
        var $input = $('<input class="mlt_file_input" type="file" name="attachment" />'),
            self = this;
        $buttonCont.empty().append($input).show();
        MT.trigger('base.attachment_shmt_button_cont', self.eventParams);
        ...
        $('div', $item).unbind('click').click(function() {
            ...
            $previewCont.hide();
            self.initInput();
            MT.trigger('base.attachment_deleted', self.eventParams);
        });
        ...
        $input.change(
            function(e) {
                $buttonCont.hide();
                MT.trigger('base.attachment_hide_button_cont', self.eventParams);
                $previewCont.show();
                ...
                $iframe = $('<div style="display:none" id="hd_' + self.uid + '"><div>').appendTo($('body'))
                    ...
                    .append($form);
                $form.submit();
                MT.trigger('base.add_photo_attachment_submit', self.eventParams);
            }
        );
    };
    ...
    this.updateItem = function(data) {
        ...
        var self = this,
            eventParams = { uid: self.uid, pluginKey: self.pluginKey, url: data.url };
        if (data.result) {
            var previewImg = new Image();
            previewImg.onload = function() {
                $item.removeClass('loading').css({ backgroundImage: 'url(' + data.url + ')' }).click(function() {
                    MT.showImageInFloatBox(data.url)
                });
                $('div', $item).unbind('click').click(function(e) {
                    e.stopPropagation();
                    self.initInput();
                    MT.trigger('base.attachment_deleted', eventParams);
                });
                MT.trigger('base.attachment_added', eventParams);
            };
    ...
    MT.bind('base.photo_attachment_reset', function(data) {
...
MT.bind('base.photo_attachment_uid_update', function(data) {
