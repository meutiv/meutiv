    ...
    var _vars = $.extend({}, (window.ajaxFileUploadParams || {}), {
        ...
        _methods = {
            ...
            updateSlot: function(slotId, fileUrl, id, filename) {
                ...
                slot.find('.mt_photo_preview_x').on('click', function() {
                    _methods.destroySlot(slotId, id);
                });
                slot.find('.mt_photo_preview_rotate').on('click', function() {
                    var photo = slot.find('.mt_photo_preview_image'),
                        _rotate;
                ...
                img.onload = function() {
                    slot.find('.mt_photo_preview_image')
                        .hide(0, function() {
                            this.style.backgroundImage = 'url(' + img.src + ')';
                            $(this).removeClass('mt_photo_preview_loading').fadeIn(300);

                            MT.trigger('photo.onRenderUploadSlot', [_elements.descEditors[slotId]], slot);
                        });
                };
                img.src = fileUrl;
            },
            initHashtagEditor: function(slotId) {
                ...
                editor.setValue(MT.getLanguageText('photo', 'describe_photo'));
                editor.on('blur', function(editor) {
                    var value = editor.getValue().trim(),
                        lineCount;

                    if (value.length === 0 || value === MT.getLanguageText('photo', 'describe_photo')) {
                    ...
                    slot.find('.mt_photo_preview_image').removeClass('mt_photo_preview_image_active');

                    if (_elements.slotArea.find('.mt_photo_preview_image_active').length === 0) {
                        _elements.slotArea.removeClass('mt_photo_preview_image_filtered');
                    }
                });
                editor.on('focus', function(editor) {
                    ...
                    if (_elements.descCache.hasOwnProperty(slotId)) {
                        ...
                        if (value === MT.getLanguageText('photo', 'describe_photo')) {
                            editor.setValue('');
                        }
                    }
                    ...
                    _elements.slotArea.addClass('mt_photo_preview_image_filtered');
                    slot.find('.mt_photo_preview_image').addClass('mt_photo_preview_image_active');
                });
                ...
                if (_vars.isHTML5) {
                    ...
                    if (_methods.isAvailableFileSize(file.size) && (typeError = _methods.isAvailableFileType(file.type))) {
                        ...
                        if (typeError === undefined) {
                            MT.error(MT.getLanguageText('photo', 'size_limit', { name: file.name, size: (_vars.maxFileSize / 1048576) }));
                        } else {
                            MT.error(MT.getLanguageText('photo', 'type_error', { name: file.name }));
                        }
                    ...
                    if (file.search(/\.(?:jpe?g|png|gif)$/i) !== -1) {
                        ...
                        _elements.dropArea.off('click').on('click', function() { alert(MT.getLanguageText('photo', 'please_wait')) });
                        ...
                        MT.error(MT.getLanguageText('photo', 'type_error', { name: file }));
            ...
            requestSuccess: function(jsonStr, slotId) {
                ...
                if (data && data.status) {
                    ...
                    _methods.destroySlot(slotId);
                    MT.error(MT.getLanguageText('photo', 'not_all_photos_uploaded'));
                }
            },
            showAlbumList: function() {
                _elements.albumList.show();
                $('.upload_photo_spinner', _elements.albumForm).removeClass('mt_dropdown_arrmt_down').addClass('mt_dropdown_arrmt_up');
            },
            hideAlbumList: function() {
                _elements.albumList.hide();
                $('.upload_photo_spinner', _elements.albumForm).removeClass('mt_dropdown_arrmt_up').addClass('mt_dropdown_arrmt_down');
            },
            setIsRuning: function() {
                _vars.isRuning = true;
                MT.inProgressNode($(':submit', owForms['ajax-upload'].form));
            },
            afterUploadTask: function() {
                if (_vars.files.length !== 0) {
                    ...
                    MT.activateNode($(':submit', owForms['ajax-upload'].form));
                }
            }
        };

    var _a = $('<a>', { class: 'mt_hidden mt_content a' }).appendTo(document.body);
    MT.addCss('.cm-hashtag{cursor:pointer;color:' + _a.css('color') + '}');
    _a.remove();
    ...
    window.ajaxFileUploader = Object.defineProperties({}, {
        init: {
            value: function() {
                ...
                if (!_vars.isHTML5) {
                    _elements.dropAreaLabel.html(MT.getLanguageText('photo', 'dnd_not_support'));
                }
                    ...
                    (function() {
                        ...
                        if (_vars.isHTML5) {
                            eventMap.drop = function(event) {
                                ...
                                _elements.dropAreaLabel.html(MT.getLanguageText('photo', 'dnd_support'));

                                return false;
                            };
                            eventMap.dragenter = function() {
                                _elements.dropArea.css('border', '1px dashed #E8E8E8');
                                _elements.dropAreaLabel.html(MT.getLanguageText('photo', 'drop_here'));
                            };
                ...
                _elements.albumList = $('.mt_dropdown_list', _elements.albumForm);
                ...
                _elements.albumList.find('li').on('click', function() {
                    ...
                    .eq(0).on('click', function() {
                        ...
                        _elements.albumInput.val(MT.getLanguageText('photo', 'create_album'));
                        $('input[name="album-name"]', _elements.albumForm).val(MT.getLanguageText('photo', 'album_name'));
                        $('textarea', _elements.albumForm).val(MT.getLanguageText('photo', 'album_desc'));
                    })
                ...
                MT.bind('photo.onCloseUploaderFloatBox', function() {
