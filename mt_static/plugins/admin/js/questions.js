...
questionValuesField.prototype = {
    ...
    construct: function(params) {
        ...
        self.valuesArea.on('click', '.remove', function() {
            if (confirm(MT.getLanguageText('admin', 'questions_edit_delete_value_confirm_message'))) {
    ...
    setValue: function(values) {
        ...
        if (list) {
            ...
            self.renderValues();
            MT.trigger('question.value.add', { values: addValues, node: self.tr });
    ...
    deleteValue: function(element) {
        ...
        div.remove();
        MT.trigger('question.value.delete', { value: item, node: self.tr });
        self.updateDataField();
    },
    ...
    if (list) {
        ...
        $.each(list, function(key, item) {
            if (item != undefined && item.trim() != '') {
                ...
                MT.error(MT.getLanguageText('admin', 'questions_values_should_not_be_empty'));
            }
        });
...
questionAccountTypesModel.prototype = {
    ...
    loadDataFromView: function() {
        ...
        $.each(self.accountTypes, function(key, item) {
            self.data[item] = self.nodes[item].hasClass('mt_checkbox_cell_marked');
        });
    },
    ...
    change: function(element) {
        ...
        var element = $(element);
        //element.toggleClass('mt_checkbox_cell_marked');

        if (element.hasClass('mt_checkbox_cell_marked')) {
            element.removeClass('mt_checkbox_cell_marked');
            element.addClass('mt_checkbox_cell');
        } else {
            element.addClass('mt_checkbox_cell_marked');
            element.removeClass('mt_checkbox_cell');
        }
        ...
        self.data[accountType] = self.nodes[accountType].hasClass('mt_checkbox_cell_marked');
...
questionPagesModel.prototype = {
    ...
    loadDataFromView: function() {
        var self = this;
        self.setRequired(self.required_node.hasClass('mt_checkbox_cell_marked'));
        self.setJoin(self.join_node.hasClass('mt_checkbox_cell_marked'));
        self.setEdit(self.edit_node.hasClass('mt_checkbox_cell_marked'));
        self.setView(self.view_node.hasClass('mt_checkbox_cell_marked'));
        self.setSearch(self.search_node.hasClass('mt_checkbox_cell_marked'));
    },
    ...
    change: function(element) {
        ...
        switch (true) {
            case (element.hasClass('required')):
                self.setRequired(element.hasClass('mt_checkbox_cell_marked'));
                changes = 'required';
                break;

            case (element.hasClass('on_join')):
                self.setJoin(element.hasClass('mt_checkbox_cell_marked'));
                changes = 'onJoin';
                break;

            case (element.hasClass('on_edit')):
                self.setEdit(element.hasClass('mt_checkbox_cell_marked'));
                changes = 'onEdit';
                break;

            case (element.hasClass('on_search')):
                self.setSearch(element.hasClass('mt_checkbox_cell_marked'));
                changes = 'onSearch';
                break;

            case (element.hasClass('on_view')):
                self.setView(element.hasClass('mt_checkbox_cell_marked'));
                changes = 'onView';
                break;
        }
...
var indexQuestions = function($params) {
    ...
    var $questionDiv = $('.mt_admin_profile_questions_list_div');
    var $questionTable = $('.mt_admin_profile_questions_list');
    ...
    MT.bind("admin.add_account_type", function(params) {

        if (params && params.result.add == true && params.accountTypeName) {
            var floatbox = MT.getActiveFloatBox();

            if (floatbox) {
                floatbox.close();
            }

            MT.info(MT.getLanguageText('admin', 'questions_account_type_was_added'));

            window.location.reload();
        } else {
            MT.error(MT.getLanguageText('admin', 'questions_account_type_added_error'));
        }
    });

    MT.bind("admin.update_account_type", function(params) {
        ...
        if (params && params.result.update == true && params.accountTypeName) {
            ...
            MT.info(MT.getLanguageText('admin', 'questions_account_type_was_updated'));
        }
    });
    ...
    $('a.question_edit_button').click(function() {
        ...
        if (questionId) {
            MT.ajaxFloatBox('ADMIN_CMP_EditQuestion', [questionId], {
                width: '700px',
                title: MT.getLanguageText('admin', 'questions_edit_profile_question_title')
            });
        }
    });
    ...
    $('a.add_account_type').click(function() {
        ...
        MT.ajaxFloatBox('ADMIN_CMP_AddAccountType', [], {
            width: '500px',
            title: MT.getLanguageText('admin', 'questions_add_account_type_title')
        });
    });
    ...
    $('a.question_edit_account_type_button').click(function() {
        ...
        if (input.length > 0 && input.val()) {
            MT.ajaxFloatBox('ADMIN_CMP_EditAccountType', [input.val()], {
                width: '500px',
                title: MT.getLanguageText('admin', 'questions_edit_account_type_title')
            });
        }
    });
    ...
    $('a.question_delete_account_type_button').click(
        function(event) {
            ...
            if (confirm(MT.getLanguageText('admin', 'questions_delete_account_type_confirmation'))) {
                self.sendRequest = true;
                $.ajax({
                    url: self.responderUrl,
                    type: 'POST',
                    ...
                    success: function(result) {
                        ...
                        if (result && result.result == 'success') {
                            MT.info(result['message']);
                            window.location.reload();
                        } else if (result['message']) {
                            MT.error(result['message']);
                        }
                    },
                    dataType: 'json'
                });
    ...
    $('input.add_new_question_button').click(function() {
        //window.location = $questionAddUrl
        MT.ajaxFloatBox('ADMIN_CMP_AddQuestion', [], {
            width: '700px',
            title: MT.getLanguageText('admin', 'questions_add_profile_question_title')
        });
    });

    $('input.add_new_section_button').on('click', function() {
        MT.ajaxFloatBox('ADMIN_CMP_AddQuestionSection', [], {
            width: '600px',
            title: MT.getLanguageText('admin', 'questions_profile_question_sections_title')
        });
    });
    ...
    $('.question_delete_button').click(
        function() {
            ...
            if (confirm(MT.getLanguageText('admin', 'questions_delete_question_confirmation_' + $questionId))) {
                self.sendRequest = true;
                $.ajax({
                    url: self.responderUrl,
                    type: 'POST',
                    ...
                    success: function(result) {
                        ...
                        if (result['result'] == 'success') {
                            ...
                            $question_tr.removeClass('mt_alt1');
                            $question_tr.removeClass('mt_alt2');

                            $questionTable.find('.question_tr:not(.no_question):odd').addClass('mt_alt2');
                            $questionTable.find('.question_tr:not(.no_question):even').addClass('mt_alt1');
            ...
            $.ajax({
                ...
                .done(
                    function(result) {
                        ...
                        var message = MT.getLanguageText('admin', 'questions_delete_section_confirmation');
    ...
    $(".edit_sectionNameLang").click(function() {
        ...
        var $element = $tr.find(".section_value .mt_section_label");
        var $name = $(this).parents(".mt_admin_profile_questions_list:eq(0)").attr("sectionName");
    ...
    $questionDiv.sortable({
        ...
        items: '.mt_admin_profile_questions_list',
        cancel: 'no_section',
        cursor: 'move',
        tolerance: 'pointer',
        handle: '.question_section_tr',
        placeholder: 'section_placeholder mt_table_2 mt_smallmargin mt_admin_content',
        forcePlaceholderSize: true,

        update: function(event, ui) {
            var order = {};

            $questionDiv.find('.mt_admin_profile_questions_list:not(.no_section)').each(function(ord, o) {
        ...
        start: function(event, ui) {
            //$(ui.placeholder).append('<table class="mt_table_2 mt_smallmargin"><tr><td colspan="9"><div style="width:869px;"></div></td></tr></table>');
        ...
        helper: function(event, ui) {
            ...
            return $('<div class="mt_dnd_helper" style="width: 180px;height: 30px; text-align:center; vertical-align:middle;"></div>');
        }
    ...
    $questionTable.not(".about_my_match").sortable({
        ...
        snapToleranse: 50,
        forcePlaceholderSize: true,
        connectWith: '.mt_admin_profile_questions_list:not(.no_section):not(.about_my_match)',

        update: function(event, ui) {

            var newSection = ui.item.parents(".mt_admin_profile_questions_list:eq(0)");
        ...
        helper: function(event, ui) {
            this.oldSection = ui.parents(".mt_admin_profile_questions_list:eq(0)");
            var itemWidth = ui.outerWidth();
            if (itemWidth > 160) {
                var k = 160 / ui.outerWidth();
                var offset = k * (event.pageX - ui.position().left);
