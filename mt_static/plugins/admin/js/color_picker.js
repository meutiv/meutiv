var ColorPicker = function($parent, callback, colorC) {
    ...
    var FValidateColor = function(color) { if (!color) { return false; } if (color.match(new RegExp("^#(([0-9a-f]{3})|([0-9a-f]{6}))$", "i"))) return true;
        else return false };
    ...
    var $colorInputSubmit = $(Fce('input')).attr('type', 'button').attr('value', 'Ok').attr('class', 'mt_ic_ok').bind('click', function() { callback(color); });

    var $colorInputSubmitWrapper = $('<span class="mt_ic_ok"></span>').append($colorInputSubmit);
    $colorInputSubmitWrapper = $('<span class="mt_button"></span>').append($colorInputSubmitWrapper);
