...
abstract class ADMIN_CTRL_StorageAbstract extends ADMIN_CTRL_Abstract
{
    ...
    protected function getFtpConnection()
    {
        ...
        catch ( LogicException $e )
        {
            ...
            $this->redirect(MT::getRequest()->buildUrlQueryString(MT::getRouter()->urlFor("ADMIN_CTRL_Storage",
                        "ftpAttrs"),
                    array(BOL_StorageService::URI_VAR_BACK_URI => urlencode(MT::getRequest()->getRequestUri()))));
        }
    ...
    protected function redirectToBackUri( $getParams )
    {
        ...
        $this->redirect(MT::getRequest()->buildUrlQueryString(MT_URL_HOME . urldecode($backUri), $getParams));
    }

    protected function getTemDirPath()
    {
        return MT_DIR_PLUGINFILES . "ow" . DS;
    }
}
