...
class ADMIN_CTRL_Logs extends ADMIN_CTRL_Abstract
{
    ...
    public function index()
    {
        ...
        if ( $typeParam || $keyParam || $searchParam )
        {
            ...
            $this->assign('dropFiltersUrl', MT::getRouter()->urlForRoute('admin_developer_tools_logs'));
        }

        $paging = new BASE_CMP_Paging($page + 1, $totalPages, 10);
        $this->assign('paging', $paging->render());

        $this->assign('totalEntriesText', MT::getLanguage()->text(
            'base',
        ...
        $this->assign('entriesPerPageText', MT::getLanguage()->text(
            'base',
    ...
    public function entry( $params )
    {
        ...
        if ( $entryId <= 0 )
        {
            $this->redirect(MT::getRouter()->urlForRoute('admin_developer_tools_logs'));
            return;
        }
        ...
        $logEntryCustomViewEvent = new MT_Event(MT_EventManager::ON_LOG_ENTRY_CUSTOM_VIEW, array(
            'entry' => $entry
        ));

        MT::getEventManager()->trigger($logEntryCustomViewEvent);
