...
class ADMIN_CTRL_Seo extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        // activate menu item
//        MT::getDocument()->getMasterPage()->
//                getMenu(MT_Navigation::ADMIN_SETTINGS)->getElement('sidebar_menu_item_seo_settings')->setActive(true);

        $this->setPageHeading(MT::getLanguage()->text('admin', 'seo_page_heading'));
    ...
    public function index()
    {
        ...
        $this->assign("currentUrl", MT::getRouter()->urlForRoute("admin_settings_seo")."?section=#sec#");
        ...
        if( MT::getRequest()->isPost() ){
            ...
            {
                MT::getFeedback()->info(MT::getLanguage()->text('admin', 'settings_submit_success_message'));
                $this->redirect();
            }
            else
            {
                MT::getFeedback()->error($form->getEmptyElementsErrorMessage());
            }
        }
    }
    ...
    public function sitemap()
    {
        ...
        if ( MT::getRequest()->isPost() && $form->isValid($_POST) )
        {
            ...
            // save schedule
            MT::getConfig()->saveConfig('base', 'seo_sitemap_schedule_update', $formValues['schedule']);

            // reload the current page
            MT::getFeedback()->info(MT::getLanguage()->text('admin', 'seo_sitemap_settings_updated'));
            $this->redirect();
        }
    ...
    public function socialMeta()
    {
        ...
        if ( MT::getRequest()->isPost() )
        {
            ...
            if ( !$result["result"] )
            {
                MT::getFeedback()->error($result["message"]);
                $this->redirect();
            }
    ...
    protected function getMenu()
    {
        ...
        $item->setLabel(MT::getLanguage()->text('admin', 'seo_page'));
        $item->setIconClass('mt_ic_files');
        $item->setKey('seo_page');
        $item->setUrl(MT::getRouter()->urlForRoute('admin_settings_seo'));
        $item->setOrder(1);
        $items[] = $item;

        $item = new BASE_MenuItem();
        $item->setLabel(MT::getLanguage()->text('admin', 'seo_sitemap'));
        $item->setIconClass('mt_ic_script');
        $item->setKey('seo_sitemap');
        $item->setUrl(MT::getRouter()->urlForRoute('admin_settings_seo_sitemap'));
        $item->setOrder(2);
        $items[] = $item;

        $item = new BASE_MenuItem();
        $item->setLabel(MT::getLanguage()->text('admin', 'seo_social_meta'));
        $item->setIconClass('mt_ic_flag');
        $item->setKey('seo_social_meta');
        $item->setUrl(MT::getRouter()->urlForRoute('admin_settings_seo_social_meta'));
        $item->setOrder(3);
        $items[] = $item;
