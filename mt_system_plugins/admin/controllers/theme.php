...
class ADMIN_CTRL_Theme extends ADMIN_CTRL_Abstract
{
    ...
    public function init()
    {
        ...
        $pageActions = array(array('name' => 'settings', 'iconClass' => 'mt_ic_gear_wheel'), array('name' => 'css', 'iconClass' => 'mt_ic_files'), array('name' => 'graphics', 'iconClass' => 'mt_ic_picture'));
        ...
        foreach ( $pageActions as $key => $item )
        {
            ...
            $menuItem->setKey($item['name'])->setLabel(MT::getLanguage()->text('admin', 'sidebar_menu_item_' . $item['name']))->setOrder($key)->setUrl($router->urlForRoute('admin_theme_' . $item['name']));
        ...
        MT::getNavigation()->activateMenuItem(MT_Navigation::ADMIN_APPEARANCE, 'admin', 'sidebar_menu_item_theme_edit');
        $this->setPageHeading(MT::getLanguage()->text('admin', 'themes_settings_page_title'));
    }

    public function settings()
    {
        $dto = $this->themeService->findThemeByKey(MT::getConfig()->getValue('base', 'selectedTheme'));

        if ( $dto === null )
        {
            throw new LogicException("Can't find theme `" . MT::getConfig()->getValue('base', 'selectedTheme') . "`");
        }
        ...
        $this->assign('resetUrl', MT::getRouter()->urlFor(__CLASS__, 'reset'));
    ...
    public function css()
    {
        if ( MT::getRequest()->isAjax() )
        {
            ...
            echo json_encode(array('message' => MT::getLanguage()->text('admin', 'css_edit_success_message')));
        }

        if ( !MT::getRequest()->isAjax() )
        {
            MT::getDocument()->getMasterPage()->getMenu(MT_Navigation::ADMIN_APPEARANCE)->getElement('sidebar_menu_item_themes_customize')->setActive(true);
        }

        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('admin')->getStaticJsUrl() . 'prettify.js');
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('admin')->getStaticJsUrl() . 'lang-css.js');
        MT::getDocument()->addStyleSheet(MT::getPluginManager()->getPlugin('admin')->getStaticCssUrl() . 'prettify.css');
        MT::getDocument()->addOnloadScript("prettyPrint();");

        $fileString = file_get_contents(MT::getThemeManager()->getSelectedTheme()->getRootDir() . BOL_ThemeService::CSS_FILE_NAME);
    ...
    {
        return array(
            'url' => MT::getStorage()->getFileUrl($this->themeService->getUserfileImagesDir() . $image->getFilename()),
            'delUrl' => MT::getRouter()->urlFor(__CLASS__, 'deleteImage', array('image-id' => $image->getId())),
    ...
    public function graphics()
    {
        ...
        $this->assign('confirmMessage', MT::getLanguage()->text('admin', 'theme_graphics_image_delete_confirm_message'));

        $cmp = MT::getClassInstance('ADMIN_CMP_UploadedFileList');
        ...
        if ( MT::getRequest()->isPost() )
        {
            ...
            {
                MT::getFeedback()->error(MT::getLanguage()->text('admin', 'theme_graphics_upload_form_fail_message'));
                $this->redirect();
            }

            MT::getFeedback()->info(MT::getLanguage()->text('admin', 'theme_graphics_upload_form_success_message'));
            $this->redirect();
        }
    }
    ...
    public function bulkOptions()
    {
        ...
        switch ($action)
        {
            case 'delete':
                ...
                if (is_null($items))
                {
                    $result = json_encode(array('error' => MT::getLanguage()->text('admin', 'not_enough_params')));
                }
                else
                {
                    foreach ($items as $item)
                    {
                        ...
                        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'theme_graphics_delete_success_message'));
                    }
                    $result = json_encode(array(
                        'success' => MT::getLanguage()->text('admin', 'theme_graphics_delete_multiple_success_message'),
                        'reload' => true
                    ));
                }
                break;
            default:
                $result = json_encode(array('error' => MT::getLanguage()->text('admin', 'undefined_action')));
                break;
        }
        echo $result;
        exit();
    ...
    private function prepareMarkup( $photo, $layout = null )
    {
        ...
        $photo->url = MT::getStorage()->getFileUrl($this->themeService->getUserfileImagesDir() . $photo->getFilename());
    ...
    {
        $this->themeService->resetImageControl(MT::getThemeManager()->getSelectedTheme()->getDto()->getId(), trim($_GET['name']));
        $this->redirect(MT::getRouter()->urlForRoute('admin_themes_edit'));
    }
    ...
    public function deleteImage( $params )
    {
        ...
        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'theme_graphics_delete_success_message'));
        $this->redirect(MT::getRouter()->urlForRoute('admin_theme_graphics'));
    }
    ...
    public function ajaxDeleteImage( $params )
    {
        ...
        return array(
            'result' => true,
            'msg' => MT::getLanguage()->text('admin', 'theme_graphics_delete_success_message'),
            'imageId' => $imageId
        );
    }
    ...
    private function initFloatbox( $params )
    {
        ...
        $document = MT::getDocument();
        $basePlugin = MT::getPluginManager()->getPlugin('base');
        ...
        $language->addKeyForJs('admin', 'slideshmt_interval');
        ...
        $document->addScriptDeclarationBeforeIncludes(
            UTIL_JsGenerator::composeJsString('
                ;window.photoViewParams = Object.defineProperties({}, {
                    ...
                    'ajaxResponder' => MT::getRouter()->urlFor('ADMIN_CTRL_Theme', 'ajaxResponder'),
...
class UploadGraphicsForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $submit->setValue(MT::getLanguage()->text('admin', 'theme_graphics_upload_form_submit_label'));
...
class AddCssForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $dto = BOL_ThemeService::getInstance()->findThemeByKey(MT::getConfig()->getValue('base', 'selectedTheme'));
        $text->setValue($dto->getCustomCss());
        $this->addElement($text);

        $submit = new Submit('submit');
        $submit->setValue(MT::getLanguage()->text('admin', 'theme_css_edit_submit_label'));
        $this->addElement($submit);

        $this->setAjax(true);
        $this->setAjaxResetOnSuccess(false);
        $this->bindJsFunction(Form::BIND_SUCCESS, 'function(data){MT.info(data.message)}');
    }
}
...
class ThemeEditForm extends Form
{
    ...
    public function __construct( $controls )
    {
        ...
        $submit = new Submit('submit');
        $submit->setValue(MT::getLanguage()->text('admin', 'theme_settings_form_submit_label'));
    ...
    public function renderInput( $params = null )
    {
        ...
        if ( $this->value !== null && ( trim($this->value) !== 'none' ) )
        {
            if ( !strstr($this->value, 'http') )
            {
                ...
                $this->value = 'url(' . MT::getThemeManager()->getSelectedTheme()->getStaticImagesUrl($this->mobile) . substr($resultString, 0, strpos($resultString, ')')) . ')';
            }

            $randId = 'if' . rand(10, 10000000);

            $script = "$('#" . $randId . "').click(function(){
                new MT_FloatBox({\$title:'" . MT::getLanguage()->text('admin', 'themes_settings_graphics_preview_cap_label') . "', \$contents:$('#image_view_" . $this->getName() . "'), width:'550px'});
            });";
            ...
            $output .= '<div class="clearfix"><a id="' . $randId . '" href="javascript://" class="theme_control theme_control_image" style="background-image:' . $this->value . ';"></a>
                <div style="float:left;padding:10px 0 0 10px;"><a href="javascript://" onclick="window.location=\'' . MT::getRequest()->buildUrlQueryString(MT::getRouter()->urlFor('ADMIN_CTRL_Theme', 'resetGraphics'), array('name' => $this->getName())) . '\'">' . MT::getLanguage()->text('admin', 'themes_settings_reset_label') . '</a></div></div>
