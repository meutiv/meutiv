...
class ADMIN_CTRL_Themes extends ADMIN_CTRL_StorageAbstract
{
    ...
    public function init()
    {
        ...
        foreach ( $pageActions as $key => $item )
        {
            ...
            $menuItem->setKey($item)->setLabel($language->text("admin", "themes_menu_item_" . $item))->setOrder($key)->setUrl(MT::getRouter()->urlFor(__CLASS__,
                    $item));
            $menuItems[] = $menuItem;
        }
    ...
    public function chooseTheme()
    {
        ...
        $activeTheme = MT::getThemeManager()->getSelectedTheme()->getDto()->getKey();
        ...
        foreach ( $themes as $theme )
        {
            ...
            if ( $theme->getLicenseCheckTimestamp() > 0 )
            {
                ...
                $themesInfo[$theme->getKey()]["license_url"] = MT::getRequest()->buildUrlQueryString($router->urlFor("ADMIN_CTRL_Storage",
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin("admin")->getStaticJsUrl() . "theme_select.js");
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin("base")->getStaticJsUrl() . "jquery.sticky.js");
        ...
        $event = new MT_Event("admin.filter_themes_to_choose", array(), $themesInfo);
        ...
        if ( MT::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                unlink($tempFile);
                $this->redirect(MT::getRequest()->buildUrlQueryString($router->urlFor(__CLASS__, "processAdd"),
    ...
    public function processAdd()
    {
        ...
        $remoteDir = MT_DIR_THEME . basename($localThemeRootPath);

        if ( file_exists($remoteDir) )
        {
            $this->feedback->error(MT::getLanguage()->text("admin", "theme_add_duplicated_dir_error",
                    array("dir" => $remoteDir)));
            $this->redirect($router->urlForRoute("admin_themes_choose"));
        }
    ...
    public function changeTheme( array $params )
    {
        ...
        $backUrl = MT::getRouter()->urlForRoute("admin_themes_choose");
        $language = MT::getLanguage();
                ...
                $themeDto->getDeveloperKey(), $themeDto->getLicenseKey())) )
        {
            if ( !isset($_GET[BOL_StorageService::URI_VAR_LICENSE_CHECK_COMPLETE]) )
            {
                $requestParams = array(
                    ...
                    BOL_StorageService::URI_VAR_ITEM_TYPE => BOL_StorageService::URI_VAR_ITEM_TYPE_VAL_THEME,
                    BOL_StorageService::URI_VAR_BACK_URI => urlencode(MT::getRequest()->getRequestUri()),
                    "back-button-uri" => urlencode(MT::getRouter()->uriForRoute("admin_themes_choose"))                );
        ...
        MT::getConfig()->saveConfig("base", "selectedTheme", $themeDto->getKey());
        MT::getEventManager()->trigger(new MT_Event("base.change_theme",
            array(/* name for back. compat. */"name" => $themeDto->getKey(), "key" => $themeDto->getKey())));
        $this->feedback->info(MT::getLanguage()->text("admin", "theme_change_success_message"));
    ...
    public function update( array $params )
    {
        ...
        $remoteDir = MT_DIR_THEME . $themeDto->getKey();
        ...
        $params = array("theme" => $themeDto->getKey(), "back-uri" => urlencode(MT::getRequest()->getRequestUri()));
    ...
    public function deleteTheme( $params )
    {
        ...
        if ( MT::getThemeManager()->getDefaultTheme()->getDto()->getKey() == $themeDto->getKey() )
        {
            $this->feedback->error($language->text("admin", "themes_cant_delete_default_theme"));
            $this->redirect(MT::getRouter()->urlForRoute("admin_themes_choose"));
        }

        if ( MT::getThemeManager()->getCurrentTheme()->getDto()->getKey() == $themeDto->getKey() )
        {
    ...
    private function getThemeDtoByKeyInParamsArray( $params )
    {
        ...
        $this->feedback->error(MT::getLanguage()->text("admin", "manage_themes_theme_not_found"));
        $this->redirect(MT::getRouter()->urlForRoute("admin_themes_choose"));
    }
}
