...
class ADMIN_CTRL_Storage extends ADMIN_CTRL_StorageAbstract
{
    ...
    public function checkItemLicense()
    {
        ...
        if ( isset($params["back-button-uri"]) )
        {
            ...
            $redirectUrl = UTIL_HtmlTag::escapeJs(MT_URL_HOME . urldecode($params["back-button-uri"]));
        ...
        if ( MT::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                if ( $result )
                {
                    ...
                    MT::getFeedback()->info($language->text("admin", "plugins_manage_license_key_check_success"));
                    $this->redirectToBackUri($params);
                    $this->redirect();
                }
                else
                {
                    MT::getFeedback()->error($language->text('admin', 'plugins_manage_invalid_license_key_error_message'));
                    $this->redirect();
                }
            }
        }
    ...
    {
        if ( !(bool) MT::getConfig()->getValue("base", "update_soft") )
        {
...
//TODO check if the result is false | null
        $params = array(
            "oldVersion" => MT::getConfig()->getValue("base", "soft_version"),
            "newVersion" => $newPlatformInfo["version"],
            "info" => $newPlatformInfo["info"]
        );
        $this->assign("text", MT::getLanguage()->text("admin", "manage_plugins_core_update_request_text", $params));
        $this->assign("redirectUrl", MT::getRouter()->urlFor(__CLASS__, "platformUpdate"));
        $this->assign("returnUrl", MT::getRouter()->urlForRoute("admin_default"));
        ...
        {
            $this->assign("phpVersionInvalidText", MT::getLanguage()->text("admin", "plugin_update_platform_invalid_php_version_msg",
    ...
    public function platformUpdate()
    {
        ...
        $language = MT::getLanguage();
        $tempDir = MT_DIR_PLUGINFILES . "ow" . DS . "core" . DS;

        $ftp = $this->getFtpConnection();

        $errorMessage = false;

        MT::getApplication()->setMaintenanceMode(true);
        ...
        else
        {
            ...
            if ( $zopen === true && file_exists($tempDir) )
            {
                ...
                $zip->close();
                $ftp->uploadDir($tempDir, MT_DIR_ROOT);
                $ftp->chmod(0777, MT_DIR_STATIC);
                $ftp->chmod(0777, MT_DIR_STATIC_PLUGIN);
            }
            else
            {
        ...
        {
            MT::getApplication()->setMaintenanceMode(false);
            MT::getFeedback()->error($errorMessage);
            $this->redirect(MT::getRouter()->urlFor("ADMIN_CTRL_Index", "index"));
        }
    ...
    public function checkUpdates()
    {
        ...
        {
            MT::getFeedback()->info(MT::getLanguage()->text("admin", "check_updates_success_message"));
        }
        else
        {
            MT::getFeedback()->error(MT::getLanguage()->text("admin", "check_updates_fail_error_message"));
        }

        $backUrl = MT::getRouter()->urlForRoute("admin_default");

        if ( isset($_GET[BOL_StorageService::URI_VAR_BACK_URI]) )
        {
            $backUrl = MT_URL_HOME . urldecode($_GET[BOL_StorageService::URI_VAR_BACK_URI]);
        }
    ...
    public function ftpAttrs()
    {
        ...
        if ( MT::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                MT::getSession()->set("ftpAttrs", $ftpAttrs);
