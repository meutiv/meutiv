...
class ADMIN_CTRL_Pages extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->setPageHeading(MT::getLanguage()->text('admin', 'pages_page_heading'));
        $this->setPageHeadingIconClass('mt_ic_files');
    }

    public function index( $params )
    {
        MT::getDocument()->getMasterPage()->getMenu(MT_Navigation::ADMIN_PAGES)->getElement('sidebar_menu_item_pages_manage')->setActive(true);
        ...
        if ( MT::getRequest()->isPost() && $form->isValid($_POST) )
        {
            $form->process($params);
        }
    }
    ...
    public function maintenance()
    {
        ...
        if ( MT::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                MT::getConfig()->saveConfig('base', 'maintenance', (bool) $data['maintenance_enable']);
                ...
                MT::getConfig()->saveConfig('base', 'splash_leave_url', $url);
                MT::getConfig()->saveConfig('base', 'splash_screen', (bool) $data['splash_screen']);

                MT::getFeedback()->info($language->text('admin', 'maintenance_submit_success_message'));
                $this->redirect();
            }
        }

        $form->getElement('maintenance_text')->setValue($language->text('admin', 'maintenance_text_value'));
        $form->getElement('maintenance_enable')->setValue((bool) MT::getConfig()->getValue('base', 'maintenance'));
        ...
        $form->getElement('leave_url')->setValue(MT::getConfig()->getValue('base', 'splash_leave_url'));
        $form->getElement('splash_screen')->setValue((bool) MT::getConfig()->getValue('base', 'splash_screen'));
    }
...
class SaveForm extends Form
{
    ...
    public function __construct( MT_Renderable $rendrable )
    {
        ...
        $titleTextField->setLabel(MT::getLanguage()->text('admin', 'pages_edit_local_page_title'))
                ->addAttribute('class', 'mt_text');
        ...
        $nameTextField->setLabel(MT::getLanguage()->text('admin', 'pages_add_menu_name'))
                ->setRequired(true)
                ->addAttribute('class', 'mt_text');
        ...
        $opts = array(
            '1' => MT::getLanguage()->text('admin', 'pages_edit_visible_for_guests'),
            '2' => MT::getLanguage()->text('admin', 'pages_edit_visible_for_members')
        );

        $visibleForCheckboxGroup->setOptions($opts);
        $visibleForCheckboxGroup->setLabel(MT::getLanguage()->text('admin', 'pages_edit_local_visible_for'));
...
//                ->setId('meta-tags')
//                ->setDescription(MT::getLanguage()->text('admin', 'pages_page_field_meta_desc'));
        ...
        $contentTextArea->setLabel(MT::getLanguage()->text('admin', 'pages_add_page_content'))
                ->setId('content')
                ->setDescription(
                        MT::getLanguage()->text('admin', 'pages_page_field_content_desc', array(
                            'src' => MT::getThemeManager()->getCurrentTheme()->getStaticImagesUrl() . 'question.png',
                            'url' => '#'
                                )
                        )
        );
        ...
        $type = (MT::getRequest()->isPost() && $_POST['type']) ? $_POST['type'] : 'local';
        ...
        $typeHiddenField->setLabel(MT::getLanguage()->text('admin', 'page_add_page_address'));
        ...
        $extOpenInNewWindow->setLabel(MT::getLanguage()->text('admin', 'pages_edit_external_url_open_in_new_window'));

        $this->addElement($extOpenInNewWindow);

        $submit = new Submit('submit');

        $this->addElement($submit->setValue(MT::getLanguage()->text('base', 'pages_add_submit')));
    }
    ...
    public function process( $params )
    {
        ...
        switch ( $_POST['type'] )
        {
            case 'local' :
                ...
                $url = str_replace(UTIL_String::removeFirstAndLastSlashes(MT::getRouter()->getBaseUrl()), '', UTIL_String::removeFirstAndLastSlashes($_POST['local-url']));
...
//~
        ...
        header('location: ' . MT::getRouter()->urlForRoute('admin_pages_main'));
        exit();
    }

}

class PageTitleValidator extends MT_Validator
{

    public function __construct()
    {
        $this->setErrorMessage(MT::getLanguage()->text('base', 'form_validator_required_error_message'));
    }
...
class LocalPageUniqueValidator extends MT_Validator
{

    public function __construct()
    {
        $this->setErrorMessage(MT::getLanguage()->text('base', 'unique_local_page_error'));
    }
...
class LocalPageUrlValidator extends MT_Validator
{
    ...
    public function isValid( $value )
    {
        ...
        $value = str_replace(UTIL_String::removeFirstAndLastSlashes(MT::getRouter()->getBaseUrl()), '', UTIL_String::removeFirstAndLastSlashes($value));
    ...
    public function getError()
    {
        if ( $this->errCode == 2 )
        {
            return MT::getLanguage()->text('base', 'pages_wrong_local_url');
        }
        ...
        return "{
        	validate : function( value ){
                if( $('#address2').attr('checked') &&  !value ){
                	throw " . json_encode(MT::getLanguage()->text('base', 'form_validator_required_error_message')) . ";
    			}

                return true;
        },
        	getErrorMessage : function(){ return " . json_encode(MT::getLanguage()->text('base', 'form_validator_required_error_message')) . " }
        }";
    }
