...
class ADMIN_CTRL_AjaxUpload extends ADMIN_CTRL_Abstract
{
    ...
    protected function getEntity( $params )
    {
        if ( empty($params["entityType"]) || empty($params["entityId"]) )
        {
            ...
            $params["entityId"] = MT::getUser()->getId();
        }
        ...
        if ( !empty($file['file']['error']) )
        {
            switch ( $file['file']['error'] )
            {
                case UPLOAD_ERR_INI_SIZE:
                    return MT::getLanguage()->text('admin', 'error_ini_size');
                case UPLOAD_ERR_FORM_SIZE:
                    return MT::getLanguage()->text('admin', 'error_form_size');
                case UPLOAD_ERR_PARTIAL:
                    return MT::getLanguage()->text('admin', 'error_partial');
                case UPLOAD_ERR_NO_FILE:
                    return MT::getLanguage()->text('admin', 'error_no_file');
                case UPLOAD_ERR_NO_TMP_DIR:
                    return MT::getLanguage()->text('admin', 'error_no_tmp_dir');
                case UPLOAD_ERR_CANT_WRITE:
                    return MT::getLanguage()->text('admin', 'error_cant_write');
                case UPLOAD_ERR_EXTENSION:
                    return MT::getLanguage()->text('admin', 'error_extension');
                default:
                    return MT::getLanguage()->text('admin', 'no_photo_uploaded');
            }
        }
        else
        {
    ...
    public function ajaxSubmitPhotos( $params )
    {
        ...
        {
            $resp = array('result' => false, 'msg' => MT::getLanguage()->text('admin', 'photo_upload_error'));
        ...
        if ( !$form->isValid($_POST) )
        {
            ...
            $resp['msg'] = MT::getLanguage()->text('admin', 'photo_upload_error');
            $this->returnResponse($resp);
        }
    ...
    protected function onSubmitComplete( $entityType, $entityId, $files )
    {
        ...
        if ( empty($files) )
        {
            $result['url'] = MT::getRouter()->urlForRoute('admin_theme_graphics');
        ...
        $result['url'] = MT::getRouter()->urlForRoute('admin_theme_graphics');
        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'photos_uploaded', array('count' => $fileCount)));
    ...
    public function upload()
    {
        if ( $this->isAvailableFile($_FILES) )
        {
            ...
            if ( ($id = BOL_FileTemporaryService::getInstance()->addTemporaryFile($_FILES['file']['tmp_name'], $_FILES['file']['name'], MT::getUser()->getId(), $order)) )
            {
            ...
            {
                $this->returnResponse(array('status' => self::STATUS_ERROR, 'msg' => MT::getLanguage()->text('admin', 'no_photo_uploaded')));
            }
        }
        else
        {
