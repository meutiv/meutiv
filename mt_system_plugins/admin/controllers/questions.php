...
class ADMIN_CTRL_Questions extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->ajaxResponderUrl = MT::getRouter()->urlFor("ADMIN_CTRL_Questions", "ajaxResponder");

        $language = MT::getLanguage();

        $this->setPageHeading($language->text('admin', 'heading_questions'));
        $this->setPageHeadingIconClass('mt_ic_files');

        MT::getNavigation()->activateMenuItem('admin_users', 'admin', 'sidebar_menu_item_questions');
    }
    ...
    public function pages( $params = array() )
    {
        ...
        $language = MT::getLanguage();
        $currentLanguageId = MT::getLanguage()->getCurrentId();
        ...
        {
            $sectionDeleteUrlList[$section] = MT::getRouter()->urlFor('ADMIN_CTRL_Questions', 'deleteSection', array("sectionName" => $section));
            ...
            foreach ( $list as $question )
            {
                ...
                $event = new MT_Event('admin.questions.get_edit_delete_question_buttons_content', array( 'question' => $question ), null);
                ...
                $event = new MT_Event('admin.questions.get_preview_question_values_content', array( 'question' => $question ), null);
                MT::getEventManager()->trigger($event);
                ...
                $event = new MT_Event('admin.questions.get_question_page_checkbox_content', array( 'actionList' => $pageCheckboxData, 'question' => $question ), $pageCheckboxData);
                MT::getEventManager()->trigger($event);
        ...
        foreach ( $questionList as $sort => $question )
        {
            ...
            MT::getDocument()->addOnloadScript("MT.registerLanguageKey('admin', 'questions_delete_question_confirmation_" . (int) $question['id'] . "', {$text});");
        ...
        $this->assign('accountTypesUrl', MT::getRouter()->urlForRoute('questions_account_types'));
        ...
        $script = ' window.indexQuest = new indexQuestions( ' . json_encode(array('questions' => $questionList, 'questionAddUrl' => MT::getRouter()->urlFor("ADMIN_CTRL_Questions", "add"), 'ajaxResponderUrl' => $this->ajaxResponderUrl)) . ' )'; //' . json_encode( array( 'questionEditUrl' => $questionEditUrl ) ) . ' ); ';

        MT::getDocument()->addOnloadScript($script);

        $jsDir = MT::getPluginManager()->getPlugin("admin")->getStaticJsUrl();

        MT::getDocument()->addScript($jsDir . "questions.js");

        $baseJsDir = MT::getPluginManager()->getPlugin("base")->getStaticJsUrl();
        MT::getDocument()->addScript($baseJsDir . "jquery-ui.min.js");

        MT::getLanguage()->addKeyForJs('admin', 'questions_profile_question_sections_title');

        MT::getLanguage()->addKeyForJs('admin', 'questions_edit_profile_question_title');
        MT::getLanguage()->addKeyForJs('admin', 'questions_add_profile_question_title');
        MT::getLanguage()->addKeyForJs('admin', 'questions_values_should_not_be_empty');
    }
    ...
    public function accountTypes( $params = array() )
    {
        ...
        foreach ( $questions as $section => $list )
        {
            ...
            foreach ( $list as $question )
            {
                ...
                //$question['questionEditUrl'] = MT::getRouter()->urlFor('ADMIN_CTRL_Questions', 'edit', array("questionId" => $question['id']));
                //$question['questionDeleteUrl'] = MT::getRouter()->urlFor('ADMIN_CTRL_Questions', 'deleteQuestion', array("questionId" => $question['id']));
                ...
                $event = new MT_Event('admin.questions.get_account_types_checkbox_content', array( 'question' => $question ), null);
                MT::getEventManager()->trigger($event);
        ...
        $this->assign('propertiesUrl', MT::getRouter()->urlForRoute('questions_properties'));
        $this->assign('deleteEditButtons', $deleteEditButtonsContent);
        $this->assign('previewValues', $previewQuestionValuesContent);
        ...
        MT::getLanguage()->addKeyForJs('admin', 'questions_add_account_type_title');
        MT::getLanguage()->addKeyForJs('admin', 'questions_edit_account_type_title');

        MT::getLanguage()->addKeyForJs('admin', 'questions_account_type_was_added');
        MT::getLanguage()->addKeyForJs('admin', 'questions_account_type_was_updated');
        MT::getLanguage()->addKeyForJs('admin', 'questions_account_type_added_error');

        MT::getLanguage()->addKeyForJs('admin', 'questions_delete_account_type_confirmation');
        ...
        $contextParentAction->setClass('mt_newsfeed_context');
        $contextAction->addAction($contextParentAction);

        $editAction = new BASE_ContextAction();
        $editAction->setKey('edit');
        $editAction->setLabel(MT::getLanguage()->text('admin', 'btn_label_edit'));
        ...
        $deleteAction->setLabel(MT::getLanguage()->text('admin', 'btn_label_delete'));
    ...
    private function addContentMenu()
    {
        ...
        $menuItem->setIconClass('mt_ic_files');
    ...
    {
        if ( !MT::getAuthorization()->isUserAuthorized(MT::getUser()->getId(), 'admin') || empty($_POST["command"]) || !MT::getRequest()->isAjax() )
        {
        ...
        switch ( $command )
        {
            case 'deleteQuestion':
                ...
                {
                    echo json_encode(array('result' => "success", 'message' => MT::getLanguage()->text('admin', 'questions_question_was_deleted'), 'deleteList' => $deleteQuestionNameList));
                    exit;
                }
            ...
            case 'findNearestSection':
                ...
                if ( !empty($sectionName) )
                {
                    ...
                    echo json_encode( array(
                        'result' => "success", 
                        'message' => MT::getLanguage()->text('admin', 'questions_delete_section_confirmation_with_move_questions' , array('sectionName' => BOL_QuestionService::getInstance()->getSectionLang($nearSection->name) ))
                    ) );
                    exit;
                }
            ...
            case 'deleteSection':
                ...
                if ( !empty($_POST['sectionName']) && mb_strlen($_POST['sectionName']) > 0 )
                {
                    ...
                    {
                        $result = array('result' => "success", 'message' => MT::getLanguage()->text('admin', 'questions_section_was_deleted'));
            ...
            case 'deleteAccountType':
                ...
                if ( !empty($_POST['accountType']) && mb_strlen($_POST['accountType']) > 0 )
                {
                    ...
                    {
                        echo json_encode(array('result' => "false", 'message' => MT::getLanguage()->text('admin', 'questions_cant_delete_last_account_type')));
                        exit;
                    }
                    else if ( $this->questionService->deleteAccountType($_POST['accountType']) )
                    {
                        echo json_encode(array('result' => "success", 'message' => MT::getLanguage()->text('admin', 'questions_account_type_was_deleted')));
                        exit;
                    }
                }
           ...
           case 'AddAccountType':
            ...
            case 'addSection' :
                ...
                BOL_LanguageService::getInstance()->addOrUpdateValue(MT::getLanguage()->getCurrentId(), 'base', 'questions_section_' . ( $questionSection->name ) . '_label', htmlspecialchars($sectionName));

                if ( MT::getDbo()->getAffectedRows() > 0 )
                {
                    echo json_encode(array('result' => true, 'message' => MT::getLanguage()->text('admin', 'questions_section_was_added')));
                }
            ...
            case 'addQuestion' :
                ...
                $form = MT::getClassInstance('ADMIN_CLASS_AddQuestionForm', 'qst_add_form', '');
                $form->process();
            ...
            case 'editQuestion' :
                ...
                {
                    echo json_encode(array('result' => false, 'errors' => array(), 'message' => MT::getLanguage()->text('admin', 'questions_not_found')));
                    exit;
                }
                ...
                $form = MT::getClassInstance('ADMIN_CLASS_EditQuestionForm', 'qst_edit_form', '');
