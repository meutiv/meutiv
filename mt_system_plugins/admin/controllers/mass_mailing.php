...
class ADMIN_CTRL_MassMailing extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->ajaxResponderUrl = MT::getRouter()->urlFor("ADMIN_CTRL_MassMailing", "ajaxResponder");
    ...
    public function index( $params = array() )
    {
        ...
        $subject->addAttribute('class', 'mt_text');
        ...
        $body->addAttribute('class', 'mt_text');
        ...
        {
            $massMailingTimestamp = MT::getConfig()->getValue( 'admin', 'mass_mailing_timestamp' );
        ...
        if ( MT::getRequest()->isPost() && $isActive && isset($_POST['startMailing']) )
        {
            if ( $massMailingForm->isValid($_POST) )
            {
                ...
                while ( $start < $total )
                {
                    ...
                    foreach ( $result as $key => $user )
                    {
                        ...
                        $vars['unsubscribe_url'] = MT::getRouter()->urlForRoute('base_massmailing_unsubscribe', array('id' => $user->id, 'code' => $code)) . '?desktopRedirect=1';
                    ...
                    $start += $count;
                    //printVar($mails);
                    MT::getMailer()->addListToQueue($mails);
                }

                MT::getFeedback()->info($language->text('admin', 'massmailing_send_mails_message', array('count' => $mailCount)));

                if ( defined( 'MT_PLUGIN_XP' ) )
                {
                    MT::getConfig()->saveConfig( 'admin', 'mass_mailing_timestamp', time() );
                }
        ...
        MT::getDocument()->addScript($jsDir . "mass_mailing.js");
    }
