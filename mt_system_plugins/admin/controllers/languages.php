...
class ADMIN_CTRL_Languages extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        MT::getDocument()->getMasterPage()->getMenu(MT_Navigation::ADMIN_SETTINGS)->getElement('sidebar_menu_item_settings_language')->setActive(true);

        $this->setPageHeading(MT::getLanguage()->text('admin', 'languages_page_heading'));
        $this->setPageHeadingIconClass('mt_ic_edit');
    ...
    public function getMenu()
    {
        ...
        $item->setLabel(MT::getLanguage()->text('admin', 'edit_language'));
        $item->setIconClass('mt_ic_edit');
        $item->setKey('edit_lang');

        if ( $this->isDevMode() )
        {
            $item->setUrl(MT::getRouter()->urlForRoute('admin_developer_tools_language'));
        }
        else
        {
            $item->setUrl(MT::getRouter()->urlForRoute('admin_settings_language'));
        }
        ...
        $item = new BASE_MenuItem();
        $item->setLabel(MT::getLanguage()->text('admin', 'available_languages'));
        $item->setIconClass('mt_ic_files');
        $item->setKey('avail_lang');

        if ( $this->isDevMode() )
        {
            $item->setUrl(MT::getRouter()->urlForRoute('admin_developer_tools_language_mod'));
        }
        else
        {
            $item->setUrl(MT::getRouter()->urlForRoute('admin_settings_language_mod'));
        }
    ...
    public function import()
    {
        ...
        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'language_import_complete_success_message'));
    ...
    public function index()
    {
        ...
        $this->assign('languageSwitchUrl', MT::getRequest()->buildUrlQueryString(null, array('language' => null)));

        $this->assign('lang_switch_url', MT::getRequest()->buildUrlQueryString(null, array('langId' => null, 'page' => null)));

        $this->assign('section_switch_url', MT::getRequest()->buildUrlQueryString(null, array('prefix' => null, 'page' => null)));

        $this->assign('searchFormActionUrl', MT::getRequest()->buildUrlQueryString(null,
        ...
        if ( isset($_POST['command']) && $_POST['command'] == 'edit-values' )
        {
            ...
            MT::getFeedback()->info(MT::getLanguage()->text('admin', 'languages_values_updated'));
            $this->redirect();
        }
            ...
            if ( trim($_POST['form_name']) === 'lang_edit' && !empty($_POST['langId']) && !empty($_POST['label']) && !empty($_POST['tag']) )
            {
                ...
                if ( $language !== null )
                {
                    if ( $_POST['tag'] != $language->tag )
                    {
                        ...
                        {
                            exit(json_encode(array('result' => false, 'message' => MT::getLanguage()->text('admin', 'msg_lang_invalid_language_tag'))));
                        }
                    }
                    ...
                    $event = new MT_Event('admin.before_save_lang_value', array('dto'=>$language));
                    MT::getEventManager()->trigger($event);

                    $this->service->save($language);

                    exit(json_encode(array('result' => true, 'message' => MT::getLanguage()->text('admin', 'language_edit_form_success_message'))));
                }
            }
        }

        exit(json_encode(array('result' => false, 'message' => MT::getLanguage()->text('admin', 'language_edit_form_error_message'))));
    }
    ...
    public function mod()
    {
        ...
        if ( isset($_POST['command']) && $_POST['command'] == 'upload-lp' )
        {
            ...
            {
                MT::getFeedback()->error(MT::getLanguage()->text('admin', 'add_language_pack_empty_file_error_message'));
                $this->redirect();
            }
        ...
        $event = new MT_Event('admin.get_additional_langs', array('langs' => $langsEventParam));
        MT::getEventManager()->trigger($event);
        ...
        $importLangForm->setAction(MT::getRouter()->urlForRoute('admin_settings_language_mod'). "/#lang_import");
    ...
    public function activate()
    {
        ...
        $url = MT::getRouter()->urlForRoute('admin_settings_language_mod');

        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'language_activated'));
    ...
    public function deactivate()
    {
        ...
        if ( $languageService->countActiveLanguages() == 1 )
        {
            ...
            $url = MT::getRouter()->urlFor('ADMIN_CTRL_Languages', 'mod');
            header("location: {$url}#lang_list");
            exit();
        }
        ...
        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'language_deactivated'));
    ...
    public function delete()
    {
        ...
        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'language_deleted'));
    ...
    public function ajaxAddKey()
    {
        ...
        if ( MT::getRequest()->isPost() && $addKeyForm->isValid($_POST) )
        {
            $data = $addKeyForm->getValues();

            if ( !$this->isDevMode() && !strstr($_SERVER['HTTP_REFERER'], 'dev-tools') )
            {
                $prefixId = $languageService->findPrefixId('mt_custom');
                ...
                while ( !$languageService->isKeyUnique('mt_custom', $unique) )
                {
                    $i++;
                    $unique = $key . $i;
                }
    ...
    public function ajaxClone()
    {
        ...
        {
            $errorMessage = MT::getLanguage()->text('admin', 'msg_lang_clone_failed');
            ...
            MT::getFeedback()->error($errorMessage);
        ...
        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'msg_lang_cloned'));
    ...
    public function ajaxOrder()
    {
        ...
        MT::getSession()->delete(BOL_LanguageService::LANG_ID_VAR_NAME);
    ...
    public function ajaxEditLanguageValuesForm()
    {
        ...
        json_encode(
            array(
                'markup' => $cmp->render(),
                'js' => MT::getDocument()->getOnloadScript(),
                'include_js' => MT::getDocument()->getScripts()
            )
        )
        );
    }
    ...
    {
        return $isDevMode = true && strstr(MT::getRequest()->getRequestUri(), 'dev-tools'); // todo: 8aa
    }
}
...
class ImportLangForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $fileField->setLabel(MT::getLanguage()->text('admin', 'lang_file'));

        $this->addElement($fileField);

        $submit = new Submit('submit');

        $this->addElement($submit->setValue(MT::getLanguage()->text('admin', 'clone_form_lbl_submit')));
    }
}
...
class AddKeyForm extends Form
{
    ...
    function __construct( $prefixes, $language, $isDevMode = false )
    {
        ...
        $this->setAction(MT::getRouter()->urlFor('ADMIN_CTRL_Languages', 'ajaxAddKey'));
        ...
        $keyTextField->setLabel(MT::getLanguage()->text('admin', 'add_key_form_lbl_key'));
        ...
        $prefixSelectBox->setOptions($options)->setLabel(MT::getLanguage()->text('admin', 'section'));
        ...
        $this->addElement(
            $valueTextArea->setRequired(true)->setLabel(MT::getLanguage()->text('admin', 'add_key_form_lbl_val', array('label' => $language->getLabel(), 'tag' => $language->getTag()))));

        $submit = new Submit('submit');

        $submit->setValue(MT::getLanguage()->text('admin', 'add_key_form_lbl_add'));
        ...
        if ( !MT::getRequest()->isAjax() )
        {
            MT::getDocument()->addOnloadScript(
				...
				switch( json['result'] ){
					...
					case 'dublicate':
						MT.info('" . MT::getLanguage()->text('admin', 'msg_dublicate_key') . "');
						break;
				}
			});");
        }
...
class CloneForm extends Form
{
    ...
    function __construct()
    {
        ...
        $this->setAction(MT::getRouter()->urlFor('ADMIN_CTRL_Languages', 'ajaxClone'));

        $this->setMethod('post');
        $labelTextField = new TextField('label');

        $labelTextField->setLabel(MT::getLanguage()->text('admin', 'clone_form_lbl_label'))->setDescription(MT::getLanguage()->text('admin', 'clone_form_descr_label'));

        $this->addElement($labelTextField);

        $tagTextField = new TextField('tag');
        $tagTextField->addValidator(new LanguageTagValidator());
        $tagTextField->setLabel(MT::getLanguage()->text('admin', 'clone_form_lbl_tag'))->setDescription(MT::getLanguage()->text('admin', 'clone_form_descr_tag'));
        ...
        $submit = new Submit('submit');
        $submit->setValue(MT::getLanguage()->text('admin', 'clone_form_lbl_submit'));
        ...
        if ( !MT::getRequest()->isAjax() )
            MT::getDocument()->addOnloadScript("owForms['{$this->getName()}'].bind('success', function(){location.reload();});");
    }
}
...
class LanguageTagValidator extends RequiredValidator
{
    public function __construct()
    {
        $this->setErrorMessage(MT::getLanguage()->text('admin', 'msg_lang_invalid_language_tag'));
    }
