...
class ADMIN_CTRL_Permissions extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->setPageHeading(MT::getLanguage()->text('admin', 'permissions_page_heading'));
        $this->setPageHeadingIconClass('mt_ic_lock');
    }
        ...
        if ( MT::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                else
                {
                    ...
                    $mail->setSubject(MT::getLanguage()->text( 'admin', 'site_password'));
                    $mail->setTextContent( MT::getLanguage()->text( 'admin', 'admin_password', array('password' => $data['password'])));
                    try
                    {
                ...
                MT::getFeedback()->info($language->text('admin', 'permission_global_privacy_settings_success_message'));
                $this->redirect();
            }
        }
    ...
    public function roles()
    {
        ...
        $this->assign('formAction', MT::getRouter()->urlFor(__CLASS__, 'savePermissions'));
    ...
    {
        $service = BOL_AuthorizationService::getInstance();
        $this->assign('formAction', MT::getRouter()->urlFor(__CLASS__, 'process'));
        ...
        foreach ( $moderators as $moderator )
        {
            ...
            $deleteModerUrls[$moderator->userId] = MT::getRouter()->urlFor(__CLASS__, 'deleteModerator', array('id' => $moderator->id));
        }
        ...
        {
            if ( !MT::getPluginManager()->isPluginActive($group->name) )
            {
                unset($groups[$key]);
            }
        }
        ...
        $this->assign('myModeratorId', $service->getModeratorIdByUserId(MT::getUser()->getId()));
        ...
        $this->assign('addFormAction', MT::getRouter()->urlFor(__CLASS__, 'addModerator'));
        ...
        $this->setPageHeading(MT::getLanguage()->text('admin', 'sidebar_menu_item_permission_moders'));
    }

    public function process()
    {
        if ( MT::getRequest()->isPost() && !empty($_POST['perm']) )
        {
            $perms = array();
            ...
            BOL_AuthorizationService::getInstance()->saveModeratorPermissionList($perms, MT::getUser()->getId());
        }

        MT::getFeedback()->info(MT::getLanguage()->text('admin', 'permissions_successfully_updated'));
        $this->redirect(MT::getRouter()->urlForRoute('admin_permissions_moderators'));
    }
    ...
    public function addModerator()
    {
        if ( MT::getRequest()->isPost() )
        {
            ...
            if ( $user === null )
            {
                MT::getFeedback()->error(MT::getLanguage()->text('admin', 'permissions_feedback_user_not_found'));
            }
            else
            {
                if ( BOL_AuthorizationService::getInstance()->addModerator($user->id) )
                {
                    MT::getFeedback()->info(MT::getLanguage()->text('admin', 'permissions_feedback_moderator_added'));
                }
                else
                {
                    MT::getFeedback()->warning(MT::getLanguage()->text('admin', 'permissions_feedback_user_is_already_moderator', array('username' => $username)));
                }
            }
        }
    ...
    public function deleteModerator( array $params )
    {
        ...
        if ( isset($params['id']) )
        {
            ...
            if ( $removed )
            {
                MT::getFeedback()->info(MT::getLanguage()->text('admin', 'permissions_feedback_user_kicked_from_moders'));
            }
            else
            {
                MT::getFeedback()->error(MT::getLanguage()->text('admin', 'permissions_feedback_cant_remove_moder'));
            }
        }
        else
        {
    ...
    public function savePermissions()
    {
        ...
        $this->redirect(MT::getRouter()->urlForRoute('admin_user_roles'));
    }
}