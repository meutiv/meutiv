...
class ADMIN_CMP_EditQuestion extends MT_Component
{
    ...
    public function __construct( $questionId )
    {
        ...
        $responderUrl = MT::getRouter()->urlFor("ADMIN_CTRL_Questions", "ajaxResponder");
        
        /* @var $addForm ADMIN_CLASS_AddQuestionForm */
        $addForm = MT::getClassInstance('ADMIN_CLASS_EditQuestionForm', 'qst_edit_form', $responderUrl);
        ...
        MT.bind( "question.value.delete", function( params ) {
        ...
        MT.bind( "question.value.add", function( params ) {
                var questionId = params.node.parents(\'form[name=qst_edit_form]:eq(0)\').find(\'input[name=questionId]\').val();
        ...
        $("form[name=qst_edit_form] a.question_label").on( "click",  function(){
            MT.ajaxFloatBox("BASE_CMP_LanguageValueEdit", [\'base\', '.json_encode($nameLang).', true], { title: '.json_encode(MT::getLanguage()->text('base', 'questions_edit_question_label_title')).' } );
        });

        $("form[name=qst_edit_form] a.question_description").on( "click", function(){
            MT.ajaxFloatBox("BASE_CMP_LanguageValueEdit", [\'base\', '.json_encode($descriptionLang).', true], { title: '.json_encode(MT::getLanguage()->text('base', 'questions_edit_description_label_title')).' } );
        });
        ...
        $("form[name=qst_edit_form]").on( "dblclick", ".values_list .tag .label", function(){
            ...
            MT.ajaxFloatBox("BASE_CMP_LanguageValueEdit", [\'base\', '.json_encode($valueLang).'+value, true], { title: '.json_encode(MT::getLanguage()->text('base', 'questions_edit_question_value_title')).' } );
        } );

        MT.bind( "admin.language_key_edit_success", function( params ) {
        ...
        $noValue = MT::getLanguage()->text('admin', 'questions_empty_lang_value');
