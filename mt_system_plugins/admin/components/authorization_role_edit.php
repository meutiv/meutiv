 ...
 * @since 1.0
 */
class ADMIN_CMP_AuthorizationRoleEdit extends MT_Component
{
    /**
     * @param int $roleId
     */
    public function __construct( $roleId )
    {
        ...
        $js = '$("input[name=displayLabel]", "#role-edit-cont").change(function(){
            if ( $(this).attr("checked") )
            {
                ...
                $(".mt_avatar_label", "#role-edit-cont").css("display", "inline-block");
            }
            else
            {
                $("#color-select-cont").css("display", "none");
                $(".mt_avatar_label", "#role-edit-cont").css("display", "none");
            }
        });
        $(".color_sample", "#role-edit-cont").click(function(){
            var color = $(this).css("background-color");
            $("#label-color-field").val(color);
            $(".mt_avatar_label", "#role-edit-cont").css("background-color", color);
        });';
        
        if ( !$role->displayLabel )
        {
            $js .= '$(".mt_avatar_label", "#role-edit-cont").css("display", "none");';
        }
        if ( !empty($role->custom) )
        {
            $js .= '$(".mt_avatar_label", "#role-edit-cont").css("background-color", "'.$role->custom.'");';
        }
    ...
    public static function process( $data )
    {
        ...
        $resp['message'] = MT::getLanguage()->text('admin', 'permissions_role_updated');
        echo json_encode($resp);
        exit;
    }
}
...
class EditRoleForm extends Form
{
    public function __construct( BOL_AuthorizationRole $role )
    {
        ...
        $this->setAjax(true);
        $this->setAction(MT::getRouter()->urlFor('ADMIN_CTRL_Users', 'ajaxEditRole'));
