 ...
 * @package ow.mt_system_plugins.admin.components
 * @since 1.0
 */
class ADMIN_CMP_UserList extends MT_Component
{
    /**
     * Constructor.
     *
    ...
    public function __construct( ADMIN_UserListParams $params )
    {
        ...
        // handle form
        if ( MT::getRequest()->isPost() && !empty($_POST['users']) )
        {
            $users = $_POST['users'];
            ...
            if ( isset($_POST['suspend']) )
            {
                ...
                MT::getFeedback()->info($language->text('admin', 'user_feedback_profiles_suspended'));
            }
            else if ( isset($_POST['reactivate']) )
            {
                ...
                MT::getFeedback()->info($language->text('admin', 'user_feedback_profiles_unsuspended'));                
            }
            else if ( isset($_POST['delete']) )
            {
                ...
                MT::getFeedback()->info($language->text('admin', 'user_delete_msg', array('count' => $deleted)));
            }
            else if ( isset($_POST['email_verify']) )
            {
                ...
                MT::getFeedback()->info($language->text('admin', 'user_feedback_email_verified'));
            }
            else if ( isset($_POST['email_unverify']) )
            {
                ...
                MT::getFeedback()->info($language->text('admin', 'user_feedback_email_unverified'));
            }
            else if ( isset($_POST['disapprove']) )
            {
                ...
                MT::getFeedback()->info($language->text('admin', 'user_feedback_profiles_disapproved'));
            }
            else if ( isset($_POST['approve']) )
            {
                ...
                MT::getFeedback()->info($language->text('admin', 'user_feedback_profiles_approved'));
            }
        ...
        $event = new MT_Event('admin.amount_users_per_page', array(), array("amountOnPage" => $onPage));
        ...
        if ( !$userList && $page > 1 )
        {
            MT::getApplication()->redirect(MT::getRequest()->buildUrlQueryString(null, array('page' => $page - 1)));
        }
        ...
        $this->assign('adminId', MT::getUser()->getId());
    ...
    private function reloadParentPage()
    {
        ...
        MT::getApplication()->redirect(MT::getRequest()->buildUrlQueryString());
    }
}
