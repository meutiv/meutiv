 ...
 * @package mt_system_plugins.admin.class
 * @since 1.0
 */
class ColorField extends FormElement
{
    ...
    public function __construct( $name )
    {
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('admin')->getStaticJsUrl() . 'color_picker.js');
    }
    ...
    public function renderInput( $params = null )
    {
        ...
        $('#color_" . $this->getId() . "').click(
            function(){
                ...
                window.colorPickers['" . $this->getId() . "'] = new MT_FloatBox({\$contents:$('#colorcont_" . $this->getId() . "'), \$title:'Color Picker'});
            }
        );";
...
class addValueField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        if ( $this->disabled )
        {
            ...
            $message = MT::getLanguage()->text('admin', 'possible_values_disable_message');

            $event = new MT_Event('admin.get.possible_values_disable_message', array('name' => $this->getName(), 'id' => $this->getId() ), $message);
        ...
        $template = '
                        <div class="clearfix question_value_block" style="cursor:move;">
                                <span class="tag">
                                    ...
                                    <a title='.json_encode(MT::getLanguage()->text('admin', 'remove_value')).' class="remove" href="javascript://"></a>
                                </span>
                        </div>';
        ...
        MT::getLanguage()->addKeyForJs('admin', 'questions_edit_delete_value_confirm_message');
        ...
        $html = '<div class="values_list">
                ...
                <div style="padding-left: 4px;" class="mt_smallmargin">'.MT::getLanguage()->text('admin', 'add_question_value_description').'</div>
                <div class="clearfix">
                    <div class="mt_left" style="width: 260px;">'.($this->tag->renderInput()).'</div>
                    <div class="mt_right">
                        <span class="mt_button">
                            <span class="mt_ic_add">
                                <input type="button" value='.json_encode(MT::getLanguage()->text('admin', 'add_button')).' class="mt_ic_add" name="'.$addButtonName.'">
                            </span>
                        </span>
                    </div>
                </div>';
