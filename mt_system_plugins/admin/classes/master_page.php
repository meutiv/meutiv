 ...
 * @package mt_system_plugins.admin.classes
 * @since 1.0
 */
class ADMIN_CLASS_MasterPage extends MT_MasterPage
{
    private $menuCmps = array();

    /**
     * @see MT_MasterPage::init()
     */
    protected function init()
    {
        $language = MT::getLanguage();

        MT::getThemeManager()->setCurrentTheme(BOL_ThemeService::getInstance()->getThemeObjectByKey(BOL_ThemeService::DEFAULT_THEME));
...
//            'menu_mobile' => BOL_NavigationService::MENU_TYPE_MOBILE
        ...
        // platform info        
        $event = new MT_Event('admin.get_soft_version_text');
        ...
        if ( empty($verString) )
        {
            $verString = MT::getLanguage()->text('admin', 'soft_version', array('version' => MT::getConfig()->getValue('base', 'soft_version'), 'build' => MT::getConfig()->getValue('base', 'soft_build')));
        }

        $this->assign('version', MT::getConfig()->getValue('base', 'soft_version'));
        $this->assign('build', MT::getConfig()->getValue('base', 'soft_build'));
        $this->assign('softVersion', $verString);
        
        $checkUrl = MT::getRouter()->urlFor("ADMIN_CTRL_Storage", "checkUpdates");
        $params = array(BOL_StorageService::URI_VAR_BACK_URI => urlencode(MT::getRequest()->getRequestUri()));
        $this->assign("checkUpdatesUrl", MT::getRequest()->buildUrlQueryString($checkUrl, $params));
    }
    ...
    public function onBeforeRender()
    {
        ...
        $language = MT::getLanguage();
        MT::getDocument()->addBodyClass('mt_admin_area');
        $this->setTemplate(MT::getThemeManager()->getMasterPageTemplate(MT_MasterPage::TEMPLATE_ADMIN));
        ...
        foreach ( $this->menuCmps as $key => $value )
        {
            ...
            $arrayToAssign[$key] = array('id' => $id, 'firstLink' => $menuItem->getUrl(), 'key' => $key, 'isActive' => $value->isActive(), 'label' => $language->text('admin', 'sidebar_' . $key), 'sub_menu' => ( $value->getElementsCount() < 2 ) ? '' : $value->render(), 'active_sub_menu' => ( $value->getElementsCount() < 2 ) ? '' : $value->render('mt_admin_submenu'));
        }
