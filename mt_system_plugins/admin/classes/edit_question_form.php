...
class ADMIN_CLASS_EditQuestionForm extends ADMIN_CLASS_AddQuestionForm
{
    ...
    public function loadQuestionData( BOL_Question $question )
    {
        ...
        foreach ( $this->presentations2types as $key => $item )
        {
            ...
            {
                $presentationsLabel[$key] = MT::getLanguage()->text('base', 'questions_question_presentation_' . $key . '_label');
            }
        }
...
//                }
                ...
                $message = MT::getLanguage()->text('admin', 'questions_question_was_not_updated_message');

                if ( $updated )
                {
                    $message = MT::getLanguage()->text('admin', 'questions_update_question_message');
                }
            ...
            {
                echo json_encode( array( 'result' => false, 'errors' => $this->getErrors(), 'message' => MT::getLanguage()->text( 'admin', 'questions_update_error' ) ) );
            }
            exit;
