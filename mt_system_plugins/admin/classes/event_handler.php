...
class ADMIN_CLASS_EventHandler
{
    ...
    public function init()
    {
        ...
        $eventManager->bind(MT_EventManager::ON_AFTER_ROUTE, array($this, "onAfterRoute"));
    ...
    public function onAddAdminNotification( ADMIN_CLASS_NotificationCollector $coll )
    {
        ...
        if ( MT::getConfig()->configExists("base", "cron_is_active") && (int) MT::getConfig()->getValue("base",
                "cron_is_active") == 0 )
        {
            $coll->add($language->text("admin", "warning_cron_is_not_active",
                    array("path" => MT_DIR_ROOT . "mt_cron" . DS . "run.php")),
                ADMIN_CLASS_NotificationCollector::NOTIFICATION_WARNING);
        }

        $items = $storageService->findItemsWithInvalidLicense();
        $licenseRequestUrl = MT::getRouter()->urlFor("ADMIN_CTRL_Storage", "checkItemLicense");
        $backUri = MT::getRouter()->getUri();
    ...
    public function onGetDisableActionList( MT_Event $e )
    {
        $params = $e->getParams();
        $data = $e->getData();
    ...
    public function onGetJoinStampDisableActionList( MT_Event $e )
    {
        $params = $e->getParams();
        $data = $e->getData();
    ...
    public function onAfterRoute( MT_Event $e )
    {
        $pluginService = BOL_PluginService::getInstance();
        $attrs = MT::getRequestHandler()->getHandlerAttributes();

        if ( is_subclass_of($attrs[MT_RequestHandler::ATTRS_KEY_CTRL], "ADMIN_CTRL_Abstract") )
        {
            ...
            {
                $params = array(
                    MT_RequestHandler::ATTRS_KEY_CTRL => "ADMIN_CTRL_Plugins",
                    MT_RequestHandler::ATTRS_KEY_ACTION => "manualUpdateRequest",
                    MT_RequestHandler::CATCH_ALL_REQUEST_KEY_REDIRECT => true
                );

                MT::getRequestHandler()->setCatchAllRequestsAttributes("admin.manual_update", $params);
                MT::getRequestHandler()->addCatchAllRequestsExclude("admin.manual_update", "ADMIN_CTRL_Plugins",
                    "manualUpdateAll");
            }
        }
    }

    public function checkIfAdminPage( MT_Event $e )
    {
        $attrs = MT::getRequestHandler()->getHandlerAttributes();

        if ( empty($attrs) || empty($attrs[MT_RequestHandler::ATTRS_KEY_CTRL]) )
        {
            return null;
        }

        return is_subclass_of($attrs[MT_RequestHandler::ATTRS_KEY_CTRL], "ADMIN_CTRL_Abstract");
    }
}
