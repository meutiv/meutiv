...
require_once MT_DIR_SYSTEM_PLUGIN . 'admin' . DS . 'classes' . DS . 'form_fields.php';
...
class ADMIN_CLASS_AddQuestionForm extends Form
{
    ...
    protected function init( $params = array() )
    {
        ...
        $event = new MT_Event('base.question.add_question_form.on_get_available_sections', $sectionsArray, $sectionsArray);
        ...
        foreach ( $configList as $config )
        {
            ...
            /* @var $qstConfig MT_FormElement */
            $qstConfig = MT::getClassInstance($className, $config->name);
        ...
        $qstSubmit->addAttribute('class', 'mt_button mt_ic_save');
    ...
    {
        if ( MT_Request::getInstance()->isPost() )
        {
            ...
            if ( $this->isValid($data) )
            {
                ...
                MT::getFeedback()->info(MT::getLanguage()->text( 'admin', 'questions_add_question_message' ));
            ...
            {
                echo json_encode( array( 'result' => false, 'errors' => $this->getErrors(), 'message' => MT::getLanguage()->text( 'admin', 'questions_add_question_error' ) ) );
            }
            exit;

            //MT::getSession()->set(self::ADD_QUESTION_SESSION_VAR, $_POST);
        }
    }
}
