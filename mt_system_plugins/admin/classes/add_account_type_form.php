...
class ADMIN_CLASS_AddAccountTypeForm extends Form {
    ...
    public function __construct(BOL_QuestionAccountType $accountType, $formName = '') {
        ...
        $this->setAjax(true);
        $this->setAction(MT::getRouter()->urlFor("ADMIN_CTRL_Questions", "ajaxResponder"));
        ...
        $submit->setValue(MT::getLanguage()->text('admin', 'questions_add_new_account_type'));

        $jsString = ' owForms[{$formName}].bind("success", function(json){
            if ( json.result.add == true) {
                MT.registerLanguageKey("base", ' . json_encode($key) . ', json.accountTypeName);
                MT.trigger("admin.add_account_type", [json], this);
            }
        }); ';

        MT::getLanguage()->addKeyForJs($prefix, $key);
        if (!empty($accountType->id)) {
            $jsString = ' owForms[{$formName}].bind("success", function(json){
                if ( json.result.update == true) {
                    MT.registerLanguageKey("base", ' . json_encode($key) . ', json.accountTypeName);
                    MT.trigger("admin.update_account_type", [json], this);
                }
            }); ';
        }
        ...
        if ($accountType->id) {
            $submit->setValue(MT::getLanguage()->text('admin', 'questions_save_account_type'));
        }
    ...
    protected function saveOrUpdateAccountType($accountTypeName, $roleId, $order) 
    {
        ...
        if (isset($order)) {
            ...
            if ( !empty($direction) )
            {
                ...
                $event = new MT_Event(BOL_QuestionService::EVENT_ON_ACCOUNT_TYPE_REORDER, array('dto' => $accountType, 'id' => $accountType->id, 'orderList' => $list));
    ...
    protected function saveLangValues($prefix, $key) {
        ...
        $languageService->generateCache(MT::getLanguage()->getCurrentId());
    }
