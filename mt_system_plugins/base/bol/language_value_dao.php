 ...
 * @since 1.0
 */
class BOL_LanguageValueDao extends MT_BaseDao
{
    ...
    {
        return MT_DB_PREFIX . 'base_language_value';
    }
    ...
    public function findLastKeyList( $first, $count, $prefix = null )
    {
        ...
        $query_part['dev-mode-order'] = !$this->isDevMode() && $prefix == null ? "IF(`p`.`prefix` = 'mt_custom', 1, 0) DESC, " : '';
    ...
    private function isDevMode()
    {
        if ( !empty($_GET) )
        {
            $arr = explode('?', MT::getRequest()->getRequestUri());

            return $arr[0] == MT::getRouter()->uriForRoute('admin_developer_tools_language');
        }

        return MT::getRequest()->getRequestUri() == MT::getRouter()->uriForRoute('admin_developer_tools_language');
    }
