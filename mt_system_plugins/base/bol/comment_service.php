...
final class BOL_CommentService
{
    ...
    public function findBatchCommentsData( array $items )
    {
        ...
        if ( MT::getUser()->isAuthenticated() )
        {
            $userInfo = BOL_AvatarService::getInstance()->getDataForUserAvatars(array(MT::getUser()->getId()));
        }
        ...
        if ( MT::getUser()->isAuthenticated() )
        {
            $resultArray['_static']['currentUserInfo'] = $currentUserInfo[MT::getUser()->getId()];
        }

        $eventParams = array('actionList' => $creditsParams);
        $resultArray['_static']['credits'] = MT::getEventManager()->call('usercredits.batch_check_balance_for_action_list', $eventParams);
