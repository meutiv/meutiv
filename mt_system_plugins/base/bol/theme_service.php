...
class BOL_ThemeService
{
    ...
    {
        return MT::getConfig()->getValue("base", "selectedTheme");
    }
    ...
    {
        MT::getConfig()->saveConfig("base", "selectedTheme", trim($name));
    }
    ...
    {
        return MT_DIR_USERFILES . "themes" . DS;
    }
    ...
    {
        return MT_URL_USERFILES . "themes/";
    }
    ...
    public function updateThemeList()
    {
        ...
        $xmlFiles = UTIL_File::findFiles(MT_DIR_THEME, array("xml"), 1);
        ...
        foreach ( $xmlFiles as $themeXml )
        {
            ...
            $result = MT::getEventManager()->call("admin.themes_list_theme_avail", array("name" => $themeKey));
    ...
    public function deleteTheme( $themeId, $deleteControlValues = false )
    {
        ...
        if ( $deleteControlValues )
        {
            ...
            $curentValue = json_decode(MT::getConfig()->getValue("base", "master_page_theme_info"), true);
            unset($curentValue[$themeId]);
            MT::getConfig()->getValue("base", "master_page_theme_info", json_encode($curentValue));
        }
...
//            $themeControls = array_merge($mobileControls, $themeControls);
    ...
    /**
     * Generates theme object for theme manager (MT_Theme).
     *
    ...
    public function saveThemeControls( $themeId, $values )
    {
        ...
        foreach ( $values as $key => $value )
        {
            ...
            if ( $namedControls[$key]["type"] == "image" )
            {
                ...
                $value = "url(" . MT::getStorage()->getFileUrl($this->getUserfileImagesDir() . $image->getFilename()) . ")";

                //TODO remove hotfix temp solution for assigning theme img data in master pages
                $curentValue = json_decode(MT::getConfig()->getValue("base", "master_page_theme_info"), true);
                $curentValue[$themeId][$namedControls[$key]["key"]] = array("src" => MT::getStorage()->getFileUrl($this->getUserfileImagesDir() . $image->getFilename()),
                    "width" => $width, "height" => $height);
                MT::getConfig()->saveConfig("base", "master_page_theme_info", json_encode($curentValue));
            }
    ...
    public function addImage( $fileArr )
    {
        ...
        MT::getStorage()->copyFile($newTempName, $this->getUserfileImagesDir() . $imageName);
    ...
    public function moveTemporaryFile( $tmpId, $title = '', $rotate = 0 )
    {
        ...
        MT::getStorage()->copyFile($tmpPath, $this->getUserfileImagesDir() . $imageName);
        if ( file_exists($newTempName) )
        {
            unlink($newTempName);
        }
    ...
    public function deleteImage( $id )
    {
        ...
        if ( $image !== null )
        {
            if ( MT::getStorage()->fileExists($this->getUserfileImagesDir() . $image->getFilename()) )
            {
                MT::getStorage()->removeFile($this->getUserfileImagesDir() . $image->getFilename());
            }
    ...
    public function updateCustomCssFile( $themeId )
    {
        ...
        if ( $theme->getCustomCssFileName() !== null )
        {
            ...
            if ( MT::getStorage()->fileExists($oldCssFilePath) )
            {
                MT::getStorage()->removeFile($oldCssFilePath);
            }

            if ( MT::getStorage()->fileExists($oldMobileCssFilePath) )
            {
                MT::getStorage()->removeFile($oldMobileCssFilePath);
            }
        }
        ...
        MT::getStorage()->copyFile($tempCssFilePath, $newCssFilePath);
        ...
        MT::getEventManager()->trigger(new MT_Event("base.update_custom_css_file", array("name" => $theme->getName())));
    }
    ...
    public function getCustomCssFileUrl( $themeName, $mobile = false )
    {
        ...
        return MT::getStorage()->getFileUrl($this->getUserfileImagesDir() . ( $mobile ? "mobile_" : '' ) . $theme->getCustomCssFileName());
    }
    ...
    {
        return MT_DIR_STATIC_THEME . $themeName . ($mobile ? DS . self::DIR_NAME_MOBILE : '') . DS;
    }
    ...
    {
        return MT_URL_STATIC_THEMES . $themeName . ($mobile ? '/' . self::DIR_NAME_MOBILE : '') . '/';
    }
    ...
    {
        return MT_DIR_THEME . $themeName . ($mobile ? DS . self::DIR_NAME_MOBILE : '') . DS;
    }
    ...
    /**
     * Update theme info in the [MT_DB_PREFIX]_base_theme table according to the theme.xml file and force theme rebuilding if necessary
     * @param $name
    ...
    private function getThemeXmlInfo( $themeXmlPath )
    {
        ...
        {
            MT::getLogger()->addEntry(__CLASS__ . "::" . __FUNCTION__ . " - `" . $themeXmlPath . "` not found");
            return null;
        }
        ...
        if ( !$xmlInfo )
        {
            MT::getLogger()->addEntry(__CLASS__ . "::" . __FUNCTION__ . " - invalid `" . $themeXmlPath . "`");
            return null;
        }
        ...
        foreach ( $propList as $prop )
        {
            ...
            {
                MT::getLogger()->addEntry(__CLASS__ . "::" . __FUNCTION__ . " - in `" . $themeXmlPath . "` property `" . $prop . "` not found");
                return null;
            }
        }
    ...
    private function unlinkControlValueImage( $controlValue )
    {
        ...
        if ( MT::getStorage()->fileExists($this->getUserfileImagesDir() . $fileName) )
        {
            MT::getStorage()->removeFile($this->getUserfileImagesDir() . $fileName);
        }
    }
}
