...
final class BOL_BillingService
{
    ...
    public function initSale( BOL_BillingSale $sale, $gatewayKey )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_AFTER_INIT_SALE, array("saleDbo" => $sale, "gateway" => $gateway));
     ...
     * @return bool
     */
    public function prepareSale( MT_BillingAdapter $adapter, BOL_BillingSale $sale )
    {
     ...
     * 
     * @param MT_BillingAdapter $adapter
     * @param BOL_BillingSale $sale
     * @return boolean
     */
    public function verifySale( MT_BillingAdapter $adapter, BOL_BillingSale $sale )
    {
        if ( !$this->getSaleById($sale->id) )
        {
            return false;
     ...
     * 
     * @param MT_BillingProductAdapter $adapter
     * @param BOL_BillingSale $sale
     * @return int
     */
    public function deliverSale( MT_BillingProductAdapter $adapter, BOL_BillingSale $sale )
    {
        if ( !$this->getSaleById($sale->id) )
        {
            return false;
        ...
        if ( $adapter->deliverSale($sale) )
        {
            ...
            $this->saveSale($sale);
            $event = new MT_Event(self::EVENT_ON_AFTER_DELIVER_SALE, array("saleDbo" => $sale, "adapter" => $adapter));
            MT::getEventManager()->trigger($event);
            return true;
        }
    ...
    public function registerRebillSale( MT_BillingAdapter $adapter, BOL_BillingSale $parentSale, $rebillTransId )
    {
        $parentSale->id = null;
     ...
     * 
     * @param string $productKey
     * @return MT_BillingProductAdapter
     */
    ...
    {
        return MT::getConfig()->getValue('base', 'billing_currency');
    }
    ...
    public function getOrderCompletedPageUrl( $hash = null )
    {
        ...
        {
            return MT::getRouter()->urlForRoute('base_billing_completed', array('hash' => $hash));
        }
        else
        {
            return MT::getRouter()->urlForRoute('base_billing_completed_st');
        }
    }
    ...
    public function getOrderCancelledPageUrl( $hash = null)
    {
        if ( isset($hash) && $sale = $this->getSaleByHash($hash) )
        {
            return MT::getRouter()->urlForRoute('base_billing_canceled', array('hash' => $hash));
        }
        else
        {
            return MT::getRouter()->urlForRoute('base_billing_canceled_st');
        }
    }
    ...
    {
        return MT::getRouter()->urlForRoute('base_billing_error');
    }
    ...
    public function  getBillingGatewayExtraInfo( $billingGatewayKey, $langKey )
    {
        ...
        return MT::getLanguage()->text($billingGatewayKey, $langKey);
    }
    ...
    public function findGatewayById( $gatewayId = null )
    {
        if( empty($gatewayId) )
        {
            $logger = MT::getLogger('mobile_billing');
