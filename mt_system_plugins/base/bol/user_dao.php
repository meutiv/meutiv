 ...
 * @since 1.0
 */
class BOL_UserDao extends MT_BaseDao
{
    const EMAIL = 'email';
    ...
    public function findMassMailingUsers( $start, $count, $ignoreUnsubscribe = false, $userRoles = array() )
    {
        ...
        if ( !empty($userRoles) && is_array($userRoles) )
        {
            ...
            $where .= " AND  ( `role`.`name` IN ( " . MT::getDbo()->mergeInClause($userRoles) . " ) ) ";
        }
    ...
    public function countUsersByQuestionValues( $questionValues, $isAdmin = false, $aditionalParams = array() )
    {
        ...
        if ( !empty($aditionalParams["limit_users_count"]) && MT_SQL_LIMIT_USERS_COUNT > 0 )
        {
            $orderFieldname = self::ACTIVITY_STAMP;
            $usersTableName = "( SELECT * FROM {$usersTableName} ORDER BY `{$orderFieldname}` DESC LIMIT " . MT_SQL_LIMIT_USERS_COUNT . " )";
        }
    ...
    public function getQuestionWhereString( BOL_Question $question, $value, $prefix = '' )
    {
        ...
        /* $event = new MT_Event('base.questions_get_search_sql', array(
