...
class BOL_QuestionService
{
    ...
    {
        $event = new MT_Event('base.questions_field_get_label', array(
        ...
        $event = new MT_Event('base.questions_field_init', array(
            'type' => 'main',
            'presentation' => $presentation,
            'fieldName' => $fieldName,
            'configs' => $configs
                    ...
                    if ( trim($this->birthdayConfig) )
                    {
                        ...
                        foreach ( $configsList as $name => $value )
                        {
                            if ( $name == 'year_range' && isset($value['from']) && isset($value['to']) )
                            {
                                ...
                                $event = new MT_Event('base.questions_question_set_range_join', array(
    ...
    public function getFormElementByQuestionDto( BOL_Question $question, array $values, $type = 'join' )
    {
        ...
        if ( !empty($class) )
        {
            $class->setLabel(MT::getLanguage()->text('base', 'questions_question_' . $question->name . '_label'));
            ...
            if ( in_array($question->type, array(BOL_QuestionService::QUESTION_VALUE_TYPE_SELECT, BOL_QuestionService::QUESTION_VALUE_TYPE_MULTISELECT) ) )
            {
                ...
                if ( method_exists($class, 'setOptions') )
                {
                    if ( !empty($values[$question->name]['values']) && is_array($values[$question->name]['values']) )
                    {
                        ...
                        {
                            $valuesArray[$value->value] = MT::getLanguage()->text( 'base', 'questions_question_' . $value->questionName . '_value_' . ($value->value) );
                        }
    ...
    public function findSignUpQuestionsForAccountType( $accountType, $baseOnly = false )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_FIND_SIGN_UP_QUESTIONS_FOR_ACCOUNT_TYPE, array(
            'account' => $accountType,
            'baseOnly' => $baseOnly
        ), $questionsList);
    ...
    public function findBaseSignUpQuestions()
    {
        ...
        $event = new MT_Event(self::EVENT_ON_FIND_BASE_SIGN_UP_QUESTIONS, array(), $questionsList);
    ...
    public function findEditQuestionsForAccountType( $accountType )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_FIND_QUESTIONS_FOR_ACCOUNT_TYPE, array(
            'account' => $accountType
        ), $questionsList);
    ...
    public function findViewQuestionsForAccountType( $accountType )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_FIND_VIEW_QUESTIONS_FOR_ACCOUNT_TYPE, array(
            'account' => $accountType
        ), $questionsList);
    ...
    public function findAllQuestionsForAccountType( $accountType )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_FIND_ALL_QUESTIONS_FOR_ACCOUNT_TYPE, array(
            'account' => $accountType
        ), $questionsList);
    ...
    {
        $event = new MT_Event(self::EVENT_ON_BEFORE_ADD_QUESTION, array(), $question);
        MT::getEventManager()->trigger($event);
    ...
    public function deleteQuestionValue( $questionName, $value )
    {
        ...
        if ( $isDelete )
        {
            ...
            $event = new MT_Event(self::EVENT_AFTER_DELETE_QUESTION_VALUE, array('questionName' => $questionName, 'value' => $value));
            MT::getEventManager()->trigger($event);
        }
    ...
    public function deleteAccountType( $accountType )
    {
        ...
        $deleted = (boolean) MT::getDbo()->getAffectedRows();

        if ( $deleted )
        {
            $event = new MT_Event(self::EVENT_ON_ACCOUNT_TYPE_DELETE, array('id' => $account->id, 'name' => $account->name));
            MT::getEventManager()->trigger($event);
        }
    ...
    public function deleteSection( $sectionName, BOL_QuestionSection &$moveQuestionsToSection = null )
    {
        ...
        $event = new MT_Event( self::EVENT_ON_SECTION_DELETE, array( 'dto' => $section ) );
        MT::getEventManager()->trigger($event);
    ...
    public function deleteQuestion( array $questionIdList )
    {
        ...
        foreach ( $questionArray as $question )
        {
            ...
            $event = new MT_Event( self::EVENT_ON_QUESTION_DELETE, array( 'questionName' => $question->name, 'dto' => $question ) );
        ...
        return (boolean) MT::getDbo()->getAffectedRows();
    }
    ...
    public function saveQuestionsData( array $data, $userId )
    {
        ...
        $event = new MT_Event('base.questions_save_data', array('userId' => $userId), $data);
        ...
        if ( $sendVerifyMail && MT::getConfig()->getValue('base', 'confirm_email') )
        {
    ...
    {
        $event = new MT_Event('base.question_text_formatter', array(
    ...
    public function reOrderQuestion( $sectionName, array $questionOrder )
    {
        ...
        $event = new MT_Event( self::EVENT_ON_SORT_QUESTION, array('sectionName' => $sectionName, 'questionOrder' => $questionOrder));
        MT::getEventManager()->trigger($event);
    ...
    public function getQuestionData( array $userIdList, array $fieldsList )
    {
        ...
        $event = new MT_Event('base.questions_get_data', array('userIdList' => $userIdList, 'fieldsList' => $fieldsList), $result);
    ...
    public function getQuestionLangKeyName( $type, $name, $value = null )
    {
        ...
        $event = new MT_Event( self::EVENT_ON_GET_QUESTION_LANG, array('type' => $type, 'name' => $name, 'value' => $value));
        MT::getEventManager()->trigger($event);
    ...
    public function getQuestionDescriptionLang( $questionName )
    {
        ...
        $text = MT::getLanguage()->text(self::QUESTION_LANG_PREFIX,$key);
    ...
    {
        return MT::getLanguage()->text(self::QUESTION_LANG_PREFIX, $this->getQuestionLangKeyName(self::LANG_KEY_TYPE_QUESTION_LABEL, $questionName));
    }

    public function getQuestionValueLang( $questionName, $value )
    {
        return MT::getLanguage()->text(self::QUESTION_LANG_PREFIX, $this->getQuestionLangKeyName(self::LANG_KEY_TYPE_QUESTION_VALUE, $questionName, $value));
    }

    public function getSectionLang( $sectionName )
    {
        return MT::getLanguage()->text(self::QUESTION_LANG_PREFIX, $this->getQuestionLangKeyName(self::LANG_KEY_TYPE_QUESTION_SECTION, $sectionName));
    }

    public function getAccountTypeLang( $accountType )
    {
        return MT::getLanguage()->text(self::QUESTION_LANG_PREFIX, $this->getQuestionLangKeyName(self::LANG_KEY_TYPE_ACCOUNT_TYPE, $accountType));
    }
    ...
    public function createQuestion( BOL_Question $question, $label, $description = '', $values = array(), $saveValuesKeys = false, $generateCache = true )
    {
        ...
        if ( $generateCache )
        {
            BOL_LanguageService::getInstance()->generateCache(MT::getLanguage()->getCurrentId());
        }
...
//                    BOL_LanguageService::getInstance()->addOrUpdateValue(MT::getLanguage()->getCurrentId(), 'base', 'questions_question_' . ($questionName) . '_value_' . $value, $label, $generateCache);
                    $event = new MT_Event(self::EVENT_AFTER_UPDATE_QUESTION_VALUE, array('dto' => $existingValuesList[$key]));
                    MT::getEventManager()->trigger($event);
                }
                else
                {
    ...
    public function addQuestionValue( $questionName, $value, $label, $sortOrder, $generateCache = true )
    {
        ...
        $event = new MT_Event(self::EVENT_AFTER_ADD_QUESTION_VALUE, array('dto' => $questionValue));
        MT::getEventManager()->trigger($event);
    ...
    public function updateQuestionsEditStamp()
    {
        ...
        {
            MT::getConfig()->saveConfig( 'base', 'profile_question_edit_stamp', time() );
    ...
    {
        return MT::getConfig()->getValue( 'base', 'profile_question_edit_stamp' );
    }
    ...
    public function getQuestionDisableActionList( BOL_question $question )
    {
        ...
        $event = new MT_Event( 'admin.disable_fields_on_edit_profile_question', array( 'questionDto' => $question ), $disableActionList );
        MT::getEventManager()->trigger($event);
    ...
    public function createAccountType( $accountTypeName, $label = '', $roleId = 0 )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_ACCOUNT_TYPE_ADD, array('dto' => $accountType, 'id' => $accountType->id));
        MT::getEventManager()->trigger($event);

        $questionNameList = $this->getRequiredQuestionsForNewAccountType();
        
        $event = new MT_Event(self::EVENT_BEFORE_ADD_QUESTIONS_TO_NEW_ACCOUNT_TYPE, array('dto' => $accountType), $questionNameList);
        MT::getEventManager()->trigger($event);
    ...
    public function getEmptyRequiredQuestionsList( $userId )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_GET_EMPTY_REQUIRED_QUESTIONS, array(
        ...
        $event = new MT_Event(self::EVENT_ON_GET_EMPTY_QUESTIONS_LIST, array(
            'account' => $user->accountType,
            'userId' => $user->id
        ), $emptyQuestionsList);
    ...
    public function getQuestionValueForUserList( BOL_Question $question, $value )
    {
        ...
        switch ( $question->presentation )
        {
            case BOL_QuestionService::QUESTION_PRESENTATION_CHECKBOX:
                ...
                if ( (int) $value === 1 )
                {
                    $stringValue = MT::getLanguage()->text('base', 'yes');
                }
            ...
            case BOL_QuestionService::QUESTION_PRESENTATION_DATE:
                ...
                $format = MT::getConfig()->getValue('base', 'date_field_format');
