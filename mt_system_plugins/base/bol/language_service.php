...
class BOL_LanguageService
{
     ...
     * @var array
     */
    private $exceptionPrefixes = array( "mobile", "nav", "mt_custom" ); // section which importing without checking plugin key
    ...
    public static function getInstance( $includeCache = true )
    {
        if ( !isset(self::$classInstance) )
        {
            try
            {
                self::$classInstance = MT::getClassInstance(self::class, $includeCache);
            }
    ...
    {
        MT::getEventManager()->trigger( new MT_Event('servicelangtools.lang_used_log', array( 'prefix' => $prefix, 'key' => $key)) );
    ...
    public function getText( $languageId, $prefix, $key, $vars = array() )
    {
        ...
        $event = new MT_Event("core.get_text", array("prefix" => $prefix, "key" => $key, "vars" => $vars));
        MT::getEventManager()->trigger($event);
    ...
    public function addOrUpdateValue( $languageId, $prefix, $key, $value, $generateCache = true )
    {
        ...
        $event = new MT_Event('base.on_after_add_or_update_value_lang', array(
    ...
    {
        return MT::getPluginManager()->getPlugin('admin')->getPluginFilesDir() . 'languages' . DS . 'import' . DS;
    }

    public static function getExportDirPath()
    {
        return MT::getPluginManager()->getPlugin('admin')->getPluginFilesDir() . 'languages' . DS . 'export' . DS;
    }

    public static function getTmpDirPath()
    {
        return MT::getPluginManager()->getPlugin('admin')->getPluginFilesDir() . 'languages' . DS . 'tmp' . DS;
    }
    ...
    {
        return MT::getPluginManager()->getPlugin('base')->getPluginFilesDir();
    }
