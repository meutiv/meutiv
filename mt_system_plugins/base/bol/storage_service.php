...
class BOL_StorageService
{
    ...
    const ITEM_DEACTIVATE_TIMEOUT_IN_DAYS = 30;
    ...
    {
        $requestArray = array("platform" => array(self::URI_VAR_BUILD => MT::getConfig()->getValue("base", "soft_build")),
            "items" => array());
        ...
        {
            MT::getLogger()->addEntry(__CLASS__ . "::" . __METHOD__ . "#" . __LINE__ . " storage request status is not OK",
                "core.update");
        ...
        {
            MT::getLogger()->addEntry(__CLASS__ . "::" . __METHOD__ . "#" . __LINE__ . " remote request returned empty result",
                "core.update");
        ...
        if ( !empty($resultArray["update"]) )
        {
            ...
            {
                MT::getConfig()->saveConfig("base", "update_soft", 1);
            }
    ...
    public function getItemInfoForUpdate( $key, $devKey, $currentBuild = 0 )
    {
        ...
        $requestUrl = MT::getRequest()->buildUrlQueryString($this->getStorageUrl(self::URI_GET_ITEM_INFO), $data);
    ...
    public function downloadPlatform()
    {
        $params = array(
            ...
            "site-url" => MT::getRouter()->getBaseUrl()
        );

        $data = array_merge($params, $this->triggerEventBeforeRequest($params));
        $requestUrl = MT::getRequest()->buildUrlQueryString($this->getStorageUrl(self::URI_DOWNLOAD_PLATFORM_ARCHIVE),
            $data);
        ...
        $archivePath = MT_DIR_PLUGINFILES . DS . $fileName;
    ...
    {
        $filePath = MT_DIR_ROOT . "mt_version.xml";
    ...
    public function getFtpConnection()
    {
        ...
        if ( !MT::getSession()->isKeySet("ftpAttrs") || !is_array(MT::getSession()->get("ftpAttrs")) )
        {
        ...
        else
        {
            ...
            try
            {
                $ftp = UTIL_Ftp::getConnection(MT::getSession()->get("ftpAttrs"));
            }
            catch ( Exception $ex )
            {
    ...
    {
        return MT_URL_HOME . "mt_updates/index.php";
    }
    ...
    {
        $event = MT::getEventManager()->trigger(new MT_Event('base.on_plugin_info_update', $params));
        $data = $event->getData();
        ...
        foreach ( $itemsToCheck as $item )
        {
            ...
            if ( isset($invalidItems[$type][$item->getKey()]) && $invalidItems[$type][$item->getKey()] == $item->getDeveloperKey() )
            {
                ...
                else if ( $this->isItemLicenseCheckPeriodExpired($item->getLicenseCheckTimestamp()) )
                {
                    ...
                    {
                        $defaultTheme = MT::getEventManager()->call("base.get_default_theme");
    ...
    {
        $event = new MT_Event(self::EVENT_ON_NOTIFY_ADMIN_ABOUT_INVALID_ITEMS, array(), $items);
        ...
        $params = array(
            ...
            "siteURL" => MT::getRouter()->getBaseUrl(),
            "adminUrl" => MT::getRouter()->urlForRoute("admin_plugins_installed")
        );

        $language = MT::getLanguage();
        $mail = MT::getMailer()->createMail();
        $mail->addRecipientEmail(MT::getConfig()->getValue("base", "site_email"));
    ...
    {
        $data["site-url"] = MT::getRouter()->getBaseUrl();
        $params = new UTIL_HttpClientParams();
        $params->addParams($data);
