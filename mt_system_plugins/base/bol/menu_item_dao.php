 ...
 * @since 1.0
 */
class BOL_MenuItemDao extends MT_BaseDao
{
    const PREFIX = 'prefix';
    const KEY = 'key';
    ...
    {
        return MT_DB_PREFIX . 'base_menu_item';
    }
    ...
    public function findMenuItems( $menuType )
    {
        return $this->dbo->queryForList("
			...
			WHERE `mi`.`" . self::TYPE . "` = :menuType ORDER BY `mi`.`order` ASC", array('menuType' => $menuType), 24 * 3600, array(self::CACHE_TAG_MENU_TYPE_LIST, MT_CacheManager::TAG_OPTION_INSTANT_LOAD));
    }
    ...
    {
        MT::getCacheManager()->clean(array(BOL_MenuItemDao::CACHE_TAG_MENU_TYPE_LIST));
    }
}
