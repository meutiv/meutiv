...
final class BOL_UserService
{
    ...
    public function getDisplayName( $userId )
    {
        ...
        $event = MT::getEventManager()->trigger(new MT_Event(self::EVENT_ON_GET_DISPLAY_NAME, array(
            'userIdList' => array($userId),
            'questionName' => $questionName
        ), $questionValue));
        $questionValue = $event->getData();

        $displayName = isset($questionValue[$userId]) ? ( isset($questionValue[$userId][$questionName]) ? $questionValue[$userId][$questionName] : '' ) : MT::getLanguage()->text('base', 'deleted_user');
    ...
    public function getDisplayNamesForList( array $userIdList )
    {
        ...
        {
            $resultArray[$value] = MT::getLanguage()->text('base', 'deleted_user');
    ...
    public function getUserUrlForUsername( $username )
    {
        ...
        return MT::getRouter()->urlForRoute('base_user_profile', array('username' => $username));
    }
    ...
    public function findOnlineList( $first, $count )
    {
        ...
        $permission = MT::getEventManager()->getInstance()->call('privacy_check_permission_for_user_list', $eventParams);
    ...
    {
        $event = new MT_Event('base.before_save_user', array('dto' => $user));
        MT::getEventManager()->trigger($event);
    ...
    public function markAsFeatured( $userId )
    {
        ...
        $event = new MT_Event(MT_EventManager::ON_USER_MARK_FEATURED, array('userId' => $userId));
        MT::getEventManager()->trigger($event);
    ...
    public function cancelFeatured( $userId )
    {
        ...
        $event = new MT_Event(MT_EventManager::ON_USER_UNMARK_FEATURED, array('userId' => $userId));
        MT::getEventManager()->trigger($event);
    }
    ...
    public function updateActivityStamp( $userId, $context )
    {
        ...
        $event = new MT_Event(self::BEFORE_USER_ONLINE, array('userId' => $userId, 'context' => $context), $userOnline);
        MT_EventManager::getInstance()->trigger($event);
        $data = $event->getData();
    ...
    {
        $event = new MT_Event(MT_EventManager::ON_USER_UNREGISTER, array('userId' => $userId, 'deleteContent' => $deleteContent));
        MT::getEventManager()->trigger($event);
    ...
    public function suspend( $userId, $message )
    {
        ...
        $event = new MT_Event(MT_EventManager::ON_USER_SUSPEND, array('userId' => $userId, 'message' => $message));
        MT::getEventManager()->trigger($event);
    }
    ...
    public function unsuspend( $userId )
    {
        ...
        $event = new MT_Event(MT_EventManager::ON_USER_UNSUSPEND, array('userId' => $userId));
        MT::getEventManager()->trigger($event);
    }
    ...
    public function block( $userId )
    {
        ...
        $dto->setUserId(MT::getUser()->getId());
        $dto->setBlockedUserId($userId);

        $this->userBlockDao->save($dto);

        $event = new MT_Event(MT_EventManager::ON_USER_BLOCK, array('userId' => MT::getUser()->getId(), 'blockedUserId' => $userId));
        MT::getEventManager()->trigger($event);
    }
    ...
    public function unblock( $userId )
    {
        ...
        $dto = $this->userBlockDao->findBlockedUser(MT::getUser()->getId(), $userId);

        $this->userBlockDao->delete($dto);

        $event = new MT_Event(MT_EventManager::ON_USER_UNBLOCK, array('userId' => MT::getUser()->getId(), 'blockedUserId' => $userId));
        MT::getEventManager()->trigger($event);
    }
    ...
    {
        return hash('sha256', MT_PASSWORD_SALT . $password);
    }
    ...
    {
        $timestamp = MT::getConfig()->configExists('base', 'user_expired_time') ? time() - MT::getConfig()->getValue('base', 'user_expired_time') * 60 : time() - 30 * 60;
    ...
    public function createUser( $username, $password, $email, $accountType = null, $emailVerify = false, $ip = null )
    {
        ...
        $user->joinIp = $ip ? $ip : ip2long(MT::getRequest()->getRemoteAddress());
    ...
    public function sendAdminInvitation( $email )
    {
        ...
        $inviteUrl = MT::getRequest()->buildUrlQueryString(MT::getRouter()->urlForRoute('base_join'), array('code' => $inviteCodeDto->getCode()));

        $mail = MT::getMailer()->createMail();
        $mail->addRecipientEmail($email);
        $mail->setSubject(MT::getLanguage()->text('base', 'mail_template_admin_invite_user_subject'));
        $mail->setHtmlContent(MT::getLanguage()->text('base', 'mail_template_admin_invite_user_content_html', array('url' => $inviteUrl)));
        $mail->setTextContent(MT::getLanguage()->text('base', 'mail_template_admin_invite_user_content_text', array('url' => $inviteUrl)));

        MT::getMailer()->addToQueue($mail);
    }
    ...
    public function disapprove( $userId )
    {
        ...
        $event = new MT_Event(MT_EventManager::ON_USER_DISAPPROVE, array('userId' => $userId));
        MT::getEventManager()->trigger($event);
    }
    ...
    public function approve( $userId )
    {
        ...
        $event = new MT_Event(MT_EventManager::ON_USER_APPROVE, array('userId' => $userId));
        MT::getEventManager()->trigger($event);
    }
    ...
    public function findUserIdListByQuestionValues( $questionValues, $first, $count, $isAdmin = false, $aditionalParams = array() )
    {
        ...
        $event = new MT_Event("base.question.before_user_search", $data, $data);
    ...
    public function getNewResetPasswordCode( $userId )
    {
        ...
        $event = new MT_Event('base.on_after_generate_password_reset_code', [], $hashedCode);
        MT::getEventManager()->trigger($event);
    ...
    public function sendWellcomeLetter( BOL_User $user )
    {
        ...
        if ( MT::getConfig()->getValue('base', 'confirm_email') && $user->emailVerify != true )
        {
            return;
        }
    ...
    public function getSignInForm( $formName = 'sign-in', $submitDecorator = 'button' )
    {
        ...
        $password->setInvitation(MT::getLanguage()->text('base', 'component_sign_in_password_invitation'));
        $password->setRequired(true);
        $form->addElement($password);

        $rememberMe = new CheckboxField('remember', 'base');
        $rememberMe->setLabel(MT::getLanguage()->text('base', 'sign_in_remember_me_label'));
    ...
    public function processSignIn( $identity, $password, $rememberMe = false )
    {
        ...
        $result = MT::getUser()->authenticate(new BASE_CLASS_StandardAuth($identity, $password));

        if ( $result->isValid() )
        {
            if ( $rememberMe )
            {
                $loginCookie = $this->saveLoginCookie(MT::getUser()->getId());
    ...
    public function processResetForm( $data )
    {
        ...
        try
        {
            $vars = array(
                ...
                'requestUrl' => MT::getRouter()->urlForRoute('base.reset_user_password_request'),
                'resetUrl' => MT::getRouter()->urlForRoute('base.reset_user_password', array('code' => $resetPasswordCode))
            );
    ...
    public function processResetPasswordForm( $data, BOL_User $user, $resetCode )
    {
        ...
        {
            throw new LogicException(MT::getLanguage()->text('base', 'reset_password_length_error_message', array('min' => UTIL_Validator::PASSWORD_MIN_LENGTH, 'max' => UTIL_Validator::PASSWORD_MAX_LENGTH)));
        }
    ...
    public function getUserViewQuestions( $userId, $adminMode = false, $questionNames = array(), $sectionNames = null )
    {
        ...
        $event = new MT_Event(self::EVENT_GET_USER_VIEW_QUESTIONS, array(
            'userId' => $userId,
            'adminMode' => $adminMode,
        ...
        foreach ( $questionArray as $sectionKey => $section )
        {
            foreach ( $section as $questionKey => $question )
            {
                ...
                $event = new MT_Event('base.questions_field_get_value', array(
                    'presentation' => $question['presentation'],
                    'fieldName' => $question['name'],
                ...
                if ( !empty($questionData[$userId][$question['name']]) )
                {
                    switch ( $question['presentation'] )
                    {
                        case BOL_QuestionService::QUESTION_PRESENTATION_CHECKBOX:
                            ...
                            {
                                $questionData[$userId][$question['name']] = MT::getLanguage()->text('base', 'yes');
                            }
                            else
                            {
    ...
    public function sendReActivationNotification( array $userIds )
    {
        ...
        if ( is_array($userIds) )
        {
            foreach ( $userIds as $item => $userId )
            {
                ...
                {
                    MT::getLogger()->addEntry(json_encode($e));
                }
            }
        }
    }
