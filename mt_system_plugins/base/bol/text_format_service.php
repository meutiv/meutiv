...
final class BOL_TextFormatService
{
    ...
    {
        return (!(bool) MT::getConfig()->getValue('base', self::CONF_USER_INPUT_CUSTOM_HTML_DISABLE));
    }
    ...
    {
        return !((bool) MT::getConfig()->getValue('base', self::CONF_USER_INPUT_RICH_MEDIA_DISABLE));
    }
    ...
    {
        return !((bool) MT::getConfig()->getValue('base', self::CONF_COMMENTS_RICH_MEDIA_DISABLE));
    }
    ...
    {
        $list = MT::getConfig()->getValue('base', self::CONF_MEDIA_RESOURCE_LIST);
...
//        $text = str_replace(array('<div>', '</div>'), array('', '<br />'), $text);
        ...
        //hotfix for ie8
        $text = str_ireplace(array('<SPAN class=mt_ws_video>'), '<span class="mt_ws_video">', $text);
    ...
    private function processHtml( $text )
    {
        ...
        while ( mb_strstr($text, '<span class="mt_ws_html">') )
        {
            $openSearchStr = '<span class="mt_ws_html">';
            $closeSearchStr = '</span>';
    ...
    private function processVideo( $text )
    {
        ...
        while ( mb_strstr($text, '<span class="mt_ws_video">') )
        {
            $openSearchStr = '<span class="mt_ws_video">';
            $closeSearchStr = '</span>';
    ...
    public static function addVideoCodeParam( $code, $name = 'wmode', $value = 'transparent' )
    {
        ...
        if ( preg_match("/<iframe[^>]*src=['\"]([^'\"]+)['\"]/i", !empty($repl) ? $repl : $code, $matches) )
        {
            ...
            {
                $src = "http:" . $matches[1];
                $src = MT::getRequest()->buildUrlQueryString($src, array($name => $value));
                $src = substr($src, 5);
            }
            else
            {
                $src = MT::getRequest()->buildUrlQueryString($matches[1], array($name => $value));
            }
