...
class BOL_NavigationService
{
    ...
    public function getMenuItems( array $menuItems )
    {
        ...
        foreach ( $menuItems as $value )
        {
            ...
            $auth = MT::getUser()->isAuthenticated();
            ...
            {
                $url = MT::getRouter()->getBaseUrl() . $value['uri'];
            }
            else if ( !empty($value['routePath']) )
            {
                $url = MT::getRouter()->urlForRoute($value['routePath']);
            }
            else if ( !empty($value['class']) && !empty($value['action']) )
            {
                $url = MT::getRouter()->urlFor($value['class'], $value['action']);
            }
            else
            {
                $url = '_INVALID_URL_';
            ...
            $menuItem->setLabel(MT::getLanguage()->text($value['prefix'], $value['menu_key']));
        ...
        $event = new MT_Event('navigation.onGetMenuItems', ['menuItems' => $menuItems], $resultArray);
