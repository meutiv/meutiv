...
final class BOL_LogService
{
    ...
    public function addEntries( array $entries )
    {
        ...
        if ( !empty($entries) )
        {
            ...
            {
                $obj = new BOL_Log();
                $obj->setKey($entry[MT_Log::KEY]);
                $obj->setType($entry[MT_Log::TYPE]);
                $obj->setMessage($entry[MT_Log::MESSAGE]);
                $obj->setTimeStamp($entry[MT_Log::TIME_STAMP]);
    ...
    public function processEntries( $entries )
    {
        return array_map(function ( $entry ) {
            ...
            $viewUrl = MT::getRouter()->urlForRoute('admin_developer_tools_logs_entry', array('id' => $entry->getId()));
            $filterByTypeUrl = MT::getRouter()->urlForRoute('admin_developer_tools_logs') . '?' . $filterByTypeQuery;
            $filterByKeyUrl = MT::getRouter()->urlForRoute('admin_developer_tools_logs') . '?' . $filterByKeyQuery;
            ...
            $processLogMessageEvent = new MT_Event(MT_EventManager::ON_PROCESS_LOG_ENTRY_MESSAGE, array(
                'entry' => $entry
            ));

            MT::getEventManager()->trigger($processLogMessageEvent);
