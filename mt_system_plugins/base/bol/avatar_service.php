...
class BOL_AvatarService
{
    ...
    public function deleteUserAvatar( $userId )
    {
        ...
        $event = new MT_Event('base.before_user_avatar_delete', array('avatarId' => $avatar->id ));
    ...
    public function cropAvatar( $userId, $path, $coords, $viewSize, array $editionalParams = array() )
    {
        ...
        $event = new MT_Event('base.after_avatar_update', array_merge($editionalParams, $params));
        MT::getEventManager()->trigger($event);
        ...
        {
            $toFilePath = MT::getPluginManager()->getPlugin('base')->getPluginFilesDir() . uniqid(md5( rand(0,9999999999) )).UTIL_File::getExtension($path);
    ...
    {
        $path = MT::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS . 'tmp' . DS;
    ...
    public function renameAvatarOriginal( $userId, $oldHash, $newHash )
    {
        ...
        MT::getStorage()->renameFile($originalPath, $newPath);
    }
    ...
    {
        $event = new MT_Event("base.avatars.get_list", array(
            "userIds" => array($userId),
            "size" => $size,
            "checkModerationStatus" => $checkModerationStatus
        ));

        $eventAvatars = MT::getEventManager()->trigger($event)->getData();
    ...
    public function getDefaultAvatarUrl( $size = 1 )
    {
        ...
        {
            case 1:
                return MT::getThemeManager()->getSelectedTheme()->getStaticImagesUrl(MT::getApplication()->isMobile()) . 'no-avatar.png';

            case 2:
                return MT::getThemeManager()->getSelectedTheme()->getStaticImagesUrl(MT::getApplication()->isMobile()) . 'no-avatar-big.png';
        }
    ...
    private function getCustomDefaultAvatarUrl( $size = 1 )
    {
        ...
        $conf = json_decode(MT::getConfig()->getValue('base', 'default_avatar'), true);

        if ( isset($conf[$size]) )
        {
            $path = MT::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS . $conf[$size];

            return MT::getStorage()->getFileUrl($path);
        }
    ...
    public function setCustomDefaultAvatar( $size, $file )
    {
        ...
        $dir = MT::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS;
        ...
        if ( is_uploaded_file($file['tmp_name']) )
        {
            ...
            if ( $storage->copyFile($file['tmp_name'], $dir . $fileName) )
            {
                ...
                $conf[$size] = $fileName;
                MT::getConfig()->saveConfig('base', 'default_avatar', json_encode($conf));
    ...
    public function getAvatarUrlByAvatarDto( $avatar, $size = 1, $hash = null, $checkModerationStatus = true )
    {
        ...
        return MT::getStorage()->getFileUrl($dir . $avatarFile);
    }
    ...
    {
        $url = MT::getPluginManager()->getPlugin('base')->getUserFilesUrl() . 'avatars/tmp/';
    ...
    {
        return MT::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS;
    }

    public function getAvatarsPluginFilesDir()
    {
        return MT::getPluginManager()->getPlugin('base')->getPluginFilesDir() . 'avatars' . DS;
    }
    ...
    public function trackAvatarChangeActivity( $userId, $avatarId )
    {
        ...
        $event = new MT_Event('feed.action', array(
            ...
            'string' => MT::getLanguage()->text('base', 'avatar_feed_string'),
            /* 'content' => '<img src="' . $this->getAvatarUrl($userId) . '" />', */
            'view' => array(
                'iconClass' => 'mt_ic_picture'
            )
        ));
        MT::getEventManager()->trigger($event);
    }
    ...
    public function getAvatarChangeGalleryItem( $entityType, $entityId, $itemId )
    {
        ...
        $event = new MT_Event('base.avatar_change_get_item', $params);
    ...
    {
        $key = MT::getSession()->get(self::AVATAR_CHANGE_SESSION_KEY);
    ...
    public function setAvatarChangeSessionKey()
    {
        ...
        {
            $key = uniqid();
            MT::getSession()->set(self::AVATAR_CHANGE_SESSION_KEY, $key);
        }
    }
    ...
    public function createAvatar( $userId, $isModerable = true, $trackAction = true, $rotateAngle = null )
    {
        ...
        $event = new MT_Event('base.before_avatar_change', array(
            'userId' => $userId,
            'avatarId' => null,
            'upload' => true,
            'crop' => false,
        ...
        if ( $avatarSet )
        {
            ...
            if ( $avatar )
            {
                ...
                MT::getStorage()->copyFile($originalPath, $avatarOriginalPath);

                $event = new MT_Event('base.after_avatar_change', array(
                    'userId' => $userId,
                    'avatarId' => $avatar->id,
                    'upload' => true,
                    'crop' => false
