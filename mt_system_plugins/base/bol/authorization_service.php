...
class BOL_AuthorizationService
{
    ...
    public function isActionAuthorized( $groupName, $actionName = null, $extra = null )
    {
        if ( $extra !== null && !is_array($extra) )
        {
            trigger_error("`ownerId` parameter has been deprecated, pass `extra` parameter instead\n"
                . MT_ErrorManager::getInstance()->debugBacktrace(), E_USER_WARNING);
        }
        ...
        {
            $userId = MT::getUser()->isAuthenticated() ? MT::getUser()->getId() : 0;
        }
        ...
        {
            MT::getLogger()->addEntry($ex->getMessage() . "\n" . print_r($ex->getTrace(), true) );
        }
    ...
    {
        $userId = !empty($userId) ? $userId : (MT::getUser()->isAuthenticated() ? MT::getUser()->getId() : 0);
    ...
    public function isActionAuthorizedForUser( $userId, $groupName, $actionName = null )
    {
        ...
        if ( $userId > 0 || MT::getUser()->isAuthenticated() )
        {
            $userId = ( $userId > 0 ) ? $userId : MT::getUser()->getId();
...
//                        || isset($this->moderatorPermissionCache[$moderatorId][$adminGroupId])
    ...
    public function generateCaches() {
        ...
        {
            $this->userRolesCache[MT::getUser()->getId()] = $this->userRoleDao->getRoleIdList(MT::getUser()->getId());
        }
    ...
    {
        if ( $userId == null && ( $userId = MT::getUser()->getId() ) === null )
        {
            return false;
        }
    ...
    {
        return MT::getLanguage()->text('base', "authorization_role_{$roleName}");
    }
    ...
    public function saveUserRole( $userId, $roleId )
    {
        ...
        $event = new MT_Event('base.after_save_user_role', array('userId' => $userId, 'roleId' => $roleId));
        MT::getEventManager()->trigger($event);
    }
    ...
    public function deleteRoleById( $id )
    {
        ...
        $eventBefore = new MT_Event(self::ON_BEFORE_ROLE_DELETE, array('roleId' => $role->getId()));
        MT::getEventManager()->trigger($eventBefore);
        ...
        $eventAfter = new MT_Event(self::ON_AFTER_ROLE_DELETE, array('roleId' => $role->getId()));
        MT::getEventManager()->trigger($eventAfter);
    }
    ...
    public function deleteUserRole( $userId, $roleId )
    {
        ...
        $event = new MT_Event('base.after_delete_user_role', array('userId' => $userId, 'roleId' => $roleId));
        MT::getEventManager()->trigger($event);
    }
