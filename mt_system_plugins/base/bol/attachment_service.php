...
final class BOL_AttachmentService
{
    ...
    public function deleteExpiredTempImages()
    {
        ...
        foreach ( $attachList as $item )
        {
            ...
            if ( MT::getStorage()->fileExists($filePath) )
            {
                MT::getStorage()->removeFile($filePath);
            }
...
//        if ( $attch != NULL )
//        {
//            if ( MT::getStorage()->fileExists($this->getAttachmentsDir() . $attch->getFileName()) )
//            {
//                MT::getStorage()->removeFile($this->getAttachmentsDir() . $attch->getFileName());
//            }
//
...
//        else
//        {
//            if ( MT::getStorage()->fileExists($this->getAttachmentsDir() . basename($url)) )
//            {
//                MT::getStorage()->removeFile($this->getAttachmentsDir() . basename($url));
//            }
//        }
//    }
...
//
//        if ( MT::getUser()->isAuthenticated() && file_exists($filePath) )
//        {
//            MT::getStorage()->copyFile($filePath, $this->getAttachmentsDir() . $attch->getFileName());
//            unlink($filePath);
//        }
//
...
//
//        return MT::getStorage()->getFileUrl($this->getAttachmentsDir() . $attch->getFileName());
//    }
    /*
    ...
    {
        return $this->processUploadedFile($pluginKey, $fileInfo, $bundle, array('jpeg', 'jpg', 'png', 'gif'), (float) MT::getConfig()->getValue('base', 'tf_max_pic_size') * 1024);
    }
    /* attachments 1.6.1 update */

//    public function getAttachmentsTempUrl()
//    {
//        return MT::getPluginManager()->getPlugin('base')->getUserFilesUrl() . 'attachments/temp/';
//    }
//
//    public function getAttachmentsTempDir()
//    {
//        return MT::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'attachments' . DS . 'temp' . DS;
//    }

    public function getAttachmentsUrl()
    {
        return MT::getStorage()->getFileUrl(MT::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'attachments') . '/';
    }

    public function getAttachmentsDir()
    {
        return MT::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'attachments' . DS;
    }
    ...
    public function processUploadedFile( $pluginKey, array $fileInfo, $bundle = null, $validFileExtensions = array(), $maxUploadSize = null, $dimensions = null )
    {
        ...
        {
            $validFileExtensions = json_decode(MT::getConfig()->getValue('base', 'attch_ext_list'), true);
        }

        if ( $maxUploadSize === null )
        {
            $maxUploadSize = MT::getConfig()->getValue('base', 'attch_file_max_size_mb');
        }
        ...
        $attachDto->setUserId(MT::getUser()->getId());
        ...
        MT::getStorage()->copyFile($tempPath, $uploadPath);
        MT::getStorage()->chmod($uploadPath, 0666);
        unlink($tempPath);
    ...
    public function deleteAttachment( $userId, $attachId )
    {
        ...
        if ( $attach !== null && MT::getUser()->isAuthenticated() && MT::getUser()->getId() == $attach->getUserId() )
        {
            MT::getStorage()->removeFile($this->getAttachmentsDir() . $attach->getFileName());
