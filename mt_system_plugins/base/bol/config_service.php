...
final class BOL_ConfigService
{
    ...
    public function addConfig( $key, $name, $value, $description = null )
    {
        ...
        $event = MT::getEventManager()->trigger(new MT_Event(self::EVENT_BEFORE_SAVE, array(
            "key" => $key,
            "name" => $name,
            "value" => $value,
            "oldValue" => null
    ...
    public function saveConfig( $key, $name, $value )
    {
        ...
        MT::getEventManager()->trigger(new MT_Event(self::EVENT_AFTER_SAVE, array(
            "key" => $key,
            "name" => $name,
            "value" => $value,
            "oldValue" => $config->getValue()
    ...
    {
        $event = MT::getEventManager()->trigger(new MT_Event(self::EVENT_BEFORE_REMOVE, array(
            "key" => $key,
            "name" => $name
        )));
        ...
        if ( $event->getData() !== false )
        {
            ...
            MT::getEventManager()->trigger(new MT_Event(self::EVENT_BEFORE_REMOVE, array(
                "key" => $key,
                "name" => $name
            )));
        }
