...
class BOL_PluginService
{
    ...
    public function getPluginsXmlInfo()
    {
        ...
        $xmlFiles = UTIL_File::findFiles(MT_DIR_PLUGIN, array("xml"), 1);
    ...
    {
        $files = UTIL_File::findFiles(MT_DIR_SYSTEM_PLUGIN, array("xml"), 1);
        $pluginData = array();
...
// install plugins list
    ...
    public function install( $key, $generateCache = true )
    {
        ...
        // trigger event
        MT::getEventManager()->trigger(new MT_Event(MT_EventManager::ON_AFTER_PLUGIN_INSTALL,
    ...
    public function addPluginDirs( BOL_Plugin $pluginDto )
    {
        ...
        {
            MT::getStorage()->copyDir($plugin->getInnerUserFilesDir(), $plugin->getUserFilesDir());
        }
        else if ( !file_exists($plugin->getUserFilesDir()) )
        {
            MT::getStorage()->mkdir($plugin->getUserFilesDir());
        }
    }
    ...
    public function uninstall( $pluginKey )
    {
        ...
        // trigger event
        MT::getEventManager()->trigger(new MT_Event(MT_EventManager::ON_BEFORE_PLUGIN_UNINSTALL,
            array("pluginKey" => $pluginDto->getKey())));
        ...
        MT::getConfig()->deletePluginConfigs($pluginDto->getKey());
        ...
        // drop plugin tables
        $tables = MT::getDbo()->queryForColumnList("SHOW TABLES LIKE '" . str_replace('_', '\_', MT_DB_PREFIX) . $pluginDto->getKey() . "\_%'");
        ...
        // trigger event
        MT::getEventManager()->trigger(new MT_Event(MT_EventManager::ON_AFTER_PLUGIN_UNINSTALL,
            array("pluginKey" => $pluginDto->getKey())));
    }
    ...
    public function activate( $key )
    {
        ...
        $pluginDto->setIsActive(true);
        $this->pluginDao->save($pluginDto);
        MT::getPluginManager()->addPackagePointers($pluginDto);

        $this->includeScript(MT_DIR_PLUGIN . $pluginDto->getModule() . DS . self::SCRIPT_ACTIVATE);

        $this->updatePluginListCache();

        // trigger event
        $event = new MT_Event(MT_EventManager::ON_AFTER_PLUGIN_ACTIVATE, array("pluginKey" => $pluginDto->getKey()));
    ...
    public function deactivate( $key )
    {
        ...
        // trigger event
        $event = new MT_Event(MT_EventManager::ON_BEFORE_PLUGIN_DEACTIVATE, array("pluginKey" => $pluginDto->getKey()));
        MT::getEventManager()->trigger($event);

        $pluginDto->setIsActive(false);
        $this->pluginDao->save($pluginDto);

        $this->includeScript(MT::getPluginManager()->getPlugin($pluginDto->getKey())->getRootDir() . self::SCRIPT_DEACTIVATE);

        $this->updatePluginListCache();

        $event = new MT_Event(MT_EventManager::ON_AFTER_PLUGIN_DEACTIVATE, array("pluginKey" => $pluginDto->getKey()));
        MT::getEventManager()->trigger($event);
    }
    ...
    {
        $timestamp = MT::getConfig()->getValue("base", "check_mupdates_ts");
        ...
        MT::getConfig()->saveConfig("base", "check_mupdates_ts", time());
    }
