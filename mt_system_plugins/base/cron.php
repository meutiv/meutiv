 ...
 */
class BASE_Cron extends MT_Cron
{
    ...
    public function generateSitemap()
    {
        ...
        $inProgress = (int) MT::getConfig()->getValue('base', 'seo_sitemap_in_progress');
        $inProgressTime = (int) MT::getConfig()->getValue('base', 'seo_sitemap_in_progress_time');

        // is it possible to start sitemap generating?
        if ( (!$inProgress || time() - $inProgressTime >= 3600) && $service->isSitemapReadyForNextBuild() )
        {
            MT::getConfig()->saveConfig('base', 'seo_sitemap_in_progress', 1);
            MT::getConfig()->saveConfig('base', 'seo_sitemap_in_progress_time', time());

            $service->generateSitemap();

            MT::getConfig()->saveConfig('base', 'seo_sitemap_in_progress', 0);
        }
    }
    ...
    {
        if ( MT::getConfig()->configExists('base', 'cron_is_active') && (int) MT::getConfig()->getValue('base', 'cron_is_active') === 0 )
        {
            MT::getConfig()->saveConfig('base', 'cron_is_active', 1);
        }
    }
    ...
    {
        MT::getCacheManager()->clean(array(), MT_CacheManager::CLEAN_OLD);
    }
    ...
    public function dropLogFile()
    {
        ...
        if ( file_exists($logFilePath) )
        {
            ...
            if ( $logFileSize !== false && $logFileSize / 1024 / 1024 >= (int) MT::getConfig()->getValue('base', 'log_file_max_size_mb') )
            {
                unlink($logFilePath);
            }
        }
