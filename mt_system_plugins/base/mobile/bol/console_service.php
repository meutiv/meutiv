 ...
 * @package mt_system_plugins.base.mobile.bol
 * @since 1.6.0
 */
class MBOL_ConsoleService
{
    ...
    public function getPageSections( $page )
    {
        ...
        foreach ( $items as $item )
        {
            ...
            /** @var $cmp MT_MobileComponent */
            if ( is_subclass_of($cmp, 'MT_Renderable') && $cmp->isVisible() )
            {
                $sections[] = array(
                    'item' => $cmp->render(),
                    'order' => $order
    ...
    public function getNewItems( $page, $timestamp )
    {
        ...
        if ( $items )
        {
            foreach ( $items as $item )
            {
                ...
                if ( is_subclass_of($cmp, 'MT_Renderable') )
                {
