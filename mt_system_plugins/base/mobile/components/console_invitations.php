 ...
 * @since 1.6.0
 */
class BASE_MCMP_ConsoleInvitations extends MT_MobileComponent
{
    /**
     * Constructor.
     */
    ...
    public static function prepareData( $invitations )
    {
        ...
        foreach ( $invitations as $invitation )
        {
            ...
            $itemEvent = new MT_Event('mobile.invitations.on_item_render', array(
            ...
            if ( !empty($item['string']) && is_array($item['string']) )
            {
                ...
                $item['string'] = MT::getLanguage()->text($key[0], $key[1], $vars);
                if ( $disabled )
                {
