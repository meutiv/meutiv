 ...
 * @since 1.0
 */
class BASE_MCMP_CommentsForm extends MT_MobileComponent
{
    ...
    public function __construct( BASE_CommentsParams $params, $id, $formName )
    {
        ...
        $form->setAjax(true);
        $form->setAction(MT::getRouter()->urlFor('BASE_CTRL_Comments', 'addComment'));
...
//        $form->bindJsFunction(Form::BIND_SUCCESS, "function(){ $('#comments-" . $id . " .comments-preloader').hide();}");
        $this->addForm($form);
        MT::getDocument()->addOnloadScript("window.owCommentCmps['$id'].initForm('" . $textArea->getId() . "', '".$submit->getId()."');");
