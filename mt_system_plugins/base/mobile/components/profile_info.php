...
class BASE_MCMP_ProfileInfo extends MT_MobileComponent
{
    /**
     *
     * @var BOL_User
    ...
    public function onBeforeRender() 
    {
        ...
        if ( $this->previewMode )
        {
            ...
            {
                if ( $question["name"] == MT::getConfig()->getValue('base', 'display_name_question') )
                {
                    continue;
                }
        ...
        $questions = BASE_CMP_UserViewWidget::getUserViewQuestions($this->user->id, MT::getUser()->isAdmin(), reset($questionNames));
        ...
        $event = new MT_Event('mbase.profile_on_collect_question_data', array('profileUserId' => $this->user->getId()), $questions);
