 ...
 * @package mt_system_plugins.base.mobile.controllers
 * @since 1.8.6
 */
class BASE_MCTRL_Edit extends BASE_CTRL_Edit
{
    ...
    public function index( $params )
    {
        ...
        // add avatar field
        $avatarValidator = MT::getClassInstance("BASE_MCLASS_EditAvatarFieldValidator", false);
        ...
        if ($displayPhotoUpload == BOL_UserService::CONFIG_JOIN_DISPLAY_AND_SET_REQUIRED_PHOTO_UPLOAD) {
            $avatarValidator = MT::getClassInstance("BASE_MCLASS_EditAvatarFieldValidator", true, $userId);
        }
        ...
        $onLoadJs = " window.edit = new MT_BaseFieldValidators( " .
            json_encode(array(
                ...
                'responderUrl' => MT::getRouter()->urlFor("BASE_MCTRL_Edit", "ajaxResponder"))) . ",
        ...
        $this->setTemplate(MT::getPluginManager()->getPlugin('base')->getMobileCtrlViewDir() . 'edit_index.html');
    }
        ...
        if ( $editForm->isValid($_POST) )
        {
            ...
            if ( !empty($userId) )
            {
                ...
                if ( empty($_POST['userPhoto']) )
                {
                    if ( $displayPhotoUpload == BOL_UserService::CONFIG_JOIN_DISPLAY_AND_SET_REQUIRED_PHOTO_UPLOAD )
                    {
                        MT::getFeedback()->error($language->text('mobile', 'avatar_cannot_be_deleted'));
                ...
                if ( !empty($_FILES['userPhoto']['tmp_name']) )
                {
                    ...
                    {
                        MT::getFeedback()->error($language->text('mobile', 'wrong_avatar_format'));
