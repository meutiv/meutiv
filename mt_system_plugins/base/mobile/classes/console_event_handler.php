 ...
 * @package ow.mt_system_plugins.base.mobile.classes
 * @since 1.6.0
 */
class BASE_MCLASS_ConsoleEventHandler
{
    ...
    public function countNewInvitations( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( $params['page'] == self::CONSOLE_NOTIFICATIONS_PAGE_KEY )
        {
            ...
            $event->add(
                array(self::CONSOLE_INVITATIONS_SECTION_KEY => $service->findInvitationCount(MT::getUser()->getId(), false))
            );
        }
    }
    ...
    public function ping( MT_Event $e )
    {
        $params = $e->getParams();
        $state = $params['state'];
