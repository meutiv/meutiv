...
class BASE_MCLASS_EventHandler extends BASE_CLASS_EventHandler
{
    ...
    public function init()
    {
        ...
        $eventManager->bind(MT_EventManager::ON_BEFORE_DOCUMENT_RENDER, array($this, 'onDocRenderAddJsDeclarations'));
        ...
        $eventManager->bind(MT_EventManager::ON_PLUGINS_INIT, array($this, 'onPluginsInitCheckUserStatus'));
    ...
    public function onCollectButtonList( BASE_CLASS_EventCollector $e )
    {
        ...
        $e->add(array('iconClass' => 'mt_ico_signin_f', 'markup' => $button->render()));
    }
    ...
    public function onMobileProfileContentMenu( BASE_CLASS_EventCollector $event )
    {
        ...
        $url = MT::getRouter()->urlForRoute('base_about_profile', array('username' => $userName));
        $resultArray = array(
    ...
    public function onDocRenderAddJsDeclarations( $e )
    {
        ...
        $scriptGen = UTIL_JsGenerator::newInstance()->setVariable(
                array('OWM', 'ajaxComponentLoaderRsp'), MT::getRouter()->urlFor('BASE_MCTRL_AjaxLoader', 'component')
        );

        //UsersApi
        $scriptGen->newObject(array('MT', 'Users'), 'OWM_UsersApi', array(array(
        ...
        {
            MT::getLanguage()->addKeyForJs('base', 'mobile_disabled_item_message');
            $params = array(
                'pages' => MBOL_ConsoleService::getInstance()->getPages(),
                'rspUrl' => MT::getRouter()->urlFor('BASE_MCTRL_Ping', 'index'),
                'lastFetchTime' => time(),
                'pingInterval' => 10,
                'desktopUrl' => MT::getRouter()->urlForRoute('base.desktop_version')
            );
    ...
    public function onActionToolbarAddUserBlockActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $resultArray["label"] = $isBlocked ? MT::getLanguage()->text('base', 'user_unblock_btn_lbl') : MT::getLanguage()->text('base', 'user_block_btn_lbl');
        ...
        $js->jQueryEvent("#" . $uniqId, "click",
            ...
            "msg" => strip_tags(MT::getLanguage()->text("base", "user_block_confirm_message"))
        ));
    ...
    public function onActionToolbarAddUserFeatureActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            "group" => 'addition',
            "label" => MT::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
            "order" => 2
        );
        ...
        $action["label"] = $isFeatured ? MT::getLanguage()->text('base', 'user_action_unmark_as_featured') : MT::getLanguage()->text('base', 'user_action_mark_as_featured');
    ...
    public function onActionToolbarAddUserApproveActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            "group" => 'addition',
            "label" => MT::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
            "href" => MT::getRouter()->urlFor('BASE_CTRL_User', 'approve', array('userId' => $userId)),
            "label" => MT::getLanguage()->text('base', 'profile_toolbar_user_approve_label'),
            "class" => '',
    ...
    public function onActionToolbarAddSuspendActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action["attributes"] = array();
        $action["label"] = $suspended ? MT::getLanguage()->text('base', 'user_unsuspend_btn_lbl') : MT::getLanguage()->text('base', 'user_suspend_btn_lbl');
    ...
    public function onActionToolbarAddEditUserActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( MT::getUser()->getId() == $params['userId']
            || (MT::getUser()->isAuthorized('base') && !BOL_AuthorizationService::getInstance()->isSuperModerator($params['userId'])) )
        {
            $resultArray = array(
                "label" => MT::getUser()->getId() == $params['userId']
                    ? MT::getLanguage()->text('mobile', 'edit_profile')
                    : MT::getLanguage()->text('mobile', 'edit_other_profile'),
                "href" => MT::getUser()->getId() == $params['userId']
                    ? MT::getRouter()->urlForRoute('base_edit')
                    : MT::getRouter()->urlForRoute('base_edit_user_datails', array('userId' => $params['userId']))
            );
    ...
    public function onActionToolbarAddDeleteActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $confirmMsg = MT::getLanguage()->text('base', 'are_you_sure');
        $callbackUrl = MT::getRouter()->urlFor('BASE_MCTRL_User', 'userDeleted');
        ...
        $resultArray = array(
            "label" => MT::getLanguage()->text('base', 'profile_toolbar_user_delete_label'),
            "class" => 'owm_red_btn',
            "href" => 'javascript://',
            "id" => $linkId,
            "group" => 'addition',
    ...
    public function onPluginsInitCheckUserStatus()
    {
        if ( MT::getUser()->isAuthenticated() )
        {
            ...
            $signOutDispatchAttrs = MT::getRouter()->getRoute('base_sign_out')->getDispatchAttrs();
            ...
            if ( MT::getConfig()->getValue('base', 'mandatory_user_approve') && !BOL_UserService::getInstance()->isApproved() )
            {
                MT::getRequestHandler()->setCatchAllRequestsAttributes('base.wait_for_approval', array('controller' => 'BASE_MCTRL_WaitForApproval', 'action' => 'index'));
                MT::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                MT::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', 'BASE_MCTRL_AjaxLoader', 'component');
                MT::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', 'BASE_MCTRL_Invitations', 'command');
                MT::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', 'BASE_MCTRL_Ping', 'index');
            }
            ...
            if ( $user !== null )
            {
                ...
                {
                    MT::getRequestHandler()->setCatchAllRequestsAttributes('base.suspended_user', array('controller' => 'BASE_MCTRL_SuspendedUser', 'action' => 'index'));
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_MCTRL_AjaxLoader');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_MCTRL_Invitations', 'command');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_MCTRL_Ping', 'index');
                }

                if ( (int) $user->emailVerify === 0 && MT::getConfig()->getValue('base', 'confirm_email') )
                {
                    MT::getRequestHandler()->setCatchAllRequestsAttributes('base.email_verify', array(MT_RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => 'BASE_MCTRL_EmailVerify', MT_RequestHandler::CATCH_ALL_REQUEST_KEY_ACTION => 'index'));

                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.email_verify', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.email_verify', 'BASE_MCTRL_EmailVerify');
                }
                ...
                {
                    MT::getRequestHandler()->setCatchAllRequestsAttributes('base.complete_profile.account_type', array('controller' => 'BASE_MCTRL_CompleteProfile', 'action' => 'fillAccountType'));
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_MCTRL_AjaxLoader');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_MCTRL_Invitations');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_MCTRL_Ping');
                ...
                else
                {
                    ...
                    if ( $questionsEditStamp >= (int) $updateDetailsStamp )
                    {
                        ...
                        {
                            MT::getRequestHandler()->setCatchAllRequestsAttributes('base.complete_profile', array('controller' => 'BASE_MCTRL_CompleteProfile', 'action' => 'fillRequiredQuestions'));
                            MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                            MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_MCTRL_AjaxLoader');
                            MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_MCTRL_Invitations');
                            MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_MCTRL_Ping');
                        }
                        else
                        {
    ...
    public function onNotificationRender( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function getUserListFields( MT_Event $e )
    {
        $params = $e->getParams();
            ...
            switch( $list )
            {
                case "birthdays":
                    if ( !empty($questionsData['birthdate']) && $list == 'birthdays'  )
                    {
                        ...
                        {
                            $birthday= MT::getLanguage()->text('base', 'date_time_tomorrow');
                            $birthdate = '<a href="#" class="mt_lbutton mt_green">' . $birthday . '</a>';
                        }
                        else if ( intval(date('d')) == intval($dinfo['day']) )
                        {
                            $birthday = MT::getLanguage()->text('base', 'date_time_today');
                            $birthdate = '<a href="#" class="mt_lbutton mt_green">' . $birthday . '</a>';
                        }
                        else
                        {
                        ...
                        $data[$userId][] = MT::getLanguage()->text('birthdays', 'birthday') . ":" . $birthdate;
                    }
                break;
            }
        }
