...
class BASE_Import extends DATAIMPORTER_CLASS_Import
{
    ...
    public function import( $params )
    {
        ...
        if ( MT::getPluginManager()->getPlugin('dataimporter') )
        {
            if ( file_exists(MT::getPluginManager()->getPlugin('dataimporter')->getRootDir() . 'langs.zip') )
            {
                MT::getLanguage()->importPluginLangs(MT::getPluginManager()->getPlugin('dataimporter')->getRootDir() . 'langs.zip', 'dataimporter');
            }
        }

        $languageId = MT::getLanguage()->getCurrentId();
        BOL_LanguageService::getInstance()->generateCache($languageId);

        MT::getDbo()->query( "TRUNCATE " . MT_DB_PREFIX . 'base_component_place_cache' ); //TODO: Use service function

        // ADD MENU ITEMS add plugin and add theme
        try
        {
            MT::getDbo()->query( "INSERT IGNORE INTO `" . MT_DB_PREFIX ."base_menu_item` ( `prefix`, `key`, `documentKey`, `type`, `order`, `routePath`, `externalUrl`, `newWindow`, `visibleFor`) VALUES ( 'admin', 'sidebar_menu_plugins_add', '', 'admin_plugins', 3, 'admin_plugins_add', NULL, 0, 2) ");
        }
        catch( Exception $ex )
        {
        ...
        try
        {
            MT::getDbo()->query( "INSERT IGNORE INTO `" . MT_DB_PREFIX ."base_menu_item` ( `prefix`, `key`, `documentKey`, `type`, `order`, `routePath`, `externalUrl`, `newWindow`, `visibleFor`) VALUES ( 'admin', 'sidebar_menu_themes_add', '', 'admin_appearance', 3, 'admin_themes_add_new', NULL, 0, 3) ");
        }
        catch( Exception $ex )
        {
        ...
        while ( true )
        {
            ...
            foreach ( $list as $user )
            {
                for ( $size = 1; $size < 4; $size++ )
                {
                    ...
                    if ( !empty($content) )
                    {
                        MT::getStorage()->fileSetContent($path, $content);
                    }
                }
            }
        }
    ...
    private function importTheme( $importDir )
    {
        ...
        {
            MT::getStorage()->copyDir($importDir . ($theme->name) . DS, BOL_ThemeService::getInstance()->getRootDir($theme->name));
        }

        MT::getStorage()->copyDir($importDir . 'themes' . DS, MT_DIR_THEME_USERFILES);
    ...
    private function importMediaPanelFiles()
    {
        ...
        foreach ($list as $dto)/*@var $dto BOL_MediaPanelFile*/
        {
            ...
            {
                MT::getStorage()->fileSetContent(MT::getPluginManager()->getPlugin('base')->getUserFilesDir().$filename, $fileContent);
            }
        }
    }
}
