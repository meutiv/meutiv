...
class BASE_Export extends DATAEXPORTER_CLASS_Export
{
    ...
    {
        return array(
            MT_DB_PREFIX . 'base_config',
            MT_DB_PREFIX . 'base_theme',
            MT_DB_PREFIX . 'base_plugin',
            MT_DB_PREFIX . 'base_theme_control',
            MT_DB_PREFIX . 'base_theme_control_value',
            MT_DB_PREFIX . 'base_component_place_cache'
        );
    }
    ...
    public function export( $params )
    {
        ...
        $this->configs['media_panel_url'] = MT::getStorage()->getFileUrl(MT::getPluginManager()->getPlugin('base')->getUserFilesDir());
    ...
    {
        $this->configs['avatarUrl'] = MT::getStorage()->getFileUrl(BOL_AvatarService::getInstance()->getAvatarsDir());

        $tableName = MT::getDbo()->escapeString(str_replace(MT_DB_PREFIX, '%%TBL-PREFIX%%', BOL_ConfigDao::getInstance()->getTableName()));
        ...
        $sql = DATAEXPORTER_BOL_ExportService::getInstance()->exportTableToSql(MT_DB_PREFIX . 'base_config', false, false, true, $query);
    ...
    {
        $currentTheme = MT::getThemeManager()->getSelectedTheme()->getDto();
        $currentThemeDir = MT::getThemeManager()->getSelectedTheme()->getRootDir();
        $currentThemeUserfilesDir = MT_DIR_THEME_USERFILES;
        ...
        $controlValueList = MT::getDbo()->queryForList(" SELECT * FROM " . BOL_ThemeControlValueDao::getInstance()->getTableName() . " WHERE themeId = :themeId ", array('themeId' => $currentTheme->id));
        ...
        UTIL_File::copyDir(MT_DIR_THEME_USERFILES, $themesDir);

        $fileList = Ow::getStorage()->getFileNameList(MT_DIR_THEME_USERFILES);
        ...
        foreach($fileList as $file)
        {
            ...
            {
                Ow::getStorage()->copyFileToLocalFS($file, $themesDir . mb_substr($file, mb_strlen(MT_DIR_THEME_USERFILES)));
            }
        }
