 ...
 * @since 1.0
 */
class BASE_CTRL_Comments extends MT_ActionController
{
    /**
    ...
    public function addComment()
    {
        ...
        {
            $errorMessage = MT::getLanguage()->text('base', 'comment_required_validator_message');
        }
        else if ( !MT::getUser()->isAuthorized($params->getPluginKey(), 'add_comment') )
        {
            $status = BOL_AuthorizationService::getInstance()->getActionStatus($params->getPluginKey(), 'add_comment');
            $errorMessage = $status['msg'];
        }
        else if ( BOL_UserService::getInstance()->isBlocked(MT::getUser()->getId(), $params->getOwnerId()) )
        {
            $errorMessage = MT::getLanguage()->text('base', 'user_block_message');
        }
        ...
        if ( BOL_TextFormatService::getInstance()->isCommentsRichMediaAllowed() && !$isMobile )
        {
            if ( !empty($_POST['attachmentInfo']) )
            {
                ...
                MT::getEventManager()->call('base.attachment_save_image', array('uid' => $tempArr['uid'], 'pluginKey' => $tempArr['pluginKey']));
        ...
        $comment = $this->commentService->addComment($params->getEntityType(), $params->getEntityId(), $params->getPluginKey(), MT::getUser()->getId(), $commentText, $attachment);

        // trigger event comment add
        $event = new MT_Event('base_add_comment', array(
        ...
        exit(json_encode(array(
            ...
            'onloadScript' => MT::getDocument()->getOnloadScript(),
    ...
    public function getMobileCommentList()
    {
        ...
        exit(json_encode(array('onloadScript' => MT::getDocument()->getOnloadScript(), 'commentList' => $commentsList->render())));
    }
    ...
    public function deleteComment()
    {
        ...
        $event = new MT_Event('base_delete_comment', array(
    ...
    private function getCommentInfoForDelete()
    {
        ...
        {
            echo json_encode(array('error' => MT::getLanguage()->text('base', 'comment_ajax_error')));
            exit();
        }
        ...
        $isModerator = MT::getUser()->isAuthorized($params->getPluginKey());
        $isOwnerAuthorized = (MT::getUser()->isAuthenticated() && $params->getOwnerId() !== null && (int) $params->getOwnerId() === (int) MT::getUser()->getId());
        $commentOwner = ( (int) MT::getUser()->getId() === (int) $comment->getUserId() );

        if ( !$isModerator && !$isOwnerAuthorized && !$commentOwner )
        {
            echo json_encode(array('error' => MT::getLanguage()->text('base', 'auth_ajax_error')));
            exit();
        }
    ...
    private function getParamsObject()
    {
        ...
        {
            $errorMessage = MT::getLanguage()->text('base', 'comment_ajax_error');
        }
