 ...
 * @since 1.0
 */
class BASE_CTRL_Rate extends MT_ActionController
{
    ...
    public function updateRate()
    {
        ...
        {
            exit(json_encode(array('errorMessage' => MT::getLanguage()->text('base', 'rate_cmp_auth_error_message'))));
        }

        if ( $userId === $ownerId )
        {
            exit(json_encode(array('errorMessage' => MT::getLanguage()->text('base', 'rate_cmp_owner_cant_rate_error_message'))));
        }
        ...
        if ( BOL_UserService::getInstance()->isBlocked(MT::getUser()->getId(), $ownerId) )
        {
            exit(json_encode(array('errorMessage' => MT::getLanguage()->text('base', 'user_block_message'))));
        }
        ...
        exit(json_encode(array('totalScoreCmp' => $totalScoreCmp->render(), 'message' => MT::getLanguage()->text('base', 'rate_cmp_success_message'))));
    }
