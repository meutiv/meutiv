...
class BASE_CTRL_Moderation extends MT_ActionController
{
    ...
    {
        $this->setPageTitle(MT::getLanguage()->text("base", "moderation_tools"));
        $this->setPageHeading(MT::getLanguage()->text("base", "moderation_tools"));
    ...
    public function flags( $params )
    {
        ...
        if ( $menu === null )
        {
            $this->redirect(MT::getRouter()->urlForRoute("base.moderation_tools"));
        }
        ...
        {
            $this->redirect(MT::getRouter()->urlForRoute("base.moderation_flags_index"));
        }
        ...
        foreach ($flags as $flag )
        {
            ...
            $tplFlags[] = array(
                ...
                "string" => MT::getLanguage()->text("base", "moderation_flags_item_string", array(
                    "userName" => $userName,
                    "userUrl" => $userUrl,
        ...
        $this->assign("responderUrl", MT::getRouter()->urlFor(__CLASS__, "flagsResponder", array(
            "group" => $currentGroup["name"]
        )));
        
        MT::getLanguage()->addKeyForJs("base", "are_you_sure");
        MT::getLanguage()->addKeyForJs("base", "moderation_delete_confirmation");
        MT::getLanguage()->addKeyForJs("base", "moderation_delete_multiple_confirmation");
        MT::getLanguage()->addKeyForJs("base", "moderation_no_items_warning");
    ...
    public function flagsResponder( $params )
    {
        if ( !MT::getRequest()->isPost() || 
                !(MT::getUser()->isAdmin() || BOL_AuthorizationService::getInstance()->isModerator()) )
        {
            throw new Redirect403Exception;
        }
        ...
        $backUrl = MT::getRouter()->urlForRoute("base.moderation_flags", array(
            "group" => $params["group"]
        ));
        ...
        if ( empty($itemKeys) )
        {
            MT::getFeedback()->warning(MT::getLanguage()->text("base", "moderation_no_items_warning"));
        ...
        MT::getFeedback()->info(MT::getLanguage()
