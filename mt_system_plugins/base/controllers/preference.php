 ...
 * @since 1.0
 */
class BASE_CTRL_Preference extends MT_ActionController
{
    ...
    public function index( $params )
    {
        ...
        $preferenceSubmit->addAttribute('class', 'mt_button mt_ic_save');
        ...
        MT::getEventManager()->trigger($formElementEvent);
        ...
        if ( MT::getRequest()->isPost() )
        {
            if( $preferenceForm->isValid($_POST) )
            {
                ...
                if ( $result )
                {
                    MT::getFeedback()->info($language->text('base', 'preference_preference_data_was_saved'));
                }
                else
                {
                    MT::getFeedback()->warning($language->text('base', 'preference_preference_data_not_changed'));
                }
        ...
        MT::getEventManager()->trigger($sectionLabelEvent);
