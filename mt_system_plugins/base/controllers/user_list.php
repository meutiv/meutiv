 ...
 * @since 1.0
 */
class BASE_CTRL_UserList extends MT_ActionController
{
    ...
    public function __construct()
    {
        ...
        $this->setPageHeading(MT::getLanguage()->text('base', 'users_browse_page_heading'));
        $this->setPageHeadingIconClass('mt_ic_user');
        $this->usersPerPage = (int)MT::getConfig()->getValue('base', 'users_count_on_page');
    ...
    public function index( $params )
    {
        ...
        $cmp = MT::getClassInstance("BASE_Members", $list, $itemCount, $this->usersPerPage, true, $listType);
    ...
    public function blocked()
    {
        ...
        if ( !MT::getUser()->isAuthenticated() || $userId === null )
        {
            throw new AuthenticateException();
        }

        // unblock action
        if ( MT::getRequest()->isPost() && !empty($_POST['userId']) )
        {
        ...
        // set page settings
        $this->setPageHeading(MT::getLanguage()->text('base', 'blocked_users_browse_page_heading'));
        $this->setPageTitle(MT::getLanguage()->text('base', 'blocked_users_browse_page_heading'));
    }

    public function forApproval()
    {
        $this->setTemplate(MT::getPluginManager()->getPlugin('base')->getCtrlViewDir() . 'user_list_index.html');
        ...
        $cmp = MT::getClassInstance("BASE_Members", $list, $itemCount, $this->usersPerPage, false, 'waiting-for-approval');
    ...
    public static function getMenu( $activeListType )
    {
        ...
        $menuArray = array(
            array(
                ...
                'url' => MT::getRouter()->urlForRoute('base_user_lists', array('list' => 'latest')),
            ...
            array(
                ...
                'url' => MT::getRouter()->urlForRoute('base_user_lists', array('list' => 'online')),
                'iconClass' => 'mt_ic_push_pin',
                'key' => 'online',
                'order' => 3
            ),
            array(
                'label' => $language->text('base', 'user_search_menu_item_label'),
                'url' => MT::getRouter()->urlForRoute('users-search'),
        ...
        if ( BOL_UserService::getInstance()->countFeatured() > 0 )
        {
            $menuArray[] =  array(
                ...
                'url' => MT::getRouter()->urlForRoute('base_user_lists', array('list' => 'featured')),
                'iconClass' => 'mt_ic_push_pin',
                'key' => 'featured',
                'order' => 2
            );
        ...
        foreach ( $questionList as $uid => $question )
        {
            ...
            if ( !empty($question['birthdate']) )
            {
                ...
                if ( $this->listKey == 'birthdays' )
                {
                    ...
                    {
                        $questionList[$uid]['birthday'] = MT::getLanguage()->text('base', 'date_time_tomorrow');

                        $birthdate = '<span class="mt_green" style="font-weight: bold; text-transform: uppercase;">' . $questionList[$uid]['birthday'] . '</a>';
                    }
                    else if ( intval(date('d')) == intval($dinfo['day']) )
                    {
                        $questionList[$uid]['birthday'] = MT::getLanguage()->text('base', 'date_time_today');

                        $birthdate = '<span class="mt_green" style="font-weight: bold; text-transform: uppercase;">' . $questionList[$uid]['birthday'] . '</span>';
                    }
                    else
                    {
                    ...
                    $fields[$uid][] = array(
                        'label' => MT::getLanguage()->text('birthdays', 'birthday'),
                        'value' => $birthdate
                    );
                }
            }
