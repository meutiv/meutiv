 ...
 * @since 1.0
 */
class BASE_CTRL_Avatar extends MT_ActionController
{
    ...
    public function ajaxResponder()
    {
        ...
        if ( isset($request['ajaxFunc']) && MT::getRequest()->isAjax() )
        {
    ...
    protected function ajaxCropPhoto( $params )
    {
        ...
        if ( !empty($params['entityType']) && !empty($params['id']) )
        {
            ...
            if ( !$item || empty($item['path']) || !MT::getStorage()->fileExists($item['path']) )
            {
        ...
        else if ( isset($params['url']) ) 
        {
            ...
            if ( !MT::getStorage()->fileExists($path)  )
            {
                if ( !file_exists($path) )
                {
        ...
        $userId = MT_Auth::getInstance()->getUserId();
    ...
    {
        if ( isset($params['avatarId']) && MT::getUser()->isAuthorized('base') )
        {
            $entityId = $params['avatarId'];
            $entityType = BASE_CLASS_ContentProvider::ENTITY_TYPE_AVATAR;

            $event = new MT_Event("moderation.approve", array(
