 ...
 * @since 1.3.2
 */
class BASE_CMP_ContextAction extends MT_Component
{
    const POSITION_LEFT = 'mt_tooltip_top_left';
    const POSITION_RIGHT = 'mt_tooltip_top_right';
    ...
    public function __construct( $position = self::POSITION_RIGHT )
    {
        ...
        $script = '$(document).on("hover", ".mt_context_action",function(e) {
                        if (e.type == "mouseenter") {
                            $(this).find(".mt_tooltip").css({opacity: 0, top: 10}).show().stop(true, true).animate({top: 18, opacity: 1}, "fast"); 
                        }
                        else { // mouseleave
                            $(this).find(".mt_tooltip").hide();  
                        }     
                    }
                );';
