 ...
 * @since 1.0
 */
class BASE_CMP_AvatarUserListSelect extends MT_Component
{
    /**
     * @var string
     */
    ...
    public function __construct( array $idList, $langs = array() )
    {
        ...
        $this->countLabel = MT::getLanguage()->text('base', 'avatar_user_list_select_count_label');
        $this->buttonLabel = MT::getLanguage()->text('base', 'avatar_user_list_select_button_label');
        $this->idList = $idList;
    ...
    public function onBeforeRender()
    {
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'avatar_user_select.js');
        MT::getDocument()->addOnloadScript("
            var cmp = new AvatarUserSelect(" . json_encode($jsArray) . ", '" . $contexId . "');
            cmp.init();  ");

        MT::getLanguage()->addKeyForJs('base', 'avatar_user_select_empty_list_message');
