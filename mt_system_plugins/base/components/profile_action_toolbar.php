 ...
 * @since 1.0
 */
class BASE_CMP_ProfileActionToolbar extends MT_Component
{
    /**
     * @deprecated constant
     */
    ...
    public function onBeforeRender()
    {
        ...
        $event = new MT_Event(self::EVENT_PROCESS_TOOLBAR, array('userId' => $this->userId), $event->getData());
    ...
    public function initToolbar( $items )
    {
        ...
        foreach ( $items as $item  )
        {
            ...
            if ( isset($item[self::DATA_KEY_CMP_CLASS]) )
            {
                ...
                $cmp = MT::getEventManager()->call('class.get_instance', array(
                    'className' => $cmpClass,
                    'arguments' => array(
        ...
        $moreGroup = array(
            "key" => "base.more",
            "label" => MT::getLanguage()->text("base", "more"),
            "toolbar" => $moreActions
        );
    ...
    public function getGroupMenu( $group )
    {
        ...
        $event = new MT_Event(self::EVENT_GROUP_MENU_CLASS, array("key" => $group["key"], "userId" => $this->userId), array(
            "mt_profile_toolbar_group",
            "mt_context_action_value_block"
        ));
    ...
    /*public function initToolbar( $items )
    {
        ...
        foreach ( $items as $item  )
        {
            ...
            if ( isset($item[self::DATA_KEY_CMP_CLASS]) )
            {
                ...
                $cmp = MT::getClassInstance($cmpClass, array(
                    'userId' => $this->userId
                ));
