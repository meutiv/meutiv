 ...
 * @since 1.0
 */
class BASE_CMP_TextFormatter extends MT_Component
{
    ...
    function __construct( $plugin, $elId, $dl, $dr, $controls, $template = null )
    {
        ...
        if ( $template == null )
        {
            $this->setTemplate(MT::getPluginManager()->getPlugin('base')->getCmpViewDir() . 'text_formatter.html');
        }

        $this->assign('plugin', $plugin);

        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery-fieldselection.js');
	...
	$('#{$ctrl['id']}').click(function(){
        ...
        MT::getDocument()->addOnloadScript($js, 100);

        $this->assign('controls', $controls);
        $this->assign('tid', $tid);


        MT::getDocument()->addScriptDeclaration("
