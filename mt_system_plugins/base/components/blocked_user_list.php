...
class BASE_CMP_BlockedUserList extends BASE_CMP_Users
{
    ...
    public function getContextMenu($userId)
    {
        ...
        $contextAction->setLabel(MT::getLanguage()->text('base', 'user_unblock_btn_lbl'));

        $url = MT::getRouter()->urlForRoute('users-blocked');
        $contextAction->setUrl('javascript://');
        $contextAction->addAttribute('onclick', "MT.postRequest('{$url}', {userId: {$userId}})");
