 ...
 * @since 1.0
 */
class BASE_CMP_DragAndDropItem extends MT_Component
{
    ...
    public function __construct( $componentUniqName, $isClone = false, $template = null, $sharedData = array() )
    {
        ...
        if ( $template !== null )
        {
            $this->setTemplate(MT::getPluginManager()->getPlugin('base')->getCmpViewDir() . $template . '.html');
        }
    ...
    {
        $settingList['shmt_title'] = $this->componentParamObject->standartParamList->showTitle;
    ...
    {
        $isUserAuthenticated = MT::getUser()->isAuthenticated();
        ...
        $userRoles = BOL_AuthorizationService::getInstance()->findUserRoleList(MT::getUser()->getId());
    ...
    public function renderView()
    {
        ...
        $contentComponent = MT::getClassInstance($className, $paramsObject);
