 ...
 * @since 1.0
 */
require_once MT_DIR_LIB . 'rss' . DS . 'rss.php';
...
class BASE_CMP_RssWidget extends BASE_CLASS_Widget
{
    ...
    public function __construct( BASE_CLASS_WidgetParameter $param )
    {
        ...
        $cachedState = MT::getCacheService()->get($cacheKey);

        if ( $cachedState === false )
        {
            try
            {
                $rssLoading = MT::getConfig()->getValue('base', 'rss_loading');
                ...
                {
                    MT::getConfig()->addConfig('base', 'rss_loading', time());
                }
                else
                {
                    MT::getConfig()->saveConfig('base', 'rss_loading', time());
                }

                $rssIterator = RssParcer::getIterator($param->customParamList['rss_url'], self::$countInterval[1]);

                MT::getConfig()->saveConfig('base', 'rss_loading', 0);
            }
            catch (Exception $e)
            {
                MT::getConfig()->saveConfig('base', 'rss_loading', 0);
            ...
            try
            {
                MT::getCacheService()->set($cacheKey, json_encode($this->rss), 60 * 60);
            }
            catch (Exception $e) {}
        }
        else
    ...
    public static function getSettingList()
    {
        ...
        $settingList['rss_url'] = array(
            ...
            'label' => MT::getLanguage()->text('base', 'rss_widget_url_label'),
            'value' => ''
        );

        $settingList['item_count'] = array(
            'presentation' => self::PRESENTATION_SELECT,
            'label' => MT::getLanguage()->text('base', 'rss_widget_count_label'),
            'value' => 5
        );
        ...
        $settingList['title_only'] = array(
            ...
            'label' => MT::getLanguage()->text('base', 'rss_widget_title_only_label'),
            'value' => false
        );
    ...
    public static function validateSettingList( $settingList )
    {
        ...
        {
            throw new WidgetSettingValidateException(MT::getLanguage()->text('base', 'rss_widget_url_invalid_msg'), 'rss_url');
        }
    }
    ...
    {
        return array(
            self::SETTING_TITLE => MT::getLanguage()->text('base', 'rss_widget_default_title'),
