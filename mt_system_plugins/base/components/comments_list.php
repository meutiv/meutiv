 ...
 * @since 1.0
 */
class BASE_CMP_CommentsList extends MT_Component
{
    /**
    ...
    public function __construct( BASE_CommentsParams $params, $id, $page = 1 )
    {
        ...
        $this->id = $id;
        $this->page = $page;
        $this->isModerator = MT::getUser()->isAuthorized($params->getPluginKey());
        $this->isOwnerAuthorized = (MT::getUser()->isAuthenticated() && $this->params->getOwnerId() !== null && (int) $this->params->getOwnerId() === (int) MT::getUser()->getId());
        $this->isBaseModerator = MT::getUser()->isAuthorized('base');
    ...
    protected function processList( $commentList )
    {
        ...
        foreach ( $commentList as $value )
        {
            ...
            if ( $value->getAttachment() !== null )
            {
                ...
                $contentAdd .= '<div class="mt_attachment mt_small" id="att' . $value->getId() . '">' . $tempCmp->render() . '</div>';
            }
    ...
    public function itemHandler( BASE_CLASS_EventProcessCommentItem $e )
    {
        ...
        if ( $this->isOwnerAuthorized || $this->isModerator || (int) MT::getUser()->getId() === (int) $value->getUserId() )
        {
            $deleteButton = true;
        }
        
        $flagButton = $value->getUserId() != MT::getUser()->getId();
        ...
        if ( $this->isBaseModerator || $deleteButton || $flagButton )
        {
            ...
            $parentAction->setClass('mt_comments_context');
            ...
            if ( $this->isBaseModerator && $value->getUserId() != MT::getUser()->getId() )
            {
            ...
            if ( $flagButton )
            {
                ...
                $flagAction->setParentKey($parentAction->getKey());
                $flagAction->addAttribute("onclick", "var d = $(this).data(); MT.flagContent(d.etype, d.eid);");
    ...
    protected function init()
    {
        ...
        MT::getEventManager()->trigger(new MT_Event('base.comment_list_prepare_data', array('list' => $commentList, 'entityType' => $this->params->getEntityType(), 'entityId' => $this->params->getEntityId())));
        MT::getEventManager()->bind('base.comment_item_process', array($this, 'itemHandler'));
        ...
        $this->assign('loadMoreLabel', MT::getLanguage()->text('base', 'comment_load_more_label'));
        ...
        if ( !$dataInit )
        {
            $staticDataArray = array(
                'respondUrl' => MT::getRouter()->urlFor('BASE_CTRL_Comments', 'getCommentList'),
                ...
                'preloaderImgUrl' => MT::getThemeManager()->getCurrentTheme()->getStaticImagesUrl() . 'ajax_preloader_button.gif'
            );
    ...
    protected function getPages( $currentPage, $pagesCount, $displayPagesCount )
    {
        ...
        if ( $first )
        {
            $pageArray[] = array('label' => MT::getLanguage()->text('base', 'paging_label_first'), 'pageIndex' => 1);
        }

        if ( $prev )
        {
            $pageArray[] = array('label' => MT::getLanguage()->text('base', 'paging_label_prev'), 'pageIndex' => ($currentPage - 1));
        }
        ...
        if ( $next )
        {
            $pageArray[] = array('label' => MT::getLanguage()->text('base', 'paging_label_next'), 'pageIndex' => ( $currentPage + 1 ));
        }

        if ( $last )
        {
            $pageArray[] = array('label' => MT::getLanguage()->text('base', 'paging_label_last'), 'pageIndex' => $pagesCount);
        }
