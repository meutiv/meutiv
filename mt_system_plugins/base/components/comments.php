 ...
 * @package ow.mt_system_plugins.base.comments
 * @since 1.0
 */
class BASE_CMP_Comments extends MT_Component
{
    /**
    ...
    public function __construct( BASE_CommentsParams $params )
    {
        ...
        $this->isAuthorized = MT::getUser()->isAuthorized($params->getPluginKey(), 'add_comment') && $params->getAddComment();
        ...
        if ( !$this->isAuthorized )
        {
            ...
            if ( empty($errorMessage) )
            {
                ...
                $errorMessage = MT::getUser()->isAuthenticated() ? $status['msg'] : MT::getLanguage()->text('base', 'comments_add_login_message');
            }
    ...
    public function initForm()
    {
        ...
        if ( $this->isAuthorized )
        {
            MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.autosize.js');
            $taId = 'cta' . $this->id;
            ...
            $jsParams['addUrl'] = MT::getRouter()->urlFor('BASE_CTRL_Comments', 'addComment');
            ...
            $jsParams['labels'] = array(
                'emptyCommentMsg' => MT::getLanguage()->text('base', 'empty_comment_error_msg'),
                'disabledSubmit' => MT::getLanguage()->text('base', 'submit_disabled_error_msg'),
                'attachmentLoading' => MT::getLanguage()->text('base', 'submit_attachment_not_loaded'),
            );
            ...
            {
                $currentUserInfo = BOL_AvatarService::getInstance()->getDataForUserAvatars(array(MT::getUser()->getId()));
                $userInfoToAssign = $currentUserInfo[MT::getUser()->getId()];
            }
        ...
        MT::getDocument()->addOnloadScript("new OwComments(" . json_encode($jsParams) . ");");
