 ...
 * @since 1.0
 */
class BASE_CMP_LanguageValueEdit extends MT_Component
{
    public $eventBased;
	...
	public static function process( $prefix, $key )
	{
            ...
            foreach ( $list as $item )
            {
                    ...
                    if ( $dto !== null )
                    {
                        $event = new MT_Event('admin.before_save_lang_value', array('dto'=>$dto));
...
class LanguageValueEditForm extends Form
{
	...
	public function __construct( $prefix, $key, $parent )
	{
		...
		$this->setAjax(true);
		$this->setAction(MT::getRouter()->urlFor('ADMIN_CTRL_Languages', 'ajaxEditLangs')."?prefix={$prefix}&key={$key}");
		...
		else
		{
            $jsString = 'owForms[{$formName}].bind("success", function(json){
                if ( json["result"] == "success") {
                    MT.trigger("admin.language_key_edit_success", [json], this);
                }
            })';
		}
