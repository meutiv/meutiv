...
class BASE_CMP_ChangePassword extends MT_Component
{
    public function __construct()
    {
        ...
        if ( MT::getRequest()->isAjax() )
        {
            ...
            if ( $form->isValid($_POST) )
            {
                ...
                BOL_UserService::getInstance()->updatePassword( MT::getUser()->getId(), $data['password'] );
        ...
        else
        {
            ...
            $form->bindJsFunction(FORM::BIND_SUCCESS, "function( json )
            {
            	if( json.result )
            	{
            	    ...
            	    MT.info(".json_encode($messageSuccess).");
                }
                else
                {
                    MT.error(".json_encode($messageError).");
                }
            ...
            //include js
            $onLoadJs = " window.changePassword = new MT_BaseFieldValidators( " .
                                                    json_encode( array (
                                                            'formName' => $form->getName(),
                                                            'responderUrl' => MT::getRouter()->urlFor("BASE_CTRL_Join", "ajaxResponder"),
            ...
            $onLoadJs .= " window.oldPassword = new MT_ChangePassword( " .
                                                    json_encode( array (
                                                            'formName' => $form->getName(),
            ...
            MT::getDocument()->addScript($jsDir . "change_password.js");
        }
    }
}
...
class OldPasswordValidator extends MT_Validator
{
    public function __construct()
    {
        $language = MT::getLanguage();
    ...
    {
        $result = BOL_UserService::getInstance()->isValidPassword( MT::getUser()->getId(), $value );
