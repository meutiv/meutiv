 ...
 * @since 1.0
 */
abstract class BASE_CMP_DragAndDropPanel extends MT_Component
{
    ...
    public function __construct( $placeName, array $componentList, $template = null )
    {
        ...
        if ( $template !== null )
        {
            $plugin = MT::getPluginManager()->getPlugin(MT::getAutoloader()->getPluginKey(get_class($this)));
        ...
        MT_ViewRenderer::getInstance()->registerFunction('dd_component', array($this, 'tplComponent'));
    ...
    {
        $baseStaticJsUrl = MT::getPluginManager()->getPlugin('BASE')->getStaticJsUrl();

        MT::getDocument()->addScript($baseStaticJsUrl . 'jquery-ui.min.js');
        MT::getDocument()->addScript($baseStaticJsUrl . 'drag_and_drop_slider.js');
        MT::getDocument()->addScript($baseStaticJsUrl . 'ajax_utils.js');
        MT::getDocument()->addScript($baseStaticJsUrl . 'drag_and_drop_handler.js');
        MT::getDocument()->addScript($baseStaticJsUrl . 'component_drag_and_drop.js');

        MT::getLanguage()->addKeyForJs('base', 'widgets_delete_component_confirm');
        MT::getLanguage()->addKeyForJs('base', 'widgets_reset_position_confirm');
        $urlAjaxResponder = MT::getRouter()->urlFor($responderController, 'processQueue');
        ...
        $js->newObject('handler', 'MT_Components_DragAndDropAjaxHandler', array($urlAjaxResponder, $sharedData));
