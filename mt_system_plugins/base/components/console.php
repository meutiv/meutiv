 ...
 * @since 1.0
 */
class BASE_CMP_Console extends MT_Component
{
    ...
    public function onBeforeRender()
    {
        ...
        foreach ( $items as $item )
        {
            ...
            $renderEvent = new MT_Event(self::RENDER_EVENT_NAME, $resultItem, $resultItem);
            MT::getEventManager()->trigger($renderEvent);
            $resultItem = $renderEvent->getData();

            $itemCmp = $resultItem['item'];
            $order = $resultItem['order'];

            if ( is_subclass_of($itemCmp, 'MT_Renderable') && $itemCmp->isVisible() )
            {
                $resultItems[] = array(
                    'item' => $itemCmp->render(),
                    'order' => $order
        ...
        $jsUrl = MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'console.js';
        MT::getDocument()->addScript($jsUrl);

        $event = new MT_Event(BASE_CTRL_Ping::PING_EVENT . '.consoleUpdate');
        MT::getEventManager()->trigger($event);
        ...
        $js->newObject(array('MT', 'Console'), 'MT_Console', array($params, $event->getData()));

        MT::getDocument()->addOnloadScript($js, 900);
    }
    ...
    const VALUE_BLOCK_CLASS_GREEN = 'mt_mild_green';
    const VALUE_BLOCK_CLASS_RED = 'mt_mild_red';
