...
class BASE_CMP_WelcomeWidget extends BASE_CLASS_Widget
{
    ...
    public function __construct( BASE_CLASS_WidgetParameter $paramObject )
    {
        ...
        $text = MT::getLanguage()->text('base', 'welcome_widget_content');
        ...
        if ( MT::getPluginManager()->isPluginActive('photo') && mb_stripos($text, self::KEY_PHOTO_UPLOAD) !== false )
        {
            $label = MT::getLanguage()->text('photo', 'upload_photos');
            $js = MT::getEventManager()->call('photo.getAddPhotoURL');
        ...
        if ( mb_stripos($text, self::KEY_CHANGE_AVATAR) !== false )
        {
            ...
            $js =  ' $("#welcomeWinget_loadAvatarChangeCmp").click(function(){'
                    . 'document.avatarFloatBox = MT.ajaxFloatBox("BASE_CMP_AvatarChange", [], {width: 749, title: ' . json_encode($label) . '});'
                    . '});';
    ...
    public static function getStandardSettingValueList()
    {
        return array(
            ...
            self::SETTING_TITLE => MT::getLanguage()->text('base', 'welcome_widget_title')
        );
    }
    ...
    {
        BOL_LanguageService::getInstance()->addOrUpdateValue(MT::getLanguage()->getCurrentId(), 'base', 'welcome_widget_content', $settingList['text']);
    ...
    {
        $content = MT::getLanguage()->text('base', 'welcome_widget_content');
        $legend = nl2br(MT::getLanguage()->text('base', 'welcome_widget_legend'));
