 ...
 * @since 1.7.5
 */
class BASE_CMP_AjaxFileUploadButton extends MT_Component
{
    ...
    public function onBeforeRender()
    {
        ...
        UTIL_JsGenerator::composeJsString(
            ';window[{$addNewFile}] = function()
                    {
                        var ajaxUploadPhotoFB = MT.ajaxFloatBox("BASE_CMP_AjaxFileUpload", [], {
                            $title: {$title},
                            addClass: "mt_admin_ajax_file_upload_form"
                        });
                    };', array(
                'addNewFile' => $id,
                'title' => MT::getLanguage()->text('base', 'upload_files'),
                'close_alert' => MT::getLanguage()->text('base', 'close_alert')
            )
        )
    );
