...
class BASE_CLASS_AvatarField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $deleteLabel = MT::getLanguage()->text('base', 'delete');
        ...
        $markup = '<div class="mt_avatar_field">';
        $markup .= UTIL_HtmlTag::generateTag('input', $this->attributes);

        if ( !$this->value )
        {
            $markup .= '<div class="mt_avatar_field_preview" style="display: none;"><img src="" alt="" /><span title="'.$deleteLabel.'"></span></div>';
        }
        else 
        {
            $markup .= '<div class="mt_avatar_field_preview" style="display: block;"><img src="' . $this->value . '" alt="" /><span title="'.$deleteLabel.'"></span></div>';            
        ...
        $markup .= '<input type="hidden" name="' . $this->attributes['name'] . '" value="' . $this->value . '" class="mt_avatar_field_value" />';
        $markup .= '</div>';
    ...
    public function getElementJs()
    {
        ...
        $jsString .= "
			formElement.getValue = function(){
                ...
                var value = $(this.input).closest('.mt_avatar_field').find('.mt_avatar_field_value').val();

		        return value;
			};

			formElement.resetValue = function(){
                $(this.input).closest('.mt_avatar_field').find('.mt_avatar_field_value').val('');
                $(this.input).closest('.mt_avatar_field').find('input[name^=\'avatarUploaded\']').val(0);
            };

			formElement.setValue = function(value){
			    $(this.input).closest('.mt_avatar_field').find('.mt_avatar_field_value').val(value);
			    $(this.input).closest('.mt_avatar_field').find('input[name^=\'avatarUploaded\']').val(1);
			};
		";
