...
class BASE_CLASS_InvitationEventHandler
{
    ...
    public function collectItems( BASE_CLASS_ConsoleItemCollector $event )
    {
        ...
        $allInvitationCount = $this->service->findInvitationCount(MT::getUser()->getId());
        $item->setIsHidden( empty($allInvitationCount) );

        $event->addItem($item, 6);
    }

    public function addInvitation( MT_Event $event )
    {
        ...
        else
        {
            ...
            $dublicateEvent = new MT_Event('invitations.on_dublicate', $dublicateParams, $data);
            MT::getEventManager()->trigger($dublicateEvent);
    ...
    public function removeInvitation( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function loadList( BASE_CLASS_ConsoleListEvent $event )
    {
        ...
        {
            $itemEvent = new MT_Event('invitations.on_item_render', array(
            ...
            MT::getEventManager()->trigger($itemEvent);
    ...
    private function processDataInterface( $params, $data )
    {
        ...
        foreach ( array('string', 'conten') as $langProperty )
        {
            if ( !empty($data[$langProperty]) && is_array($data[$langProperty]) )
            {
                ...
                {
                    $data[$langProperty] = MT::getLanguage()->text($key[0], $key[1], $vars);
                }
            }
        }
    ...
    public function renderItem( MT_Event $event )
    {
        $params = $event->getParams();
        $data = $event->getData();
        ...
        if ( $interface['viewed'] )
        {
            $item->addClass('mt_console_new_message');
        }
    ...
    public function pluginActivate( MT_Event $e )
    {
        $params = $e->getParams();
    ...
    public function pluginDeactivate( MT_Event $e )
    {
        $params = $e->getParams();
        $pluginKey = $params['pluginKey'];
    ...
    public function pluginUninstall( MT_Event $e )
    {
        $params = $e->getParams();
        $pluginKey = $params['pluginKey'];
    ...
    {
        MT::getEventManager()->bind('invitations.on_item_render', array($this, 'renderItem'));
        MT::getEventManager()->bind('invitations.add', array($this, 'addInvitation'));

        MT::getEventManager()->bind('invitations.remove', array($this, 'removeInvitation'));
    }

    public function init()
    {
        MT::getEventManager()->bind(MT_EventManager::ON_PLUGINS_INIT, array($this, 'afterInits'));

        MT::getEventManager()->bind(MT_EventManager::ON_AFTER_PLUGIN_ACTIVATE, array($this, 'pluginActivate'));
        MT::getEventManager()->bind(MT_EventManager::ON_BEFORE_PLUGIN_DEACTIVATE, array($this, 'pluginDeactivate'));
        MT::getEventManager()->bind(MT_EventManager::ON_BEFORE_PLUGIN_UNINSTALL, array($this, 'pluginUninstall'));

        MT::getEventManager()->bind('console.load_list', array($this, 'loadList'));
        MT::getEventManager()->bind('console.ping', array($this, 'ping'));
        MT::getEventManager()->bind('console.collect_items', array($this, 'collectItems'));

        MT::getEventManager()->bind('notifications.send_list', array($this, 'sendList'));
    }
