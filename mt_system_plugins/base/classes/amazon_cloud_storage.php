 ...
 * @since 1.0
 */
require( MT_DIR_LIB . 'amazonS3' . DS . 'S3.php' );

class BASE_CLASS_AmazonCloudStorage implements MT_Storage
{
    ...
    {
        $this->cloudfilesTmpDir = MT_DIR_PLUGINFILES . 'base' . DS . 'cloudfiles' . DS;

        // Connect to Rackspace Cloud Files
        $this->s3 = new S3(MT_AMAZON_S3_ACCESS_KEY, MT_AMAZON_S3_SECRET_KEY);
        $this->bucketName = MT_AMAZON_S3_BUCKET_NAME;
        $this->bucketUrl = MT_AMAZON_S3_BUCKET_URL;
    ...
    private function getCloudFilePath( $path )
    {
        ...
        $prefixLength = strlen(MT_DIR_ROOT);
        $filePathLength = strlen($path);

        if ( $prefixLength <= $filePathLength && substr($path, 0, $prefixLength) === MT_DIR_ROOT )
        {
            $cloudPath = str_replace(MT_DIR_ROOT, '', $path);
    ...
    private function getLocalFSPath( $cloudPath )
    {
        ...
        $result = MT_DIR_ROOT . str_replace('/', DS, $cloudPath);
    ...
    private function triggerFileUploadEvent( $path, $size )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_FILE_UPLOAD, $params);
    ...
    private function triggerFileDeleteEvent( $path, $size )
    {
        ...
        $event = new MT_Event(self::EVENT_ON_FILE_DELETE, $params);
        MT::getEventManager()->trigger($event);
    }
