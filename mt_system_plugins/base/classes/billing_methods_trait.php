 ...
 * @package ow.mt_system_plugins.base.classes
 * @since 1.8.2
 */
...
trait BASE_CLASS_BillingMethodsTrait
{
    ...
    public function completed( array $params )
    {
        ...
        $this->setPageHeadingIconClass('mt_ic_cart');
    }
    ...
    {
        $this->assign('message', MT::getLanguage()->text('base', 'billing_order_canceled'));

        $this->setPageHeading(MT::getLanguage()->text('base', 'billing_order_status_page_heading'));
        $this->setPageHeadingIconClass('mt_ic_cart');
    }
    ...
    {
        $this->assign('message', MT::getLanguage()->text('base', 'billing_order_failed'));
    ...
    {
        if ( MT::getRequest()->isPost() && $_POST['action'] == 'update_products' )
        {
            ...
            MT::getApplication()->redirect(urldecode($_POST['back_url']));
        }
    }
