...
class BASE_CLASS_EventHandler
{
    ...
    public function genericInit()
    {
        ...
        $eventManager->bind(MT_EventManager::ON_USER_LOGIN, array($this, 'onUserLoginSaveStatistics'));
        ...
        $eventManager->bind(MT_EventManager::ON_USER_UNREGISTER, array($this, 'onDeleteUserContent'));
        $eventManager->bind(MT_EventManager::ON_USER_LOGIN, array($this, 'onUserLogin'));
        $eventManager->bind(MT_EventManager::ON_USER_LOGOUT, array($this, 'onUserLogout'));
        $eventManager->bind(MT_EventManager::ON_USER_REGISTER, array($this, 'onJoinMandatoryUserApprove'));
        $eventManager->bind(MT_EventManager::ON_USER_EDIT, array($this, 'onUserEditFeed'));
        $eventManager->bind(MT_EventManager::ON_USER_REGISTER, array($this, 'onJoinFeed'));
        ...
        $eventManager->bind(MT_EventManager::ON_USER_REGISTER, array($this, 'onUserRegisterWelcomeLetter'));
        $eventManager->bind(MT_EventManager::ON_USER_UNREGISTER, array($this, 'onUserUnregisterDeleteDisaproved'));
        ...
        $eventManager->bind(MT_EventManager::ON_USER_UNREGISTER, array($this, 'onUserUnregisterRemovePreference'));
        $eventManager->bind('base.update_entity_items_status', array($this, 'onUpdateEntityItemsStatus'));
        $eventManager->bind(MT_EventManager::ON_USER_UNREGISTER, array($this, 'onDeleteMediaPanelFiles'));
        $eventManager->bind(MT_EventManager::ON_USER_UNREGISTER, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(MT_EventManager::ON_USER_SUSPEND, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(MT_EventManager::ON_USER_SUSPEND, array($this, 'sendSuspendNotification'));
        $eventManager->bind(MT_EventManager::ON_USER_UNSUSPEND, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(MT_EventManager::ON_USER_APPROVE, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(MT_EventManager::ON_USER_DISAPPROVE, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(MT_EventManager::ON_USER_MARK_FEATURED, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(MT_EventManager::ON_USER_UNMARK_FEATURED, array($this, 'clearUserListQueryCache'));
        ...
        $eventManager->bind(MT_EventManager::ON_USER_LOGIN, array($this, 'onUserLoginSetAdminCookie'));
        ...
        $eventManager->bind(MT_EventManager::ON_USER_UNREGISTER, array($this, 'onUserUnregisterClearMailQueue'));

        $eventManager->bind('socialsharing.get_entity_info', array($this, 'sosialSharingGetUserInfo'));

        $eventManager->bind(MT_EventManager::ON_USER_REGISTER, array($this, 'setAccountTypeUserRoleOnUserRegister'));
        $eventManager->bind(MT_EventManager::ON_USER_REGISTER, array($this, 'deleteInviteCode'));
        $eventManager->bind('base.before_save_user', array($this, 'setUserRoleOnChangeAccountType'));

        $eventManager->bind(MT_EventManager::ON_JOIN_FORM_RENDER, array($this, 'onInviteMembersProcessJoinForm'));
    ...
    public function init()
    {
        ...
        $eventManager->bind(MT_EventManager::ON_FINALIZE, array($this, 'addJsDeclarations'));
        ...
        $eventManager->bind(MT_EventManager::ON_BEFORE_PLUGIN_UNINSTALL, array($this, 'onPluginUninstallDeleteComments'));
        ...
        $eventManager->bind(MT_EventManager::ON_BEFORE_PLUGIN_UNINSTALL, array($this, 'onPluginsUninstallDeleteGateway'));
        $eventManager->bind(MT_EventManager::ON_USER_UNREGISTER, array($this, 'onDeleteUserAttachments'));
        $eventManager->bind(MT_EventManager::ON_FINALIZE, array($this, 'onFinalizeAddScrollJs'));
        $eventManager->bind('join.get_captcha_field', array($this, 'getCaptcha'));
        $eventManager->bind(MT_EventManager::ON_FINALIZE, array($this, 'onFinalizeCheckIfSiteFullyInstalled'));
        $eventManager->bind(MT_EventManager::ON_AFTER_ROUTE, array($this, 'onPluginsInitCheckUserStatus'));
    ...
    public function onCollectQuickLinks( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( $userId )
        {
            ...
            {
                $event->add(array(
                    BASE_CMP_QuickLinksWidget::DATA_KEY_LABEL => MT::getLanguage()->text('base', 'my_blocked_users'),
                    BASE_CMP_QuickLinksWidget::DATA_KEY_URL => MT::getRouter()->urlForRoute('users-blocked'),
                    BASE_CMP_QuickLinksWidget::DATA_KEY_COUNT => $blockedCount,
                    BASE_CMP_QuickLinksWidget::DATA_KEY_COUNT_URL => MT::getRouter()->urlForRoute('users-blocked')
                ));
            }
        }
    }
     ...
     * Get sitemap urls
     *
     * @param MT_Event $event
     * @return void
     */
    public function onSitemapGetUrls( MT_Event $event )
    {
        ...
        switch ( $params['entity'] )
        {
            ...
            case 'base_pages' :
                ...
                $urls = array(
                    MT_URL_HOME,
                    MT::getRouter()->urlForRoute('base.mobile_version'),
                    MT::getRouter()->urlForRoute('base_join'),
                    MT::getRouter()->urlForRoute('static_sign_in'),
                    MT::getRouter()->urlForRoute('base_forgot_password')
                );
            ...
            case 'user_list' :
                ...
                {
                    $event->setData(array(
                        MT::getRouter()->urlForRoute('users'),
                        MT::getRouter()->urlForRoute('base_user_lists', array(
                            'list' => 'latest'
                        )),
                        MT::getRouter()->urlForRoute('base_user_lists', array(
                            'list' => 'featured'
    ...
    public function onAfterAdd( MT_Event $event )
    {
        $params = $event->getParams();
        $entityTypes = explode(',',
            MT::getConfig()->getValue('base', 'site_statistics_disallowed_entity_types'));
    ...
    public function onGetClassInstance( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function deleteEntityFlags( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onCollectModerationWidgetContent( BASE_CLASS_EventCollector $event )
    {
        ...
        $event->add(array(
            "name" => "flags",
            "label" => MT::getLanguage()->text("base", "flagged_content"),
    ...
    public function onCollectModerationToolsMenu( BASE_CLASS_EventCollector $event )
    {
        ...
        $event->add(array(
            "url" => MT::getRouter()->urlForRoute("base.moderation_flags_index"),
            "label" => MT::getLanguage()->text("base", "flagged_content"),
            "iconClass" => "mt_ic_clock",
            "key" => "flags"
        ));
    }

    public function deleteInviteCode( MT_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onEmergencyExit( MT_Event $e )
    {
        if ( !empty($_COOKIE['adminToken']) && trim($_COOKIE['adminToken']) == MT::getConfig()->getValue('base', 'admin_cookie') )
        {
            MT::getSession()->set('errorData', serialize($e->getParams()));
        }
    }

    public function onAfterBillingDeliverSale( MT_Event $event )
    {
        $params  = $event->getParams();
    ...
    public function onUserLoginSaveStatistics( MT_Event $event )
    {
        $params = $event->getParams();
        BOL_SiteStatisticService::getInstance()->addEntity('user_login', $params['userId']);
    }

    public function onUserLoginSetAdminCookie( MT_Event $event )
    {
        $params = $event->getParams();
        ...
        if ( BOL_AuthorizationService::getInstance()->isSuperModerator($params['userId']) )
        {
            ...
            MT::getConfig()->saveConfig('base', 'admin_cookie', $newToken);
            setcookie('adminToken', $newToken, time() + 3600 * 24 * 100, '/', '', false, true);
        }
    }

    public function onUserLogout( MT_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onUserLogin( MT_Event $e )
    {
        $params = $e->getParams();
        $userId = (int) $params['userId'];
        ...
        BOL_UserService::getInstance()->onLogin($userId, MT::getApplication()->getContext());
    }
    ...
    public function onPluginsInitCheckUserStatus()
    {
        ...
        {
            $user = BOL_UserService::getInstance()->findUserById(MT::getUser()->getId());

            if ( MT::getConfig()->getValue('base', 'mandatory_user_approve') && MT::getUser()->isAuthenticated() && !BOL_UserService::getInstance()->isApproved() )
            {
                MT::getRequestHandler()->setCatchAllRequestsAttributes('base.wait_for_approval', array('controller' => 'BASE_CTRL_WaitForApproval', 'action' => 'index'));
                MT::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', 'BASE_CTRL_User', 'signOut');
            }

            if ( $user !== null )
            {
                if ( BOL_UserService::getInstance()->isSuspended($user->getId()) && !MT::getUser()->isAdmin() )
                {
                    MT::getRequestHandler()->setCatchAllRequestsAttributes('base.suspended_user', array('controller' => 'BASE_CTRL_SuspendedUser', 'action' => 'index'));
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_User', 'signOut');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_Avatar');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_Edit');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_DeleteUser');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_Captcha');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_Console');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_AjaxLoader');
                }

                if ( (int) $user->emailVerify === 0 && MT::getConfig()->getValue('base', 'confirm_email') )
                {
                    MT::getRequestHandler()->setCatchAllRequestsAttributes('base.email_verify', array(MT_RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => 'BASE_CTRL_EmailVerify', MT_RequestHandler::CATCH_ALL_REQUEST_KEY_ACTION => 'index'));

                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.email_verify', 'BASE_CTRL_User', 'signOut');
                    MT::getRequestHandler()->addCatchAllRequestsExclude('base.email_verify', 'BASE_CTRL_EmailVerify');
                }
                ...
                if ( !empty($attrs[MT_RequestHandler::ATTRS_KEY_CTRL]) )
                {
                    $parents = class_parents($attrs[MT_RequestHandler::ATTRS_KEY_CTRL], true);
                ...
                if ( !$isAdminUrl )
                {
                    ...
                    {
                        MT::getRequestHandler()->setCatchAllRequestsAttributes('base.complete_profile.account_type', array('controller' => 'BASE_CTRL_CompleteProfile', 'action' => 'fillAccountType'));
                        MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_Console', 'listRsp');
                        MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_User', 'signOut');
                        MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'INSTALL_CTRL_Install');
                        MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_BaseDocument', 'installCompleted');
                        MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_AjaxLoader');
                        MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_AjaxComponentAdminPanel');
                        $this->excludeStaticPage('base.complete_profile.account_type');
                    }
                    else
                    {
                        $questionsEditStamp = MT::getConfig()->getValue('base', 'profile_question_edit_stamp');
                        $updateDetailsStamp = BOL_PreferenceService::getInstance()->getPreferenceValue('profile_details_update_stamp', MT::getUser()->getId());

                        if ( $questionsEditStamp >= (int) $updateDetailsStamp )
                        {
                            require_once MT_DIR_CORE . 'validator.php';
                            $questionList = BOL_QuestionService::getInstance()->getEmptyRequiredQuestionsList($user->id);

                            if ( !empty($questionList) )
                            {
                                MT::getRequestHandler()->setCatchAllRequestsAttributes('base.complete_profile', array('controller' => 'BASE_CTRL_CompleteProfile', 'action' => 'fillRequiredQuestions'));
                                MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_Console', 'listRsp');
                                MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_User', 'signOut');
                                MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'INSTALL_CTRL_Install');
                                MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_BaseDocument', 'installCompleted');
                                MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_AjaxLoader');
                                MT::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_AjaxComponentAdminPanel');
                                $this->excludeStaticPage('base.complete_profile');
                            }
                            else
                            {
                                BOL_PreferenceService::getInstance()->savePreferenceValue('profile_details_update_stamp', time(), MT::getUser()->getId());
                            }
                        }
                    }
                }
    ...
    public function addPageBanner( BASE_CLASS_EventCollector $event )
    {
        ...
        {
            $event->add(MT_ADS_XP_TOP);
        }
        elseif ( $params['key'] == 'base.content_bottom' )
        {
            $event->add(MT_ADS_XP_BOT);
        }
    }

    public function getQuestionLabel( MT_Event $event )
    {
        $params = $event->getParams();
        ...
        {
            $event->setData(MT::getLanguage()->text('base', 'questions_question_birthday_label_presentation_age'));
        }
    }

    public function onFinalizeCheckIfSiteFullyInstalled( MT_Event $event )
    {
        if ( !MT::getConfig()->getValue("base", "install_complete") )
        {
            MT::getApplication()->redirect(MT::getRouter()->urlForRoute("base_page_install_completed") . "?redirect=1");
        }
    }

    public function getCaptcha( MT_Event $e )
    {
    ...
    public function onFinalizeAddScrollJs( $e )
    {
        ...
        MT::getDocument()->addScript($plugin->getStaticJsUrl() . 'jquery.mousewheel.js');
        MT::getDocument()->addScript($plugin->getStaticJsUrl() . 'jquery.jscrollpane.js');
    }

    public function clearUserListQueryCache( MT_Event $event )
    {
        $params = $event->getParams();
        $userId = (int) $params['userId'];

        MT::getCacheManager()->clean(array(BOL_UserDao::CACHE_TAG_ALL_USER_LIST));
    }

    public function sendSuspendNotification( MT_Event $event )
    {
        $params = $event->getParams();
        $userId = (int) $params['userId'];
        ...
        $txt = MT::getLanguage()->text('base', 'suspend_notification_text', array('realName' => $displayName, 'suspendReason' => $message));
        $html = MT::getLanguage()->text('base', 'suspend_notification_html', array('realName' => $displayName, 'suspendReason' => $message));

        $subject = MT::getLanguage()->text('base', 'suspend_notification_subject');
    ...
    public function onDeleteUserAttachments( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onDeleteMediaPanelFiles( MT_Event $e )
    {
        $params = $e->getParams();
        $userId = (int) $params['userId'];
    ...
    public function onPluginsUninstallDeleteGateway( MT_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onSaveAttachmentImage( MT_Event $event )
    {
        $params = $event->getParams();
...
//    public function onDeleteAttachmentImage( MT_Event $event )
//    {
//        $params = $event->getParams();
//
//        if ( !empty($params["id"]) )
...
//        }
//        else if ( !empty($params['url']) && strstr($params['url'], MT::getStorage()->getFileUrl(MT::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'attachments')) )
//        {
...
//    }
    ...
    public function onUpdateEntityItemsStatus( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onUserUnregisterRemovePreference( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onPluginUninstallDeleteComments( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    {
        if ( MT::getConfig()->getValue('base', 'cron_is_configured') || defined('MT_PLUGIN_XP') )
        {
            return;
        }

        $coll->add(MT::getLanguage()->text('admin', 'cron_configuration_required_notice', array(
            'helpUrl' => 'http://docs.oxwall.org/install:cron'
        )));
    }

    public function addJsDeclarations( MT_Event $e )
    {
        //Langs
        MT::getLanguage()->addKeyForJs('base', 'ajax_floatbox_users_title');
        MT::getLanguage()->addKeyForJs('base', 'flag_as');
        MT::getLanguage()->addKeyForJs('base', 'delete_user_confirmation_label');
        MT::getLanguage()->addKeyForJs('base', 'authorization_limited_permissions');
        MT::getLanguage()->addKeyForJs('base', 'avatar_change');
        MT::getLanguage()->addKeyForJs('base', 'avatar_crop');

        $scriptGen = UTIL_JsGenerator::newInstance()->setVariable(array('MT', 'ajaxComponentLoaderRsp'), MT::getRouter()->urlFor('BASE_CTRL_AjaxLoader', 'component'));
        $scriptGen->setVariable(array('MT', 'ajaxAttachmentLinkRsp'), MT::getRouter()->urlFor('BASE_CTRL_Attachment', 'addLink'));

        //Ping
        $scriptGen->addScript('MT.getPing().setRspUrl({$url});', array(
            'url' => MT::getRouter()->urlFor('BASE_CTRL_Ping', 'index')
        ));

        //UsersApi
        $scriptGen->newObject(array('MT', 'Users'), 'MT_UsersApi', array(array(
            "rsp" => MT::getRouter()->urlFor('BASE_CTRL_AjaxUsersApi', 'rsp')
        )));

        MT::getDocument()->addScriptDeclaration($scriptGen->generateJs());

        //Light Cron
        $cronReady = MT::getConfig()->configExists('base', 'cron_is_configured') && MT::getConfig()->getValue('base', 'cron_is_configured');

        if ( !$cronReady && !defined('MT_PLUGIN_XP') )
        {
            MT::getDocument()->addOnloadScript(UTIL_JsGenerator::composeJsString(
                '$.get({$cron});'
                , array(
                'cron' => MT::getRequest()->buildUrlQueryString(MT_URL_HOME . 'mt_cron/run.php', array(
                    'ow-light-cron' => 1
                ))
            )));
        }
    ...
    {
        $e->add(array(
            'title' => MT::getLanguage()->text('base', 'console_item_label_dashboard'),
            'iconClass' => 'mt_ic_house',
            'url' => MT::getRouter()->urlForRoute('base_member_dashboard'),
            'order' => 1
        ));

        $e->add(array(
            'title' => MT::getLanguage()->text('base', 'console_item_label_profile'),
            'iconClass' => 'mt_ic_user',
            'url' => MT::getRouter()->urlForRoute('base_member_profile'),
            'order' => 3
        ));
    }

    public function onAddComment( MT_Event $event )
    {
        $params = $event->getParams();
        ...
        $url = MT::getRouter()->urlForRoute('base_user_profile', array('username' => BOL_UserService::getInstance()->getUserName($entityId)));

        $avatars = BOL_AvatarService::getInstance()->getDataForUserAvatars(array($userId));
        $avatar = $avatars[$userId];

        $event = new MT_Event('notifications.add', array(
            'pluginKey' => 'base',
    ...
    public function onNotifyActions( BASE_CLASS_EventCollector $e )
    {
        $e->add(array(
            'section' => 'base',
            'sectionLabel' => MT::getLanguage()->text('base', 'notification_section_label'),
            'action' => 'base_add_user_comment',
            'description' => MT::getLanguage()->text('base', 'email_notifications_setting_user_comment'),
            'sectionIcon' => 'mt_ic_file',
            'selected' => true
        ));
    }
    ...
    public function onPreferenceMenuItem( BASE_CLASS_EventCollector $event )
    {
        ...
        $menuItem->setIconClass('mt_ic_gear_wheel');
    ...
    public function onDashboardMenuItem( BASE_CLASS_EventCollector $event )
    {
        ...
        $menuItem->setIconClass('mt_ic_house');
        ...
        $menuItem->setIconClass('mt_ic_user');
    ...
    public function onUserUnregisterDeleteDisaproved( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onUserUnregisterClearMailQueue( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onInviteMembersProcessJoinForm( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onActionToolbarAddUserBlockActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( $params['userId'] == MT::getUser()->getId() )
        {
            return;
        }
        ...
        $uniqId = uniqid("block-");
        $isBlocked = BOL_UserService::getInstance()->isBlocked($userId, MT::getUser()->getId());

        $resultArray[BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL] = $isBlocked ? MT::getLanguage()->text('base', 'user_unblock_btn_lbl') : MT::getLanguage()->text('base', 'user_block_btn_lbl');

        $toggleText = !$isBlocked ? MT::getLanguage()->text('base', 'user_unblock_btn_lbl') : MT::getLanguage()->text('base', 'user_block_btn_lbl');

        $resultArray[BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS] = $isBlocked ? 'mt_mild_green' : 'mt_mild_red';

        $toggleClass = !$isBlocked ? 'mt_mild_green' : 'mt_mild_red';
        ...
        $js->jQueryEvent("#" . $uniqId, "click", 'var self = $(this); toggle = function() {
            MT.Utils.toggleText(self, e.data.toggleText);
            MT.Utils.toggleAttr(self, "class", e.data.toggleClass);
            MT.Utils.toggleAttr(self, "data-command", e.data.toggleCommand);
        };
        if ( self.attr("data-command") == "block" )
            MT.Users.blockUserWithConfirmation(e.data.userId, toggle);
        else {
            MT.Users.unBlockUser(e.data.userId);
            toggle();
        }'
            , array("e"), array(
                "userId" => $userId,
    ...
    public function onActionToolbarAddUserFeatureActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            ...
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_LABEL => MT::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
        ...
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL] = $isFeatured ? MT::getLanguage()->text('base', 'user_action_unmark_as_featured') : MT::getLanguage()->text('base', 'user_action_mark_as_featured');

        $toggleText = !$isFeatured ? MT::getLanguage()->text('base', 'user_action_unmark_as_featured') : MT::getLanguage()->text('base', 'user_action_mark_as_featured');
        ...
        $js = UTIL_JsGenerator::newInstance();
        $js->jQueryEvent("#" . $uniqId, "click", 'MT.Users[$(this).attr("data-command") == "feature" ? "featureUser" : "unFeatureUser"](e.data.userId);
         MT.Utils.toggleText(this, e.data.toggleText);
         MT.Utils.toggleAttr(this, "data-command", e.data.toggleCommand);'
            , array("e"), array(
                "userId" => $userId,
                "toggleText" => $toggleText,
                "toggleCommand" => $toggleCommand
    ...
    public function onActionToolbarAddUserApproveActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_KEY => 'base.moderation',
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_LABEL => MT::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_HREF => MT::getRouter()->urlFor('BASE_CTRL_User', 'approve', array('userId' => $userId)),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL => MT::getLanguage()->text('base', 'profile_toolbar_user_approve_label'),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS => 'mt_mild_green',
    ...
    public function onActionToolbarAddAuthActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            ...
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL => MT::getLanguage()->text('base', 'authorization_give_user_role'),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS => 'mt_mild_green',
        ...
        $js = UTIL_JsGenerator::newInstance()->jQueryEvent('#' . $uniqId, 'click', 'window.baseChangeUserRoleFB = MT.ajaxFloatBox("BASE_CMP_GiveUserRole", [e.data.userId], { width:556, title: e.data.title });', array('e'), array(
            'userId' => $userId,
            'title' => MT::getLanguage()->text('base', 'authorization_user_roles')
        ));
    ...
    public function onActionToolbarAddSuspendActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_ATTRIBUTES] = array();
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL] = $suspended ? MT::getLanguage()->text('base', 'user_unsuspend_btn_lbl') : MT::getLanguage()->text('base', 'user_suspend_btn_lbl');

        $toggleText = !$suspended ? MT::getLanguage()->text('base', 'user_unsuspend_btn_lbl') : MT::getLanguage()->text('base', 'user_suspend_btn_lbl');
        ...
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS] = $suspended ? "mt_mild_green" : "mt_mild_red";

        $toggleClass = !$suspended ? "mt_mild_green" : "mt_mild_red";

        $rsp = MT::getRouter()->urlFor('BASE_CTRL_SuspendedUser', 'ajaxRsp');
        $rsp = MT::getRequest()->buildUrlQueryString($rsp, array(
            "userId" => $userId
        ));

        MT::getLanguage()->addKeyForJs('base', 'suspend_floatbox_title');
        ...
        $js->jQueryEvent("#" . $uniqId, "click", ' 
            ...
            {
                MT.ajaxFloatBox("BASE_CMP_SetSuspendMessage", [e.data.userId],{width: 520, title: MT.getLanguageText(\'base\', \'suspend_floatbox_title\', {\'displayName\': e.data.displayName})}); 
            }
            else
            {
                MT.trigger("base.on_suspend_command", ["unsuspend"])
            } '
            , array("e"), array(
                "userId" => $userId,
                "toggleText" => $toggleText,
        ...
        $js->addScript( ' MT.bind("base.on_suspend_command", function( command, message ) {
                var element = $("#"+{$uniqId});

                MT.Users[command == "suspend" ? "suspendUser" : "unSuspendUser"]({$userId}, null, message);
                MT.Utils.toggleText(element, {$toggleText});
                MT.Utils.toggleAttr(element, "class", {$toggleClass});
                MT.Utils.toggleAttr(element, "data-command", {$toggleCommand});
    ...
    public function onActionToolbarAddFlagActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( BOL_AuthorizationService::getInstance()->isSuperModerator($params['userId'])
            || $params['userId'] == MT::getUser()->getId() )
        {
            return;
        }
        ...
        $script = UTIL_JsGenerator::newInstance()->jQueryEvent('#' . $linkId, 'click', 'MT.flagContent(e.data.entityType, e.data.entityId);'
            , array('e'), array(
        ...
        $resultArray = array(
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL => MT::getLanguage()->text('base', 'flag'),
    ...
    public function onActionToolbarAddDeleteActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $callbackUrl = MT::getRouter()->urlFor('BASE_CTRL_User', 'userDeleted');

        $linkId = 'ud' . rand(10, 1000000);
        $script = UTIL_JsGenerator::newInstance()->jQueryEvent('#' . $linkId, 'click', 'MT.Users.deleteUser(e.data.userId, e.data.callbackUrl, false);'
            , array('e'), array('userId' => $userId, 'callbackUrl' => $callbackUrl));

        MT::getDocument()->addOnloadScript($script);

        $resultArray = array(
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL => MT::getLanguage()->text('base', 'profile_toolbar_user_delete_label'),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS => 'mt_mild_red',
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_HREF => 'javascript://',
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_ID => $linkId,
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_KEY => 'base.moderation',
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_LABEL => MT::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
    ...
    public function onUserRegisterWelcomeLetter( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onUserAvatarCommentFeed( MT_Event $event )
    {
        $params = $event->getParams();
        ...
        MT::getEventManager()->trigger(new MT_Event('feed.activity', array(
    ...
    public function onUserAvatarLikeFeed( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onLikeUserJoin( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onUserJoinCommentFeed( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onJoinFeed( MT_Event $event )
    {
        $params = $event->getParams();
        ...
        $event = new MT_Event('feed.action', array(
            'pluginKey' => 'base',
    ...
    public function onUserEditFeed( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onJoinMandatoryUserApprove( MT_Event $event )
    {
        $params = $event->getParams();

        if ( !MT::getConfig()->getValue('base', 'mandatory_user_approve') )
        {
            $e = new MT_Event(MT_EventManager::ON_USER_APPROVE, array('userId' => (int) $params['userId']));
            MT::getEventManager()->trigger($e);
    ...
    {
        $event->add(array('site_name' => MT::getConfig()->getValue('base', 'site_name')));
        $event->add(array('site_url' => MT_URL_HOME));
        $event->add(array('site_email' => MT::getConfig()->getValue('base', 'site_email')));
        $event->add(array('default_currency' => BOL_BillingService::getInstance()->getActiveCurrency()));
    }

    public function onDeleteUserContent( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function sosialSharingGetUserInfo( MT_Event $event )
    {
        $params = $event->getParams();
        $data = $event->getData();
        $data['display'] = false;
    ...
    public function setAccountTypeUserRoleOnUserRegister( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function setUserRoleOnChangeAccountType( MT_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onAfterAvatarUpdate( MT_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onProvideMetaInfoForPage( MT_Event $event )
    {
        $document = MT::getDocument();
        $language = MT::getLanguage();

        if( !$document || !$document instanceof MT_HtmlDocument )
        {
            return;
        }
        ...
        if( !empty($params["title"]) )
        {
            ...
            if (!empty($parts[1]) && $parts[1] === 'meta_title_user_page')
            {
                        ...
                        || empty($vars['user_location']))
                {
                    ...
                    $profileTitle = $profileTitle . $alterDelimiter . MT::getConfig()->getValue('base', 'site_name');
        ...
        $document->addMetaInfo("og:site_name", MT::getConfig()->getValue('base', 'site_name'));
    ...
    public function generateEmailVerificationCode(MT_Event $event)
    {
        $min = 10000;
        $max = 99999;
    ...
    {
        $usedRoute = MT::getRouter()->getUsedRoute();
        ...
        if ($usedRoute) {
            ...
            if ($dispatchAttrs['controller'] === 'BASE_CTRL_StaticDocument') {
                ...
                $staticDoc = $navService->findStaticDocument(MT::getRequest()->getRequestUri());
                ...
                if ($staticDoc) {
                    ...
                    if ($exclude) {
                        MT::getRequestHandler()->addCatchAllRequestsExclude(
