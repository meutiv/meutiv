 ...
 * @package mt_core
 * @method static MT_Dispatcher getInstance()
 * @since 1.0
 */
class MT_Dispatcher
{
    ...
    public function dispatch()
    {
        ...
        {
            MT::getRequest()->setUriParams($this->dispatchAttributes[self::ATTRS_KEY_VARLIST]);
        }

        $plugin = MT::getPluginManager()->getPlugin(MT::getAutoloader()->getPluginKey($this->dispatchAttributes[self::ATTRS_KEY_CTRL]));
        ...
        try
        {
            $controller = MT::getClassInstance($this->dispatchAttributes[self::ATTRS_KEY_CTRL]);
        }
        ...
        if ( $controller === null || !$controller instanceof MT_ActionController )
        {
    ...
    private function getControllerActionDefaultTemplate()
    {
        ...
        $templateFilename = MT::getAutoloader()->classToFilename($this->dispatchAttributes[self::ATTRS_KEY_CTRL], false) . '_'
            . MT::getAutoloader()->classToFilename(ucfirst($this->dispatchAttributes[self::ATTRS_KEY_ACTION]), false) . '.html';
        ...
        if ( $catchRequest )
        {
            if ( isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_REDIRECT]) && (bool) $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_REDIRECT] )
            {
                ...
                $redirectUrl = ($route !== null) ?
                    ...
                    MT::getRouter()->urlFor($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_CTRL], $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_ACTION], $params);

                $redirectUrl = MT::getRequest()->buildUrlQueryString($redirectUrl, array('back_uri' => MT::getRequest()->getRequestUri()));
                ...
                if ( isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_JS]) && (bool) $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_JS] )
                {
                    ...
                    if ( array_key_exists('base.members_only', $this->catchAllRequestsAttributes) )
                    {
                        if ( $this->dispatchAttributes[self::CATCH_ALL_REQUEST_KEY_CTRL] === 'BASE_CTRL_User' && $this->dispatchAttributes[self::CATCH_ALL_REQUEST_KEY_ACTION] === 'standardSignIn' )
                        {
                            ...
                            MT::getDocument()->addOnloadScript("window.location = '" . MT::getRequest()->buildUrlQueryString($redirectUrl, array('back_uri' => $backUri)) . "'");
                            return null;
                        }
                        else
                        {
                            $ru = MT::getRequest()->buildUrlQueryString(MT::getRouter()->urlForRoute('static_sign_in'), array('back_uri' => MT::getRequest()->getRequestUri()));
