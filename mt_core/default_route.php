...
//    }
    ...
    public function generateUri( $controller, $action = null, array $params = array() )
    {
        ...
        $className = str_replace(MT::getAutoloader()->getPackagePointer($controller) . '_', '', $controller);

        $plugin = MT::getPluginManager()->getPlugin(MT::getAutoloader()->getPluginKey($controller));
    ...
    {//TODO check if method is in try/catch
        ...
        for ( $i = 0; $i < $arraySize; $i++ )
        {
            if ( $i === 0 )
            {
                try
                {
                    $classPrefix = strtoupper(MT::getPluginManager()->getPluginKey($uriArray[$i])) . '_' . $controllerNamePrefixAdd . $this->controllerNamePrefix;
                }
            ...
            if ( $i === 1 )
            {
                ...
                if ( ( $classPath = MT::getAutoloader()->getClassPath($ctrClass) ) && !file_exists($classPath) || !$classPath )
                {
                    throw new Redirect404Exception('Invalid uri was provided for routing!');
                }
