...
abstract class FormElement
{
    ...
    public function addFilter( MT_IFilter $filter )
    {
        $this->filters[] = $filter;
    }
    ...
    {
        if ( !$validator instanceof MT_Validator )
        {
...
abstract class InvitationFormElement extends FormElement
{
    ...
    public function __construct( $name, $id = '')
    {
        ...
        $this->setInvitation(MT::getLanguage()->text('base', 'form_element_common_invitation_text'));
    }
...
class DateField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'date_field.js');

        MT::getDocument()->addOnloadScript(" if( window.date_field == undefined ) { window.date_field = {}; } window.date_field['" . $this->getId() . "'] = new DateField( '" . ( $this->getName() ) . "' ); ");
        ...
        $config = MT::getConfig()->getValue('base', 'date_field_format');
        ...
        if ( $config === 'dmy' )
        {
            $result = '<div class="' . $this->getAttribute('name') . '">
                            ...
                            <div class="mt_inline owm_inline">' . UTIL_HtmlTag::generateTag('select', $dayAttributes, true, $dayOptionsString) . '</div>
                            <div class="mt_inline owm_inline">' . UTIL_HtmlTag::generateTag('select', $monthAttributes, true, $monthOptionsString) . '</div>
                            <div class="mt_inline owm_inline">' . UTIL_HtmlTag::generateTag('select', $yearAttributes, true, $yearOptionsString) . '</div>
                        </div>';
        }
        else
        {
...
class Submit extends FormElement
{
    ...
    public function __construct( $name, $id = '',  $decorator = 'button' )
    {
        ...
        $this->setValue(MT::getLanguage()->text('base', 'form_element_submit_default_value'));
    ...
    public function renderInput( $params = null )
    {
        ...
        {
            $finalMarkup = MT::getThemeManager()->processDecorator($this->decorator, $params);
        }
        else
        {
...
class RadioField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $renderedString = '<ul class="mt_radio_group clearfix">';
...
class CheckboxGroup extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $renderedString = '<ul class="mt_checkbox_group clearfix">';
...
class Selectbox extends InvitationFormElement
{
    ...
    public function __construct( $name, $id = '' )
    {
        ...
        $this->setInvitation(MT::getLanguage()->text('base', 'form_element_select_field_invitation_label'));
...
class Multiselect extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        return
            '<table class="mt_multiselect" id="' . $this->getId() . '">
                <tr>
                    <td>' . $chicesSelectMarkup . '</td>
                    <td><input type="button" class="select" value="" /><br /><br /><br /><input type="button" class="deselect mt_ic_left_arrow" value="" /></td>
    ...
    public function getElementJs()
    {
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'multiselect_field.js');
...
class SuggestField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        {
            $this->addAttribute('value', $this->value);
            $this->addAttribute('class', 'mt_inputready');
        }

        $this->addAttribute('type', 'text');

        $scriptUrl = MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery-suggest.js';
        MT::getDocument()->addScript($scriptUrl);

        $js = '
            var $input = $("#' . $this->getId() . '");
            var initialLabel = $input.val();
            $input.parent().find(".mt_suggest_invitation").click(function(){ $input.focus(); });
            $input.suggest(' . $this->responderUrl . ', {autoSuggest: true, minchars: ' . $this->minChars . ', onFocus: function(first){
                    $(this).removeClass("mt_inputready");
        ...
        return '<div class="mt_suggest_field">' . UTIL_HtmlTag::generateTag('input', $this->attributes) . '<div class="mt_suggest_invitation"></div></div>';
    }
}
...
class TagsField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $tagTpl = '<div class="tags-field-tag-cont" style="margin: 2px 2px 0pt 0pt; padding: 2px 5px; background: rgb(204, 204, 204) none repeat scroll 0% 0%; float: left;"><span class="tags-field-tag">${tag}</span> <a class="mt_lbutton mt_red tags-field-del-tag">x</a></div>';
        ...
        $tpl = '<span id=${id}>' . '${hidden}' . '<input class="mt_text" type="text">' . '${tags}' . '</span>';
        ...
        MT::getDocument()->addScript($plugin->getStaticJsUrl() . 'tags_field.js');
...
{
    const CAPTCHA_PREFIX = 'mt_captcha_';
    ...
    public function renderInput( $params = null )
    {
        ...
        $captchaUrl = MT_URL_HOME . 'captcha.php';
        $captchaResponderUrl = MT::getRouter()->urlFor('BASE_CTRL_Captcha', 'ajaxResponder');
        $captchaClass = $this->getName() . '_' . $this->getId();
        $uniqueId = md5(time());

        $jsDir = MT::getPluginManager()->getPlugin("base")->getStaticJsUrl();
        MT::getDocument()->addScript($jsDir . "captcha.js");

        $string = ' window.' . $this->jsObjectName . ' = new MT_Captcha( ' . json_encode(array('captchaUrl' => $captchaUrl,
                'captchaClass' => $captchaClass,
                'captchaId' => $this->getId(),
                'responderUrl' => $captchaResponderUrl
            )) . ');';

        MT::getDocument()->addOnloadScript($string);

        return '<div class="' . $captchaClass . '">
                    <div class="mt_automargin clearfix" style="width: 230px;">
                            <div class="mt_left"><img src="' . $captchaUrl . '" id="siimage"></div>
                            <div class="mt_right" style="padding-top: 21px;"><span class="ic_refresh mt_automargin" id="siimage_refresh" style="cursor:pointer;"></span></div>
                    </div>
...
class AgeRange extends FormElement implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'age_range_field.js');
        MT::getDocument()->addOnloadScript(" window." . $this->getName() . " = new AgeRangeField( " . json_encode($this->getName()) . ", " . json_encode($this->minAge) . ", " . ( $this->maxAge ) . " ); ");
        ...
        $result = '<div id="' . $this->getAttribute('id') . '"class="' . $this->getAttribute('name') . '">
                        <div class="mt_range_from mt_inline ">' . $language->text('base', 'form_element_from') . '</div>
                        <div class="mt_inline">' . UTIL_HtmlTag::generateTag('select', $fromAgeAttrs, true, $fromAgeOptionsString) . '</div>
                        <div class="mt_range_to mt_inline">' . $language->text('base', 'form_element_to') . '</div>
                        <div class="mt_inline">' . UTIL_HtmlTag::generateTag('select', $toAgeAttrs, true, $toAgeOptionsString) . '</div>
                        <div class="mt_range_label mt_inline">' . $language->text('base', 'form_element_age_range') . '</div>
                    </div>';
...
//    }
    ...
    public function renderInput( $params = null )
    {
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'range_field.js');
        MT::getDocument()->addOnloadScript(" window." . $this->getName() . " = new RangeField( '" . ( $this->getName() ) . "', " . ( $this->minValue ) . ", " . ( $this->maxValue ) . " ); ");
...
class DateRange extends FormElement implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $result = '<div id="' . $this->getAttribute('id') . '" class="' . $this->getAttribute('name') . '">
                       ' . $language->text('base', 'form_element_from') . '  <div class="mt_inline">' . ( $this->minDate->renderInput() ) . '</div>
                       ' . $language->text('base', 'form_element_to') . '
                       <div class="mt_inline">' . ( $this->maxDate->renderInput() ) . '</div>
                    </div>';
...
class BillingGatewaySelectionField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $event = new MT_Event(BOL_BillingService::EVENT_ON_COLLECT_GATEWAYS_LIST, array(), $gateways);
        ...
        if ( $gateways )
        {
            ...
            $cont_id = $id . '-cont';
            $renderedString .= '<ul class="mt_billing_gateways clearfix" id="' . $cont_id . '">';
            ...
            foreach ( $paymentOptions as $option )
            {
                ...
                if ( $gatewaysNumber == 1 )
                {
                    $renderedString .= '<li style="display: inline-block; padding-right: 20px;">' . MT::getLanguage()->text('base', 'billing_pay_with') . '</li>';
            ...
            $js = 'var $url_field = $("#url-' . $id . '");
                ...
                $("ul#' . $cont_id . ' input").change(function(){
                    ...
                    MT.trigger("core.gateway_changed", [{gatewayKey: this.value}]);
                });';
        ...
        {
            $renderedString = MT::getLanguage()->text('base', 'billing_no_gateways');
        }
    ...
    protected function getAdapterData( $gateways )
    {
        ...
        {
            /* @var $adapter MT_BillingAdapter */
            if ( $adapter = MT::getClassInstance($gateway->adapterClassName) )
            {
...
class YearRange extends FormElement implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'age_range_field.js');
        MT::getDocument()->addOnloadScript(" window." . $this->getName() . " = new AgeRangeField( " . json_encode($this->getName()) . ", " . json_encode($this->minYear) . ", " . json_encode($this->maxYear) . " ); ");
        ...
        $result = '<div id="' . $this->getAttribute('id') . '"class="' . $this->getAttribute('name') . '">
                        <div class="mt_range_from mt_inline">' . $language->text('base', 'form_element_from') . '</div>
                        <div class="mt_inline">' . UTIL_HtmlTag::generateTag('select', $fromYearAttrs, true, $fromAgeOptionsString) . '</div>
                        <div class="mt_range_to mt_inline">' . $language->text('base', 'form_element_to') . '</div>
                        <div class="mt_inline">' . UTIL_HtmlTag::generateTag('select', $toYearAttrs, true, $toAgeOptionsString) . '</div>
                        <div class="mt_range_label mt_inline">' . $language->text('base', 'form_element_year_range') . '</div>
                    </div>';
...
class MobileWysiwygTextarea extends Textarea
{
    ...
    public function __construct( $name, $id = '',  $pluginKey = 'blog', array $buttons = array() )
    {
        ...
        $stringValidator->setErrorMessage(MT::getLanguage()->text('base', 'text_is_too_long', array('max_symbols_count' => 50000)));
    ...
    public function renderInput( $params = null )
    {
        ...
        if ( MT::getRegistry()->get('baseWsInit') === null )
        {
            if ( in_array(BOL_TextFormatService::WS_BTN_IMAGE, $this->buttons) )
            {
                MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.html5_upload.js');
            }

            MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'suitup.jquery.js');
            MT::getDocument()->addStyleSheet(MT::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'suitup.css');

            // register js langs
            MT::getLanguage()->addKeyForJs('base', 'ws_button_label_link');
            MT::getLanguage()->addKeyForJs('base', 'ws_button_label_video');
            MT::getLanguage()->addKeyForJs('base', 'ws_error_video');

            MT::getRegistry()->set('baseWsInit', true);
        }
        ...
        $js->addScript('$("#" + {$uniqId}).suitUp({$buttons}, {$imageUploadUrl}, {$embedUrl}).show();', array(
            ...
            'uniqId' => $this->getId(),
            'imageUploadUrl' => MT::getRouter()->urlFor('BASE_CTRL_MediaPanel', 'ajaxUpload', array(
                'pluginKey' => $this->pluginKey
            )),
            'embedUrl' => MT::getRouter()->urlFor('BASE_MCTRL_Oembed', 'getAjaxEmbedCode')
        ));
...
class WysiwygTextarea extends InvitationFormElement
{
    ...
    public function __construct( $name, $id = '', array $buttons = null, $init = true )
    {
        ...
        if ( MT::getRequest()->isMobileUserAgent() )
        {
    ...
    public function renderInput( $params = null )
    {
        ...
        if ( MT::getRegistry()->get('baseWsInit') === null )
        {
            ...
            $array = array(
                'editorCss' => MT::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'htmlarea_editor.css',
                'themeImagesUrl' => MT::getThemeManager()->getCurrentTheme()->getStaticImagesUrl(),
                'imagesUrl' => MT::getRouter()->urlFor('BASE_CTRL_MediaPanel', 'index', array('pluginKey' => 'blog', 'id' => '__id__')),
                'labels' => array(
                    'buttons' => array(
                    ...
                    'common' => array(
                        ...
                        'linkNewWindowLabel' => $language->text('base', 'ws_link_new_windmt_label'),
                    ),
                    'messages' => array(
                ...
                'buttonCode' => MT::getThemeManager()->processDecorator('button', array('label' => '#label#', 'class' => 'mt_ic_add mn_submit')),
            ...
            MT::getDocument()->addScriptDeclarationBeforeIncludes($script);
            MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'htmlarea.js');
            MT::getDocument()->addStyleSheet(MT::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'htmlarea.css');
            MT::getRegistry()->set('baseWsInit', true);
        }
        ...
        if ( $this->init )
        {
            MT::getDocument()->addOnloadScript("$('#{$this->getId()}').htmlarea( " . json_encode($params) . " );");
        }
...
class TagsInputField extends FormElement
{
    ...
    public function __construct( $name, $id = '')
    {
        parent::__construct($name, $id);
        $this->value = array();
        $this->invLabel = MT::getLanguage()->text('base', 'tags_input_field_invitation');
    ...
    public function renderInput( $params = null )
    {
        ...
        MT::getDocument()->addScript(MT::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.tagsinput.js');
