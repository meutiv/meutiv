 ...
 * @package mt_core
 * @method static MT_ThemeManager getInstance()
 * @since 1.0
 */
class MT_ThemeManager
{
    ...
    {
        $selectedTheme = MT::getConfig()->getValue('base', 'selectedTheme');

        if ( empty($this->themeObjects[$selectedTheme]) )
        {
            $this->themeObjects[$selectedTheme] = $this->themeService->getThemeObjectByKey(MT::getConfig()->getValue('base', 'selectedTheme'));
        }
     ...
     * Sets current theme.
     *
     * @param MT_Theme $theme
     */
    public function setCurrentTheme( MT_Theme $theme )
    {
        if ( $theme === null )
        {
            return;
    ...
    public function addDecorator( $decoratorName, $pluginKey )
    {
        ...
        $this->decorators[trim($decoratorName)] = MT::getPluginManager()->getPlugin($pluginKey)->getDecoratorDir() . $decoratorName . '.html';
    }
...
//        }
    ...
    public function processBlockDecorator( $name, array $params, $content = '' )
    {
        ...
        switch ( $name )
        {
            case 'box':
                if ( !empty($params['langLabel']) )
                {
                    ...
                    $params['label'] = MT::getLanguage()->text($tmpArr[0], $tmpArr[1]);
                }
                ...
                $params['addClass'] = $params['type'] . (empty($params['addClass']) ? '' : ' ' . $params['addClass']) . ( $params['capEnabled'] ? '' : ' mt_no_cap' );
        ...
        $params = $event->getProperties();
        $viewRenderer->assignVar('data', $params);
        $markup = $viewRenderer->renderTemplate(MT::getThemeManager()->getDecorator($name));
