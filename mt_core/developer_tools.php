 ...
 * @package mt_core
 * @method static MT_DeveloperTools getInstance()
 * @since 1.8.3
 */
class MT_DeveloperTools
{
    ...
    {
        $configDev = (int) MT::getConfig()->getValue("base", self::CONFIG_NAME);

        if ( $configDev > 0 )
        {
            $this->refreshEntitiesCache($configDev);
            MT::getConfig()->saveConfig("base", self::CONFIG_NAME, 0);
            MT::getApplication()->redirect();
        }

        if ( defined("MT_DEV_MODE") && MT_DEV_MODE )
        {
            $this->refreshEntitiesCache(MT_DEV_MODE);
        }

        // show profiler only for desktop and if it's enabled
        if ( !MT::getApplication()->isDesktop() || MT::getRequest()->isAjax() || !MT_PROFILER_ENABLE )
        {
            return;
        }

        MT_View::setCollectDevInfo(true);
        MT::getEventManager()->setDevMode(true);
        MT::getEventManager()->bind("base.append_markup", array($this, "onAppendMarkup"));
    }
    /* ---------------------------------------- Developer handlers -------------------------------------------------- */
    ...
    public function refreshEntitiesCache( $options = 1 )
    {
        ...
        MT::getEventManager()->trigger(new MT_Event(self::EVENT_UPDATE_CACHE_ENTITIES, array("options" => $options)));
    }
    ...
    {
        MT_ViewRenderer::getInstance()->clearCompiledTpl();
    }
    ...
    public function clearThemeCache()
    {
        ...
        if ( MT::getConfig()->configExists("base", "cachedEntitiesPostfix") )
        {
            MT::getConfig()->saveConfig("base", "cachedEntitiesPostfix", UTIL_String::getRandomString());
        }
    }
    ...
    public function onAppendMarkup( BASE_CLASS_EventCollector $event )
    {
        ...
        $view = new MT_View();
        $view->setTemplate(MT::getPluginManager()->getPlugin("base")->getCmpViewDir() . "dev_tools_tpl.html");

        // get current request attributes
        $requestHandlerData = MT::getRequestHandler()->getDispatchAttributes();

        try
        {
            $ctrlPath = MT::getAutoloader()->getClassPath($requestHandlerData["controller"]);
        }
        catch ( Exception $e )
        {
            $ctrlPath = "not_found";
        ...
        // rendered view data
        $view->assign("renderedItems", $this->getViewInfo(MT_View::getDevInfo()));

        // sql queries data
        $filter = !empty($_GET["pr_query_log_filter"]) ? trim($_GET["pr_query_log_filter"]) : null;
        $view->assign("database",
            $this->getSqlInfo(MT::getDbo()->getQueryLog(), MT::getDbo()->getTotalQueryExecTime(), $filter));

        // events data
        $view->assign("events", $this->getEventInfo(MT::getEventManager()->getLog()));

        $view->assign("clrBtnUrl", MT::getRequest()->buildUrlQueryString(MT::getRouter()->urlFor("BASE_CTRL_Base", "turnDevModeOn"),
            array("back-uri" => urlencode(MT::getRouter()->getUri()))));
    ...
    protected function getViewInfo( array $viewData )
    {
        ...
        foreach ( $viewData as $class => $item )
        {
            try
            {
                $src = MT::getAutoloader()->getClassPath($class);
            }
            catch ( Exception $e )
            {
                $src = "not_found";
            ...
            if ( is_subclass_of($class, MT_MasterPage::class) )
            {
                $viewDataArray["mp"] = $addItem;
            }
            else if ( is_subclass_of($class, MT_ActionController::class) )
            {
                $viewDataArray["ctrl"] = $addItem;
            }
            else if ( is_subclass_of($class, MT_Component::class) )
            {
