...
function smarty_function_text_edit( $params, $smarty )
{
    ...
    $text = MT::getLanguage()->text($prefix, $key, $params);
    
    $keyDto = BOL_LanguageService::getInstance()->findKey($prefix, $key);
    
    if ( !$keyDto )
    {
        return '<span class="mt_red">' . $text . '</span>';
    }
    
    $script = '$("a.mt_text_edit").click(function(){
        var self=$(this), lang = this.rel.split("+");
        MT.editLanguageKey(lang[0],lang[1], function(e){
            self.text(e.value);     
        });
    });';
    ...
    return '<a href="javascript://" rel='.$rel.' class="mt_text_edit">' . $text . '</a>';
}
