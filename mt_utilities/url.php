...
class UTIL_Url
{
    ...
    */
    ...
    {
        $userFilesUrl = MT::getStorage()->getFileUrl(MT_DIR_USERFILES);
        $path = null;

        if ( stripos($uri, MT_URL_HOME) !== false )
        {
            $path = str_replace(MT_URL_HOME, MT_DIR_ROOT, $uri);
            $path = str_replace('/', DS, $path);
        }
        else if ( stripos($uri, $userFilesUrl) !== false )
        {
            $path = str_replace($userFilesUrl, MT_DIR_USERFILES, $uri);
            $path = str_replace('/', DS, $path);
        }
